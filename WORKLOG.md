...
### 2026-06-06 (최신)
- 한 일: 홈 화면(page.tsx) Mevu풍 재디자인 + 아기 카드/배너에 실제 사진 연결
  - 디자인: 히어로 풀배너+점인디케이터, 둥근 칩, 알약 NEW/BEST 뱃지, 떠오르는 하단탭, mospic 로고(이탤릭+✦)
  - 핑크 토큰(#FF4B7C) 전체 통일 (성별선택/생성버튼/단계표시/탭밑줄)
  - 카드·배너에 image 필드 추가 (있으면 사진, 없으면 이모지)
  - public/cards/ 폴더 신설 + baby.jpg 추가
  - 인기 BEST 카드 + 히어로 첫 배너에 baby.jpg 연결
  - 배너 사진: objectPosition "center 25%" (얼굴 안 잘리게) + 아래쪽 그라데이션 + 흰 글씨(사진 있을 때만)
- 다음에 할 것: 증명사진 5장(NEW카드+스타일4) / 복셀 1장 생성해서 카드 image에 연결
  - 증명사진/복셀 화면도 핑크 토큰으로 디자인 통일 (id-photo는 이미 완료)
- 주의: 이미지 파일명은 영어소문자+띄어쓰기 없이 (예: idphoto-1.jpg). public/cards/ 에 저장 후 image:"/cards/파일명" 으로 연결
