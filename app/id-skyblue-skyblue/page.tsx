...
export default function IdSkyblueSkybluePage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      void addToHistory(data.output, "증명사진 (하늘색)");
    } catch (e: unknown) {
      clearTimeout(tid);
  ...
  const handleDownload = (url: string, idx: number) => {
    ...
    a.download = `id-skyblue-${idx + 1}.png`;
    ...
    <div style={{ maxWidth: 480, margin: "0 auto", minHeight: "100vh", background: "#F7F8FA", fontFamily: "var(--font-noto), 'Apple SD Gothic Neo', sans-serif" }}>
      ...
      <div style={{ padding: "18px 18px 100px" }}>
        <div style={{ background: "#EAF3FF", borderRadius: 16, padding: "16px 18px", marginBottom: 22 }}>
          <p style={{ fontSize: 14, fontWeight: 800, color: "#3B82F6", margin: "0 0 5px" }}>☀️ 맑고 산뜻한 첫인상</p>
          <p style={{ fontSize: 12.5, color: "#6B8BB3", margin: 0, lineHeight: 1.55 }}>하늘빛 블루 셔츠에 깨끗한 하늘색 배경. 얼굴이 정면으로 잘 보이는 사진을 3~6장 올려주시면, 가장 잘 나온 증명사진 3장을 만들어드려요.</p>
        </div>
          ...
          <>
            <div style={{ background: "#fff", borderRadius: 20, padding: "20px 18px", boxShadow: "0 2px 16px rgba(0,0,0,0.04)" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                ...
                <span style={{ fontSize: 12, fontWeight: 700, color: images.length >= MIN_PHOTOS ? "#3B82F6" : "#9B9B9B" }}>{images.length}/{MAX_PHOTOS}장</span>
              </div>

              {/* 썸네일 그리드 */}
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 8 }}>
                {images.map((img, idx) => (
                  <div key={idx} style={{ position: "relative", aspectRatio: "1", borderRadius: 12, overflow: "hidden", border: "1.5px solid #3B82F6" }}>
              ...
              <p style={{ fontSize: 11.5, color: "#9B9B9B", margin: "12px 0 0", lineHeight: 1.5 }}>💡 얼굴이 정면으로 또렷하게 나온 사진일수록 결과가 좋아요. 뒷모습·옆모습·얼굴이 가려진 사진은 피해주세요.</p>
            </div>
