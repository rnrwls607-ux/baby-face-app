...
export default function IdSkyblueSkybluePage() {
  ...
  const COIN_GATED = LIVE_COIN_CONCEPTS.includes("idskyblue");
  const COIN_COST = CONCEPTS.idskyblue?.coinCost ?? 0;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/id-skyblue-skyblue", {
        method: "POST",
      ...
      void addToHistory(data.output, "증명사진 (하늘색)", Array.isArray(data.originalUrls) ? data.originalUrls : undefined);
    } catch (e: unknown) {
      clearTimeout(tid);
  ...
  const handleDownload = (url: string, idx: number) => { void saveImage(url, `id-skyblue-${idx + 1}.png`); };
  const handleShare = (url: string, idx: number) => { void shareImage(url, `id-skyblue-${idx + 1}.png`, "MOSPIC에서 만든 사진이에요 · mospic.com"); };
  ...
  return (
    <div style={{ maxWidth: 480, margin: "0 auto", minHeight: "100vh", background: "#F7F8FA", fontFamily: "var(--font-noto), 'Apple SD Gothic Neo', sans-serif" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "0 12px", height: 56, position: "sticky", top: 0, background: "#fff", zIndex: 10 }}>
        ...
        <span style={{ fontSize: 16, fontWeight: 800, color: "#191919" }}>하늘빛 블루 셔츠</span>
      </div>
      ...
      <div style={{ padding: "18px 18px 100px" }}>
        {results.length === 0 && (
          <>
            ...
            {BA_LIVE.includes("idskyblue") ? (
              <BeforeAfterHero pairs={[1, 2, 3].flatMap(n => [
                { before: `/examples/ba/idskyblue-before-${n}.webp`, after: `/examples/ba/idskyblue-after-${n}.webp` },
                { before: `/examples/ba/idskyblue-before.webp`, after: `/examples/ba/idskyblue-after-${n}.webp` },
              ])} />
            ) : (
              <PreviewCard image="/details/idskyblue.webp" caption="하늘빛 블루 셔츠 증명사진, 미리 만나보세요" accent={ACCENT} />
            )}
            ...
            <button onClick={handleSubmit} disabled={loading || images.length < MIN_PHOTOS}
              style={{ width: "100%", marginTop: 18, background: loading || images.length < MIN_PHOTOS ? "#E8E9ED" : "#3B82F6", color: loading || images.length < MIN_PHOTOS ? "#AEB2BA" : "#fff", border: "none", borderRadius: 16, padding: "16px 0", fontSize: 16, fontWeight: 800, cursor: loading || images.length < MIN_PHOTOS ? "not-allowed" : "pointer", boxShadow: loading || images.length < MIN_PHOTOS ? "none" : "0 6px 18px rgba(59,130,246,0.32)" }}>
              ...
              {results.map((url, idx) => (
                <div key={idx}>
                  <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 8 }}>
                    <span style={{ fontSize: 13, fontWeight: 800, color: "#3B82F6", background: "#EAF3FF", borderRadius: 8, padding: "3px 10px" }}>{idx + 1}번</span>
                  </div>
                  ...
                  <button onClick={() => handleDownload(url, idx)}
                    style={{ width: "100%", background: "#3B82F6", color: "#fff", border: "none", borderRadius: 14, padding: "14px 0", fontSize: 14, fontWeight: 800, cursor: "pointer", boxShadow: "0 6px 18px rgba(59,130,246,0.3)" }}>이 사진 저장하기</button>
