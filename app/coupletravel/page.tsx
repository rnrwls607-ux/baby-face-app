...
export default function CoupletravelPage() {
  ...
  const COIN_GATED = LIVE_COIN_CONCEPTS.includes("coupletravel");
  const COIN_COST = CONCEPTS.coupletravel?.coinCost ?? 0;
  ...
  const [scene, setScene] = useState(""); // 여행지 칩 — 미선택 시 버튼 비활성
  ...
  const handleSubmit = async () => {
    ...
    if (!scene) { setError("여행지를 선택해주세요."); return; }
    ...
    try {
      ...
      const res = await fetch("/api/coupletravel", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ image1: c1, image2: c2, gender1: genders[0], gender2: genders[1], scene }),
        signal: ctrl.signal,
      });
      clearTimeout(tid);
      ...
      void addToHistory(data.output, "커플 여행 스냅", Array.isArray(data.originalUrls) ? data.originalUrls : undefined);
    } catch (e: unknown) {
      clearTimeout(tid);
  ...
  const handleDownload = () => { void saveImage(result, "coupletravel.png"); };
  const handleShare = () => { void shareImage(result, "coupletravel.png", "MOSPIC에서 만든 사진이에요 · mospic.com"); };
  const canSubmit = !!images[0] && !!images[1] && !!genders[0] && !!genders[1] && !!scene && !loading;
  ...
  return (
    <div style={{ maxWidth: 480, margin: "0 auto", minHeight: "100vh", background: "#F7F8FA", fontFamily: "var(--font-noto), 'Apple SD Gothic Neo', sans-serif" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "0 12px", height: 56, position: "sticky", top: 0, background: "#fff", zIndex: 10 }}>
        ...
        <span style={{ fontSize: 16, fontWeight: 800, color: "#191919" }}>커플 여행 스냅</span>
      </div>
      <div style={{ padding: "18px 18px 100px" }}>
        <div style={{ background: "#FFEAF1", borderRadius: 16, padding: "16px 18px", marginBottom: 22 }}>
          <p style={{ fontSize: 14, fontWeight: 800, color: "#FF4B7C", margin: "0 0 5px" }}>✈️ 둘이 함께, 여행 스냅</p>
          <p style={{ fontSize: 12.5, color: "#B36B85", margin: 0, lineHeight: 1.55 }}>두 사람의 사진을 한 장씩 올리고 여행지를 고르면 함께 떠난 듯한 커플 여행 스냅을 만들어드려요. 따로 찍은 사진도 OK예요.</p>
        </div>
        {!result && (
          <>
            {/* 결과 예시 — 각자 셀카 → 함께 화보 (2인 라인 전용 구성) */}
            {BA_LIVE.includes("coupletravel") ? (
              <BeforeAfterHero pairs={[1, 2, 3, 4].flatMap(n => [
                { before: `/examples/ba/coupletravel-before-${n}.webp`, after: `/examples/ba/coupletravel-after-${n}.webp` },
                { before: `/examples/ba/coupletravel-before.webp`, after: `/examples/ba/coupletravel-after-${n}.webp` },
              ])} />
            ) : (
              <>
              <div style={{ background: "#fff", borderRadius: 20, padding: 14, marginBottom: 8, boxShadow: "0 2px 16px rgba(0,0,0,0.04)" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 6, position: "relative" }}>
                    ...
                    <img src="/examples/coupletravel_b1.webp" alt="" loading="lazy" decoding="async" style={{ width: "100%", aspectRatio: "4/5", objectFit: "cover", borderRadius: 10, display: "block" }} />
                    <img src="/examples/coupletravel_b2.webp" alt="" loading="lazy" decoding="async" style={{ width: "100%", aspectRatio: "4/5", objectFit: "cover", borderRadius: 10, display: "block" }} />
                  </div>
                  ...
                  <div style={{ flex: 2, position: "relative" }}>
                    ...
                    <img src="/examples/coupletravel_a.webp" alt="" loading="lazy" decoding="async" style={{ width: "100%", aspectRatio: "4/5", objectFit: "cover", borderRadius: 14, display: "block" }} />
                  </div>
                </div>
                <p style={{ fontSize: 11.5, color: "#9B9B9B", textAlign: "center", margin: "10px 0 2px", fontWeight: 600 }}>각자 찍은 사진 두 장이, 커플 여행 스냅으로</p>
              </div>
              </>
            )}
            ...
            <div style={{ background: "#fff", borderRadius: 20, padding: "20px 18px", boxShadow: "0 2px 16px rgba(0,0,0,0.04)" }}>
              ...
              {/* 여행지 칩 — setter 실호출 (era 버그 전례 방지) */}
              <p style={{ fontSize: 13, fontWeight: 700, color: "#191919", margin: "16px 0 8px" }}>여행지 선택</p>
              <div style={{ display: "flex", gap: 6 }}>
                {([["jeju", "✈️ 제주"], ["europe", "🏰 유럽"], ["beach", "🏖️ 해변"], ["citynight", "🌃 도시 야경"]] as const).map(([v, l]) => (
                  <button key={v} onClick={() => setScene(v)} style={{ ...chipStyle(scene === v), fontSize: 12 }}>{l}</button>
                ))}
              </div>
            </div>
            <TipChips tips={[{ icon: "face", label: "정면 얼굴" }, { icon: "sun", label: "밝은 곳에서" }, { icon: "eye", label: "얼굴 가리지 않기" }]} />
            <PrivacyLine />
            <ProCongestionHint concept="coupletravel" />
            <button onClick={handleSubmit} disabled={!canSubmit}
              ...
              {loading ? `만드는 중... (${elapsed}초)` : <>여행 스냅 만들기 ✨{COIN_GATED && COIN_COST > 0 && <span style={{ fontSize: 13, fontWeight: 700, opacity: 0.9 }}> · <CoinIcon size={14} onColor /> {COIN_COST}</span>}</>}
            </button>
          </>
        )}
        {loading && (
          <div style={{ marginTop: 28, textAlign: "center" }}>
            ...
            <p style={{ fontSize: 14, color: "#9B9B9B", marginTop: 10, fontWeight: 600 }}>AI가 두 사람을 여행지로 데려가고 있어요...</p>
          </div>
        )}
        {error && (
          <div style={{ background: "#FFEAF1", border: "1px solid #FF4B7C33", borderRadius: 12, padding: "13px 16px", marginTop: 16 }}>
            ...
            <ProCongestionError concept="coupletravel" error={error} />
        ...
        {result && (
          <div>
            ...
            <div style={{ borderRadius: 20, overflow: "hidden", marginBottom: 14, boxShadow: "0 4px 20px rgba(0,0,0,0.08)" }}>
              <img src={result} alt="커플 여행 스냅" style={{ width: "100%", display: "block" }} />
            </div>
            <div style={{ display: "flex", gap: 10 }}>
