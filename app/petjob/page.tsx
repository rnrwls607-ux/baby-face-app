...
export default function PetjobPage() {
  const router = useRouter();
  // 코인 게이트 표시·가드 (coinCost는 표시 전용 — 요금의 진실원은 서버 withCoin) — ★스위치 날 벌크 앵커
  const COIN_GATED = LIVE_COIN_CONCEPTS.includes("petjob");
  const COIN_COST = CONCEPTS.petjob?.coinCost ?? 0;
  ...
  const [job, setJob] = useState(""); // 직업 칩 — 미선택 시 버튼 비활성
  ...
  const handleSubmit = async () => {
    ...
    if (!job) { setError("직업을 선택해주세요."); return; }
    ...
    try {
      ...
      const res = await fetch("/api/petjob", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ image: compressed, job }),
        signal: ctrl.signal,
      });
      clearTimeout(tid);
      ...
      void addToHistory(data.output, "펫 직업 변신", Array.isArray(data.originalUrls) ? data.originalUrls : undefined);
    } catch (e: unknown) {
      clearTimeout(tid);
  ...
  const chipStyle = (active: boolean) => ({
    padding: "11px 0", borderRadius: 14, fontSize: 13.5, fontWeight: 800, cursor: "pointer",
    border: active ? "1.5px solid #FF4B7C" : "1.5px solid #EFF0F3",
    background: active ? "#FFEAF1" : "#fff",
    color: active ? "#FF4B7C" : "#9B9B9B",
  });
  const handleDownload = () => { void saveImage(result, "petjob.png"); };
  const handleShare = () => { void shareImage(result, "petjob.png", "MOSPIC에서 만든 사진이에요 · mospic.com"); };
  return (
    <div style={{ maxWidth: 480, margin: "0 auto", minHeight: "100vh", background: "#F7F8FA", fontFamily: "var(--font-noto), 'Apple SD Gothic Neo', sans-serif" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "0 12px", height: 56, position: "sticky", top: 0, background: "#fff", zIndex: 10 }}>
        ...
        <span style={{ fontSize: 16, fontWeight: 800, color: "#191919" }}>펫 직업 변신</span>
      </div>
      <div style={{ padding: "18px 18px 100px" }}>
        {!result && (
          <>
            {/* 결과 예시 — BA_LIVE면 비포/애프터 라이브, 아니면 기존 PreviewCard (무변화 폴백) */}
            {BA_LIVE.includes("petjob") ? (
              <BeforeAfterHero pairs={[1, 2, 3].flatMap(n => [
                { before: `/examples/ba/petjob-before-${n}.webp`, after: `/examples/ba/petjob-after-${n}.webp` },
                { before: `/examples/ba/petjob-before.webp`, after: `/examples/ba/petjob-after-${n}.webp` },
              ])} />
            ) : (
              <PreviewCard placeholder="🩺" caption="펫 직업 변신, 미리 만나보세요" />
            )}
            ...
            <UploadZone
              ...
              onRemove={() => setImage("")}
            />
            {/* 직업 칩 — setter 실호출 (era 버그 전례 방지) */}
            <div style={{ background: "#fff", borderRadius: 20, padding: "16px 18px", boxShadow: "0 2px 16px rgba(0,0,0,0.04)", marginTop: 14 }}>
              <p style={{ fontSize: 13, fontWeight: 700, color: "#191919", margin: "0 0 10px" }}>어떤 직업으로 변신할까요?</p>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 8 }}>
                {([["doctor", "🩺 의사"], ["firefighter", "🚒 소방관"], ["police", "👮 경찰"], ["chef", "👨‍🍳 셰프"]] as const).map(([v, l]) => (
                  <button key={v} onClick={() => setJob(v)} style={chipStyle(job === v)}>{l}</button>
                ))}
              </div>
            </div>
            <TipChips tips={[{ icon: "face", label: "얼굴 정면" }, { icon: "sun", label: "밝은 곳에서" }, { icon: "eye", label: "얼굴 또렷하게" }]} />
            <PrivacyLine />
            <ProCongestionHint concept="petjob" />

            <button onClick={handleSubmit} disabled={loading || !image || !job}
              style={{ width: "100%", marginTop: 18, background: loading || !image || !job ? "#E8E9ED" : "#FF4B7C", color: loading || !image || !job ? "#AEB2BA" : "#fff", border: "none", borderRadius: 16, padding: "16px 0", fontSize: 16, fontWeight: 800, cursor: loading || !image || !job ? "not-allowed" : "pointer", boxShadow: loading || !image || !job ? "none" : "0 6px 18px rgba(255,75,124,0.32)" }}>
              {loading ? `만드는 중... (${elapsed}초)` : <>직업 사진 만들기 ✨{COIN_GATED && COIN_COST > 0 && <span style={{ fontSize: 13, fontWeight: 700, opacity: 0.9 }}> · <CoinIcon size={14} onColor /> {COIN_COST}</span>}</>}
            </button>
          </>
        )}
        {loading && (
          <div style={{ marginTop: 28, textAlign: "center" }}>
            ...
            <p style={{ fontSize: 14, color: "#9B9B9B", marginTop: 10, fontWeight: 600 }}>AI가 유니폼을 맞추고 있어요...</p>
            <LoadingSaveNote />
          </div>
        )}
        {error && (
          <div style={{ background: "#FFEAF1", border: "1px solid #FF4B7C33", borderRadius: 12, padding: "13px 16px", marginTop: 16 }}>
            ...
            <ProCongestionError concept="petjob" error={error} />
        ...
        {result && (
          <div>
            ...
            <div style={{ borderRadius: 20, overflow: "hidden", marginBottom: 14, boxShadow: "0 4px 20px rgba(0,0,0,0.08)" }}>
              <img src={result} alt="펫 직업 변신" style={{ width: "100%", display: "block" }} />
            </div>
