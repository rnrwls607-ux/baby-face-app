...
export default function HairstylePage() {
  ...
  const COIN_GATED = LIVE_COIN_CONCEPTS.includes("hairstyle");
  const COIN_COST = CONCEPTS.hairstyle?.coinCost ?? 0;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      void addToHistory(data.output, "헤어 체인지", Array.isArray(data.originalUrls) ? data.originalUrls : undefined);
    } catch (e: unknown) {
      clearTimeout(tid);
  ...
  const handleDownload = () => { void saveImage(result, "hairstyle.png"); };
  const handleShare = () => { void shareImage(result, "hairstyle.png", "MOSPIC에서 만든 사진이에요 · mospic.com"); };
  return (
      ...
      <div style={{ padding: "18px 18px 100px" }}>
        {!result && (
          <>
            ...
            {BA_LIVE.includes("hairstyle") ? (
              <BeforeAfterHero pairs={[1, 2, 3].flatMap(n => [
                { before: `/examples/ba/hairstyle-before-${n}.webp`, after: `/examples/ba/hairstyle-after-${n}.webp` },
                { before: `/examples/ba/hairstyle-before.webp`, after: `/examples/ba/hairstyle-after-${n}.webp` },
              ])} />
            ) : (
              <PreviewCard placeholder="👤" caption="헤어 체인지, 미리 만나보세요" />
            )}
            ...
            <UploadZone
              ...
              onRemove={() => setImage("")}
            />
            <TipChips tips={[{ icon: "face", label: "정면 얼굴" }, { icon: "sun", label: "밝은 곳에서" }, { icon: "eye", label: "이마 보이게" }]} />
            <PrivacyLine />
            ...
            <button onClick={handleSubmit} disabled={loading || !image}
              ...
              {loading ? `만드는 중... (${elapsed}초)` : <>헤어스타일 바꿔보기 ✨{COIN_GATED && COIN_COST > 0 && <span style={{ fontSize: 13, fontWeight: 700, opacity: 0.9 }}> · <CoinIcon size={14} onColor /> {COIN_COST}</span>}</>}
            </button>
          </>
        )}
        {loading && (
