...
const ERA_OPTIONS = [
  ...
  { key: "gyeongseong", label: "🎩 1920 경성" },
  { key: "retro", label: "📻 7080 레트로" },
  { key: "medieval", label: "👑 중세 유럽" },
  { key: "future", label: "🤖 미래 도시" },
];
...
export default function EraPage() {
  ...
  const COIN_GATED = LIVE_COIN_CONCEPTS.includes("era");
  const COIN_COST = CONCEPTS.era?.coinCost ?? 0;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/era", {
        ...
        body: JSON.stringify({ image: compressed, era }),
        signal: ctrl.signal,
      });
      clearTimeout(tid);
  ...
  const handleDownload = () => { void saveImage(result, "era.png"); };
  const handleShare = () => { void shareImage(result, "era.png", "MOSPIC에서 만든 사진이에요 · mospic.com"); };
  return (
        ...
        {!result && (
          <>
            ...
            <button onClick={handleSubmit} disabled={loading || !image || !era}
              ...
              {loading ? `만드는 중... (${elapsed}초)` : <>시대 변신하기 ✨{COIN_GATED && COIN_COST > 0 && <span style={{ fontSize: 13, fontWeight: 700, opacity: 0.9 }}> · <CoinIcon size={14} onColor /> {COIN_COST}</span>}</>}
            </button>
          </>
        )}
        {loading && (
          <div style={{ marginTop: 28, textAlign: "center" }}>
            ...
            <p style={{ fontSize: 14, color: "#9B9B9B", marginTop: 10, fontWeight: 600 }}>AI가 시간 여행을 떠나고 있어요...</p>
            <LoadingSaveNote />
          </div>
        )}
        {error && (
