...
export default function BizManDbPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      void addToHistory(data.output, "비즈프로필 (남성·더블브레스티드)");
    } catch (e: unknown) {
      clearTimeout(tid);
  ...
  const handleDownload = (url: string, idx: number) => {
    ...
    a.download = `biz-man-db-${idx + 1}.png`;
    ...
    <div style={{ maxWidth: 480, margin: "0 auto", minHeight: "100vh", background: "#F7F8FA", fontFamily: "var(--font-noto), 'Apple SD Gothic Neo', sans-serif" }}>
      ...
      <div style={{ padding: "18px 18px 100px" }}>
        <div style={{ background: "#EAF3FF", borderRadius: 16, padding: "16px 18px", marginBottom: 22 }}>
          <p style={{ fontSize: 14, fontWeight: 800, color: "#3B5BA5", margin: "0 0 5px" }}>💼 남성 더블브레스티드</p>
          <p style={{ fontSize: 12.5, color: "#6B7DA8", margin: 0, lineHeight: 1.55 }}>깔끔한 네이비 더블브레스티드 정장, 차분한 회색 배경의 남성 전문가 프로필이에요. 얼굴이 정면으로 잘 보이는 사진을 3~6장 올려주시면, 팔짱·두 손 모음·자연스러운 각도까지 서로 다른 포즈의 프로필 3장을 만들어드려요.</p>
        </div>
