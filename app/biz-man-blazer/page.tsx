...
export default function BizManBlazerPage() {
  ...
  const COIN_GATED = LIVE_COIN_CONCEPTS.includes("bizmblazer");
  const COIN_COST = CONCEPTS.bizmblazer?.coinCost ?? 0;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      void addToHistory(data.output, "비즈프로필 (남성·네이비 블레이저)", Array.isArray(data.originalUrls) ? data.originalUrls : undefined);
    } catch (e: unknown) {
      clearTimeout(tid);
  ...
  const handleDownload = (url: string, idx: number) => { void saveImage(url, `biz-man-blazer-${idx + 1}.png`); };
  const handleShare = (url: string, idx: number) => { void shareImage(url, `biz-man-blazer-${idx + 1}.png`, "MOSPIC에서 만든 사진이에요 · mospic.com"); };
