...
export default function BizBlueskirtGrayPage() {
  ...
  const COIN_GATED = LIVE_COIN_CONCEPTS.includes("bizblueskirt");
  const COIN_COST = CONCEPTS.bizblueskirt?.coinCost ?? 0;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/biz-blueskirt-gray", {
        method: "POST",
      ...
      void addToHistory(data.output, "비즈프로필 (블루스커트)", Array.isArray(data.originalUrls) ? data.originalUrls : undefined);
    } catch (e: unknown) {
      clearTimeout(tid);
  ...
  const handleDownload = (url: string, idx: number) => { void saveImage(url, `biz-blueskirt-gray-${idx + 1}.png`); };
  const handleShare = (url: string, idx: number) => { void shareImage(url, `biz-blueskirt-gray-${idx + 1}.png`, "MOSPIC에서 만든 사진이에요 · mospic.com"); };
  ...
  return (
    <div style={{ maxWidth: 480, margin: "0 auto", minHeight: "100vh", background: "#F7F8FA", fontFamily: "var(--font-noto), 'Apple SD Gothic Neo', sans-serif" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "0 12px", height: 56, position: "sticky", top: 0, background: "#fff", zIndex: 10 }}>
        ...
        <span style={{ fontSize: 16, fontWeight: 800, color: "#191919" }}>블루 스커트 프로필</span>
      </div>
      ...
      <div style={{ padding: "18px 18px 100px" }}>
        {results.length === 0 && (
          <>
            ...
            {BA_LIVE.includes("bizblueskirt") ? (
              <BeforeAfterHero pairs={[1, 2, 3].flatMap(n => [
                { before: `/examples/ba/bizblueskirt-before-${n}.webp`, after: `/examples/ba/bizblueskirt-after-${n}.webp` },
                { before: `/examples/ba/bizblueskirt-before.webp`, after: `/examples/ba/bizblueskirt-after-${n}.webp` },
              ])} />
            ) : (
              <PreviewCard image="/details/bizblueskirt.webp" caption="블루 스커트 프로필, 미리 만나보세요" accent={ACCENT} />
            )}
