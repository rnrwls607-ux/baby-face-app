...
export default function CampsnapPage() {
  const router = useRouter();
  // 코인 게이트 표시·가드 (coinCost는 표시 전용 — 요금의 진실원은 서버 withCoin) — ★스위치 날 벌크 앵커
  const COIN_GATED = LIVE_COIN_CONCEPTS.includes("campsnap");
  const COIN_COST = CONCEPTS.campsnap?.coinCost ?? 0;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/campsnap", {
        method: "POST",
      ...
      void addToHistory(data.output, "감성 캠핑 스냅", Array.isArray(data.originalUrls) ? data.originalUrls : undefined);
    } catch (e: unknown) {
      clearTimeout(tid);
  ...
  const handleDownload = () => { void saveImage(result, "campsnap.png"); };
  const handleShare = () => { void shareImage(result, "campsnap.png", "MOSPIC에서 만든 사진이에요 · mospic.com"); };
  return (
    <div style={{ maxWidth: 480, margin: "0 auto", minHeight: "100vh", background: "#fff", fontFamily: "var(--font-noto), 'Apple SD Gothic Neo', sans-serif" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "0 16px", height: 56, borderBottom: "1px solid #f0f0f0", position: "sticky", top: 0, background: "#fff", zIndex: 10 }}>
        ...
        <span style={{ fontSize: 16, fontWeight: 700, color: "#111" }}>감성 캠핑 스냅</span>
      </div>
      <div style={{ padding: "20px 20px 100px" }}>
        {!result && (
          <>
            {/* 결과 예시 — BA_LIVE면 비포/애프터 라이브, 아니면 기존 PreviewCard (무변화 폴백) */}
            {BA_LIVE.includes("campsnap") ? (
              <BeforeAfterHero pairs={[1, 2, 3].flatMap(n => [
                { before: `/examples/ba/campsnap-before-${n}.webp`, after: `/examples/ba/campsnap-after-${n}.webp` },
                { before: `/examples/ba/campsnap-before.webp`, after: `/examples/ba/campsnap-after-${n}.webp` },
              ])} />
            ) : (
              <PreviewCard placeholder="🏕️" caption="감성 캠핑 스냅, 미리 만나보세요" />
            )}
            ...
            <PrivacyLine />
            <ProCongestionHint concept="campsnap" />
            <button onClick={handleSubmit} disabled={loading || !image}
              ...
              {loading ? `만드는 중... (${elapsed}초)` : <>감성 캠핑 스냅 만들기 🏕️{COIN_GATED && COIN_COST > 0 && <span style={{ fontSize: 13, fontWeight: 700, opacity: 0.9 }}> · <CoinIcon size={14} onColor /> {COIN_COST}</span>}</>}
            </button>
          </>
        )}
        {loading && (
          <div style={{ marginTop: 24, textAlign: "center" }}>
            ...
            <p style={{ fontSize: 14, color: "#888", marginTop: 8 }}>AI가 감성 캠핑 스냅로 스타일링하고 있어요...</p>
            <LoadingSaveNote />
          </div>
        )}
        {error && (
          <div style={{ background: "#FFF0F3", border: "1px solid #FFD6E0", borderRadius: 12, padding: "12px 16px", marginTop: 16 }}>
            ...
            <ProCongestionError concept="campsnap" error={error} />
        ...
        {result && (
          <div>
            ...
            <div style={{ borderRadius: 16, overflow: "hidden", marginBottom: 12 }}>
              <img src={result} alt="감성 캠핑 스냅" style={{ width: "100%", display: "block" }} />
            </div>
