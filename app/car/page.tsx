...
export default function CarPage() {
  ...
  const COIN_GATED = LIVE_COIN_CONCEPTS.includes("car");
  const COIN_COST = CONCEPTS.car?.coinCost ?? 0;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      void addToHistory(data.output, "중고차 사진", Array.isArray(data.originalUrls) ? data.originalUrls : undefined);
    } catch (e: unknown) {
      clearTimeout(tid);
  ...
  const handleDownload = () => { void saveImage(result, "car.png"); };
  const handleShare = () => { void shareImage(result, "car.png", "MOSPIC에서 만든 사진이에요 · mospic.com"); };
  return (
      ...
      <div style={{ padding: "18px 18px 100px" }}>
        {!result && (
          <>
            ...
            {BA_LIVE.includes("car") ? (
              <BeforeAfterHero pairs={[1, 2, 3].flatMap(n => [
                { before: `/examples/ba/car-before-${n}.webp`, after: `/examples/ba/car-after-${n}.webp` },
                { before: `/examples/ba/car-before.webp`, after: `/examples/ba/car-after-${n}.webp` },
              ])} />
            ) : (
            ...
            <UploadZone
              ...
              onRemove={() => setImage("")}
            />
            <TipChips tips={[{ icon: "expand", label: "차 전체 담기" }, { icon: "sun", label: "밝은 곳에서" }, { icon: "level", label: "수평 맞추기" }]} />
            <PrivacyLine />
            ...
            <button onClick={handleSubmit} disabled={loading || !image}
              ...
              {loading ? `만드는 중... (${elapsed}초)` : <>중고차 사진 보정하기 ✨{COIN_GATED && COIN_COST > 0 && <span style={{ fontSize: 13, fontWeight: 700, opacity: 0.9 }}> · <CoinIcon size={14} onColor /> {COIN_COST}</span>}</>}
            </button>
          </>
        )}
        {loading && (
