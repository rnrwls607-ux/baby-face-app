...
export default function BizKhakiGrayPage() {
  ...
  const COIN_GATED = LIVE_COIN_CONCEPTS.includes("bizkhaki");
  const COIN_COST = CONCEPTS.bizkhaki?.coinCost ?? 0;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      void addToHistory(data.output, "비즈프로필 (카키수트)", Array.isArray(data.originalUrls) ? data.originalUrls : undefined);
    } catch (e: unknown) {
      clearTimeout(tid);
  ...
  const handleDownload = (url: string, idx: number) => { void saveImage(url, `biz-khaki-gray-${idx + 1}.png`); };
  const handleShare = (url: string, idx: number) => { void shareImage(url, `biz-khaki-gray-${idx + 1}.png`, "MOSPIC에서 만든 사진이에요 · mospic.com"); };
