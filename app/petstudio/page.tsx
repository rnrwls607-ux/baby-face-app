...
export default function PetstudioPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      void addToHistory(data.output, "펫 스튜디오", Array.isArray(data.originalUrls) ? data.originalUrls : undefined);
    } catch (e: unknown) {
      clearTimeout(tid);
  ...
  const handleDownload = () => { void saveImage(result, "petstudio.png"); };
  const handleShare = () => { void shareImage(result, "petstudio.png", "MOSPIC에서 만든 사진이에요 · mospic.com"); };
  return (
      ...
      <div style={{ padding: "18px 18px 100px" }}>
        {!result && (
          <>
            ...
            {BA_LIVE.includes("petstudio") ? (
              <BeforeAfterHero pairs={[1, 2, 3].flatMap(n => [
                { before: `/examples/ba/petstudio-before-${n}.webp`, after: `/examples/ba/petstudio-after-${n}.webp` },
                { before: `/examples/ba/petstudio-before.webp`, after: `/examples/ba/petstudio-after-${n}.webp` },
              ])} />
            ) : (
              <PreviewCard placeholder="🐾" caption="펫 스튜디오 화보, 미리 만나보세요" />
            )}
