...
export default function SportyPage() {
  ...
  const COIN_GATED = LIVE_COIN_CONCEPTS.includes("sporty");
  const COIN_COST = CONCEPTS.sporty?.coinCost ?? 0;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/sporty", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ image: compressed, option }),
        signal: ctrl.signal,
      });
      clearTimeout(tid);
      ...
      const optionLabel = OPTIONS.find(o => o.key === option)?.label || "스포티 화보";
      void addToHistory(data.output, `스포티 화보 ${optionLabel}`, Array.isArray(data.originalUrls) ? data.originalUrls : undefined);
    } catch (e: unknown) {
      clearTimeout(tid);
  ...
  const handleDownload = () => { void saveImage(result, "sporty.png"); };
  const handleShare = () => { void shareImage(result, "sporty.png", "MOSPIC에서 만든 사진이에요 · mospic.com"); };
  return (
    <div style={{ maxWidth: 480, margin: "0 auto", minHeight: "100vh", background: "#F7F8FA", fontFamily: "var(--font-noto), 'Apple SD Gothic Neo', sans-serif" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "0 12px", height: 56, position: "sticky", top: 0, background: "#fff", zIndex: 10 }}>
        ...
        <span style={{ fontSize: 16, fontWeight: 800, color: "#191919" }}>스포티 화보</span>
      </div>
      <div style={{ padding: "18px 18px 100px" }}>
        {!result && (
          <>
            {/* 결과 예시 — BA_LIVE면 비포/애프터 라이브, 아니면 기존 PreviewCard (무변화 폴백) */}
            {BA_LIVE.includes("sporty") ? (
              <BeforeAfterHero pairs={[1, 2, 3].flatMap(n => [
                { before: `/examples/ba/sporty-before-${n}.webp`, after: `/examples/ba/sporty-after-${n}.webp` },
                { before: `/examples/ba/sporty-before.webp`, after: `/examples/ba/sporty-after-${n}.webp` },
              ])} />
            ) : (
              <PreviewCard placeholder="🎾" caption="올드머니 스포티 화보, 미리 만나보세요" />
            )}
            ...
            <div style={{ background: "#fff", borderRadius: 20, padding: "18px 18px", boxShadow: "0 2px 16px rgba(0,0,0,0.04)", marginBottom: 18 }}>
              <p style={{ fontSize: 13, fontWeight: 700, color: "#191919", marginBottom: 10, marginTop: 0 }}>어떤 스포츠 룩으로 할까요?</p>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 8 }}>
                {OPTIONS.map(o => (
                  <button key={o.key} onClick={() => setOption(o.key)} style={chipStyle(option === o.key)}>{o.label}</button>
                ))}
              </div>
            </div>
            <UploadZone
            ...
            <PrivacyLine />
            <button onClick={handleSubmit} disabled={loading || !image || !option}
              style={{ width: "100%", marginTop: 18, background: loading || !image || !option ? "#E8E9ED" : "#FF4B7C", color: loading || !image || !option ? "#AEB2BA" : "#fff", border: "none", borderRadius: 16, padding: "16px 0", fontSize: 16, fontWeight: 800, cursor: loading || !image || !option ? "not-allowed" : "pointer", boxShadow: loading || !image || !option ? "none" : "0 6px 18px rgba(255,75,124,0.32)" }}>
              {loading ? `만드는 중... (${elapsed}초)` : <>스포티 화보 만들기 ✨{COIN_GATED && COIN_COST > 0 && <span style={{ fontSize: 13, fontWeight: 700, opacity: 0.9 }}> · <CoinIcon size={14} onColor /> {COIN_COST}</span>}</>}
            </button>
          </>
        )}
        {loading && (
          <div style={{ marginTop: 28, textAlign: "center" }}>
            ...
            <p style={{ fontSize: 14, color: "#9B9B9B", marginTop: 10, fontWeight: 600 }}>AI가 스포티 화보를 촬영하는 중...</p>
            <LoadingSaveNote />
          </div>
        )}
        {error && (
        ...
        {result && (
          <div>
            ...
            <div style={{ borderRadius: 20, overflow: "hidden", marginBottom: 14, boxShadow: "0 4px 20px rgba(0,0,0,0.08)" }}>
              <img src={result} alt="스포티 화보" style={{ width: "100%", display: "block" }} />
            </div>
            <div style={{ display: "flex", gap: 10 }}>
              ...
              <button onClick={() => { setResult(""); setOption(""); }}
                style={{ flex: 1, background: "#fff", color: "#191919", border: "1.5px solid #EFF0F3", borderRadius: 14, padding: "15px 0", fontSize: 14, fontWeight: 700, cursor: "pointer" }}>다른 스포츠로</button>
            </div>
            ...
            <p style={{ fontSize: 11, color: "#BFC3CB", textAlign: "center", marginTop: 14, lineHeight: 1.6 }}>※ 사진은 유지돼요. &quot;다른 스포츠로&quot;를 누르고 옵션만 바꿔서 또 만들어보세요!</p>
          </div>
        )}
      </div>
    </div>
