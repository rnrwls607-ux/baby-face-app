...
export default function BizShirringGrayPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/biz-shirring-gray", {
        method: "POST",
      ...
      void addToHistory(data.output, "비즈프로필 (셔링블라우스)");
    } catch (e: unknown) {
      clearTimeout(tid);
  ...
  const handleDownload = (url: string, idx: number) => {
    ...
    a.download = `biz-shirring-gray-${idx + 1}.png`;
  ...
  return (
    <div style={{ maxWidth: 480, margin: "0 auto", minHeight: "100vh", background: "#F7F8FA", fontFamily: "var(--font-noto), 'Apple SD Gothic Neo', sans-serif" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "0 12px", height: 56, position: "sticky", top: 0, background: "#fff", zIndex: 10 }}>
        ...
        <span style={{ fontSize: 16, fontWeight: 800, color: "#191919" }}>셔링 블라우스 프로필</span>
      </div>

      <div style={{ padding: "18px 18px 100px" }}>
        <div style={{ background: "#FFF0F5", borderRadius: 16, padding: "16px 18px", marginBottom: 22 }}>
          <p style={{ fontSize: 14, fontWeight: 800, color: ACCENT, margin: "0 0 5px" }}>🎀 우아한 셔링 블라우스</p>
          <p style={{ fontSize: 12.5, color: "#7A6A70", margin: 0, lineHeight: 1.55 }}>부드러운 셔링 블라우스와 베이지 스커트, 밝은 회색 배경의 우아한 여성 전문가 프로필이에요. 정면 사진 3~6장을 올리면, 팔짱·손모음·자연스러운 각도까지 서로 다른 포즈의 프로필 3장을 만들어드려요. (여성 전용 컨셉이에요.)</p>
        </div>
