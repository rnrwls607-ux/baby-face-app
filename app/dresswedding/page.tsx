...
const ROLE_OPTIONS: { key: "bride" | "groom"; label: string }[] = [
  { key: "bride", label: "👰 신부 (드레스)" },
  { key: "groom", label: "🤵 신랑 (턱시도)" },
];
const BG_OPTIONS = [
  { key: "hall", label: "💒 웨딩홀" },
  { key: "garden", label: "🌿 가든" },
...
export default function DressweddingPage() {
  const router = useRouter();
  // 코인 게이트 표시·가드 (coinCost는 표시 전용 — 요금의 진실원은 서버 withCoin) — ★스위치 날 벌크 앵커
  const COIN_GATED = LIVE_COIN_CONCEPTS.includes("dresswedding");
  const COIN_COST = CONCEPTS.dresswedding?.coinCost ?? 0;
  ...
  const [role, setRole] = useState<"bride" | "groom">("bride"); // 기본값 신부
  const [bg, setBg] = useState<string>("hall");                 // 기본값 웨딩홀
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState("");
  const [error, setError] = useState("");
  const [elapsed, setElapsed] = useState(0);
  const [pendingPick, setPendingPick] = useState<string | null>(null); // 배경 재생성 확인 대기 — 과금 전 1회 확인
  ...
  const handleSubmit = async (pick?: string) => {
    ...
    try {
      ...
      const res = await fetch("/api/dresswedding", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ image: compressed, role, bg: chosenBg }),
        signal: ctrl.signal,
      });
      clearTimeout(tid);
      ...
      void addToHistory(data.output, "웨딩 스냅", Array.isArray(data.originalUrls) ? data.originalUrls : undefined);
    } catch (e: unknown) {
      clearTimeout(tid);
  ...
  const handleDownload = () => { void saveImage(result, "dresswedding.png"); };
  const handleShare = () => { void shareImage(result, "dresswedding.png", "MOSPIC에서 만든 사진이에요 · mospic.com"); };
  return (
    <div style={{ maxWidth: 480, margin: "0 auto", minHeight: "100vh", background: "#fff", fontFamily: "var(--font-noto), 'Apple SD Gothic Neo', sans-serif" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "0 16px", height: 56, borderBottom: "1px solid #f0f0f0", position: "sticky", top: 0, background: "#fff", zIndex: 10 }}>
        ...
        <span style={{ fontSize: 16, fontWeight: 700, color: "#111" }}>웨딩 스냅</span>
      </div>
      <div style={{ padding: "20px 20px 100px" }}>
        {!result && (
          <>
            {/* 결과 예시 — BA_LIVE면 비포/애프터 라이브, 아니면 기존 PreviewCard (무변화 폴백) */}
            {BA_LIVE.includes("dresswedding") ? (
              <BeforeAfterHero pairs={[1, 2, 3].flatMap(n => [
                { before: `/examples/ba/dresswedding-before-${n}.webp`, after: `/examples/ba/dresswedding-after-${n}.webp` },
                { before: `/examples/ba/dresswedding-before.webp`, after: `/examples/ba/dresswedding-after-${n}.webp` },
              ])} />
            ) : (
              <PreviewCard placeholder="💍" caption="웨딩 스냅, 미리 만나보세요" />
            )}
            <StepIndicator current={result ? 3 : loading ? 2 : 1} />
            {/* ★2단 선택 — 1단 성별(의상) → 2단 배경 */}
            <div style={{ background: "#fff", borderRadius: 20, padding: "18px 18px", boxShadow: "0 2px 16px rgba(0,0,0,0.04)", marginBottom: 18 }}>
              <p style={{ fontSize: 13, fontWeight: 700, color: "#191919", marginBottom: 10, marginTop: 0 }}>어떤 의상으로 입을까요?</p>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 8 }}>
                {ROLE_OPTIONS.map(o => (
                  <button key={o.key} onClick={() => setRole(o.key)} style={chipStyle(role === o.key)}>{o.label}</button>
                ))}
              </div>
              <p style={{ fontSize: 13, fontWeight: 700, color: "#191919", margin: "18px 0 10px" }}>배경은 어디로 할까요?</p>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 8 }}>
                {BG_OPTIONS.map(o => (
                  <button key={o.key} onClick={() => setBg(o.key)} style={chipStyle(bg === o.key)}>{o.label}</button>
                ))}
              </div>
            </div>
            <UploadZone
              ...
              cameraFacing="user"
            />
            <TipChips tips={[{ icon: "face", label: "정면 얼굴" }, { icon: "sun", label: "밝은 곳에서" }, { icon: "eye", label: "어깨까지 보이게" }]} />
            <PrivacyLine />
            <ProCongestionHint concept="dresswedding" />
            <button onClick={() => handleSubmit()} disabled={loading || !image}
              ...
              {loading ? `만드는 중... (${elapsed}초)` : <>웨딩 스냅 만들기 💍{COIN_GATED && COIN_COST > 0 && <span style={{ fontSize: 13, fontWeight: 700, opacity: 0.9 }}> · <CoinIcon size={14} onColor /> {COIN_COST}</span>}</>}
            </button>
          </>
        )}
        {loading && (
        ...
        {error && (
          <div style={{ background: "#FFF0F3", border: "1px solid #FFD6E0", borderRadius: 12, padding: "12px 16px", marginTop: 16 }}>
            ...
            <ProCongestionError concept="dresswedding" error={error} />
        ...
        {result && (
          <div>
            ...
            <div style={{ borderRadius: 16, overflow: "hidden", marginBottom: 12 }}>
              <img src={result} alt="웨딩 스냅" style={{ width: "100%", display: "block" }} />
            </div>
            {/* 다른 배경으로 — 사진을 다시 올리지 않고 칩만 눌러 재생성 (의상은 위에서 고른 그대로) */}
            <div style={{ background: "#fff", borderRadius: 20, padding: "18px 18px", boxShadow: "0 2px 16px rgba(0,0,0,0.04)", margin: "0 0 12px" }}>
              <p style={{ fontSize: 13, fontWeight: 700, color: "#191919", marginBottom: 10, marginTop: 0 }}>다른 배경에서도 만들어볼까요?</p>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 8 }}>
                {BG_OPTIONS.map(o => (
                  <button key={o.key} onClick={() => (COIN_GATED && COIN_COST > 0 ? setPendingPick(o.key) : void handleSubmit(o.key))} disabled={loading} style={chipStyle(bg === o.key)}>{o.label}</button>
                ))}
              </div>
            </div>
      ...
      <RegenConfirmSheet open={pendingPick !== null} question="다른 배경으로 새로 만들까요?" cost={COIN_COST}
