...
export default function BizManCharcoalPage() {
  ...
  const COIN_GATED = LIVE_COIN_CONCEPTS.includes("bizmcharcoal");
  const COIN_COST = CONCEPTS.bizmcharcoal?.coinCost ?? 0;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      void addToHistory(data.output, "비즈프로필 (남성·차콜 정장)", Array.isArray(data.originalUrls) ? data.originalUrls : undefined);
    } catch (e: unknown) {
      clearTimeout(tid);
      ...
      <div style={{ padding: "18px 18px 100px" }}>
        {results.length === 0 && (
          <>
            ...
            {BA_LIVE.includes("bizmcharcoal") ? (
              <BeforeAfterHero pairs={[1, 2, 3, 4, 5, 6].flatMap(n => [
                { before: `/examples/ba/bizmcharcoal-before-${n}.webp`, after: `/examples/ba/bizmcharcoal-after-${n}.webp` },
                { before: `/examples/ba/bizmcharcoal-before.webp`, after: `/examples/ba/bizmcharcoal-after-${n}.webp` },
              ])} />
            ) : (
