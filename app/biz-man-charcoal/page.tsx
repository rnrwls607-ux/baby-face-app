...
export default function BizManCharcoalPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/biz-man-charcoal", {
        method: "POST",
      ...
      void addToHistory(data.output, "비즈프로필 (남성·차콜 정장)");
    } catch (e: unknown) {
      clearTimeout(tid);
  ...
  const handleDownload = (url: string, idx: number) => {
    ...
    a.download = `biz-man-charcoal-${idx + 1}.png`;
  ...
  return (
    <div style={{ maxWidth: 480, margin: "0 auto", minHeight: "100vh", background: "#F7F8FA", fontFamily: "var(--font-noto), 'Apple SD Gothic Neo', sans-serif" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "0 12px", height: 56, position: "sticky", top: 0, background: "#fff", zIndex: 10 }}>
        ...
        <span style={{ fontSize: 16, fontWeight: 800, color: "#191919" }}>남성 차콜 정장</span>
      </div>

      <div style={{ padding: "18px 18px 100px" }}>
        <div style={{ background: "#EAF3FF", borderRadius: 16, padding: "16px 18px", marginBottom: 22 }}>
          <p style={{ fontSize: 14, fontWeight: 800, color: "#3B5BA5", margin: "0 0 5px" }}>💼 남성 차콜 정장</p>
          <p style={{ fontSize: 12.5, color: "#6B7DA8", margin: 0, lineHeight: 1.55 }}>깔끔한 차콜 그레이 정장, 차분한 회색 배경의 남성 전문가 프로필이에요. 얼굴이 정면으로 잘 보이는 사진을 3~6장 올려주시면, 팔짱·두 손 모음·자연스러운 각도까지 서로 다른 포즈의 프로필 3장을 만들어드려요.</p>
        </div>
