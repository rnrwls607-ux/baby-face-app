...
export default function IdBurgundyPage() {
  ...
  const COIN_GATED = LIVE_COIN_CONCEPTS.includes("idburgundy");
  const COIN_COST = CONCEPTS.idburgundy?.coinCost ?? 0;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/id-burgundy", {
        method: "POST",
      ...
      void addToHistory(data.output, "증명사진 (버건디 오프숄더)", Array.isArray(data.originalUrls) ? data.originalUrls : undefined);
    } catch (e: unknown) {
      clearTimeout(tid);
  ...
  const handleDownload = (url: string, idx: number) => { void saveImage(url, `id-burgundy-${idx + 1}.png`); };
  const handleShare = (url: string, idx: number) => { void shareImage(url, `id-burgundy-${idx + 1}.png`, "MOSPIC에서 만든 사진이에요 · mospic.com"); };
  ...
  return (
    <div style={{ maxWidth: 480, margin: "0 auto", minHeight: "100vh", background: "#F7F8FA", fontFamily: "var(--font-noto), 'Apple SD Gothic Neo', sans-serif" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "0 12px", height: 56, position: "sticky", top: 0, background: "#fff", zIndex: 10 }}>
        ...
        <span style={{ fontSize: 16, fontWeight: 800, color: "#191919" }}>버건디 오프숄더 프로필</span>
      </div>
      ...
      <div style={{ padding: "18px 18px 100px" }}>
        {results.length === 0 && (
          <>
            ...
            {BA_LIVE.includes("idburgundy") ? (
              <BeforeAfterHero pairs={[1, 2, 3].flatMap(n => [
                { before: `/examples/ba/idburgundy-before-${n}.webp`, after: `/examples/ba/idburgundy-after-${n}.webp` },
                { before: `/examples/ba/idburgundy-before.webp`, after: `/examples/ba/idburgundy-after-${n}.webp` },
              ])} />
            ) : (
        ...
        {loading && (
          <div style={{ marginTop: 28, textAlign: "center" }}>
            ...
            <p style={{ fontSize: 14, color: "#9B9B9B", marginTop: 10, fontWeight: 600 }}>AI가 프로필 3장을 각각 만들고 있어요...</p>
