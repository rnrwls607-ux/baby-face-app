...
export default function GhostfitPage() {
  const router = useRouter();
  // 코인 게이트 표시·가드 (coinCost는 표시 전용 — 요금의 진실원은 서버 withCoin) — ★스위치 날 벌크 앵커
  const COIN_GATED = LIVE_COIN_CONCEPTS.includes("ghostfit");
  const COIN_COST = CONCEPTS.ghostfit?.coinCost ?? 0;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/ghostfit", {
        method: "POST",
      ...
      void addToHistory(data.output, "고스트 마네킹 착장컷", Array.isArray(data.originalUrls) ? data.originalUrls : undefined);
    } catch (e: unknown) {
      clearTimeout(tid);
  ...
  const handleDownload = () => { void saveImage(result, "ghostfit.png"); };
  const handleShare = () => { void shareImage(result, "ghostfit.png", "MOSPIC에서 만든 사진이에요 · mospic.com"); };
  return (
    <div style={{ maxWidth: 480, margin: "0 auto", minHeight: "100vh", background: "#fff", fontFamily: "var(--font-noto), 'Apple SD Gothic Neo', sans-serif" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "0 16px", height: 56, borderBottom: "1px solid #f0f0f0", position: "sticky", top: 0, background: "#fff", zIndex: 10 }}>
        ...
        <span style={{ fontSize: 16, fontWeight: 700, color: "#111" }}>고스트 마네킹 착장컷</span>
      </div>
      <div style={{ padding: "20px 20px 100px" }}>
        {!result && (
          <>
            {/* 결과 예시 — BA_LIVE면 비포/애프터 라이브, 아니면 기존 PreviewCard (무변화 폴백) */}
            {BA_LIVE.includes("ghostfit") ? (
              <BeforeAfterHero pairs={[1, 2, 3].flatMap(n => [
                { before: `/examples/ba/ghostfit-before-${n}.webp`, after: `/examples/ba/ghostfit-after-${n}.webp` },
                { before: `/examples/ba/ghostfit-before.webp`, after: `/examples/ba/ghostfit-after-${n}.webp` },
              ])} />
            ) : (
              <PreviewCard placeholder="👕" caption="고스트 마네킹 착장컷, 미리 만나보세요" />
            )}
            ...
            <button onClick={() => handleSubmit()} disabled={loading || !image}
              ...
              {loading ? `만드는 중... (${elapsed}초)` : <>고스트 마네킹 착장컷 만들기 👕{COIN_GATED && COIN_COST > 0 && <span style={{ fontSize: 13, fontWeight: 700, opacity: 0.9 }}> · <CoinIcon size={14} onColor /> {COIN_COST}</span>}</>}
            </button>
          </>
        )}
        {loading && (
          <div style={{ marginTop: 24, textAlign: "center" }}>
            ...
            <p style={{ fontSize: 14, color: "#888", marginTop: 8 }}>AI가 고스트 마네킹 착장컷 만드는 중이에요...</p>
            <LoadingSaveNote />
          </div>
        )}
        {error && (
        ...
        {result && (
          <div>
            ...
            <div style={{ borderRadius: 16, overflow: "hidden", marginBottom: 12 }}>
              <img src={result} alt="고스트 마네킹 착장컷" style={{ width: "100%", display: "block" }} />
            </div>
