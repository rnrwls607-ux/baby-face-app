...
export default function FoodPage() {
  ...
  const COIN_GATED = LIVE_COIN_CONCEPTS.includes("food");
  const COIN_COST = CONCEPTS.food?.coinCost ?? 0;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      void addToHistory(data.output, "음식 사진", Array.isArray(data.originalUrls) ? data.originalUrls : undefined);
    } catch (e: unknown) {
      clearTimeout(tid);
  ...
  const handleDownload = () => { void saveImage(result, "food.png"); };
  const handleShare = () => { void shareImage(result, "food.png", "MOSPIC에서 만든 사진이에요 · mospic.com"); };
  return (
      ...
      <div style={{ padding: "18px 18px 100px" }}>
        {!result && (
          <>
            ...
            {BA_LIVE.includes("food") ? (
              <BeforeAfterHero pairs={[1, 2, 3].flatMap(n => [
                { before: `/examples/ba/food-before-${n}.webp`, after: `/examples/ba/food-after-${n}.webp` },
                { before: `/examples/ba/food-before.webp`, after: `/examples/ba/food-after-${n}.webp` },
              ])} />
            ) : (
            ...
            <button onClick={handleSubmit} disabled={loading || !image}
              ...
              {loading ? `만드는 중... (${elapsed}초)` : <>음식 사진 보정하기 ✨{COIN_GATED && COIN_COST > 0 && <span style={{ fontSize: 13, fontWeight: 700, opacity: 0.9 }}> · <CoinIcon size={14} onColor /> {COIN_COST}</span>}</>}
            </button>
          </>
        )}
        {loading && (
          <div style={{ marginTop: 28, textAlign: "center" }}>
            ...
            <p style={{ fontSize: 14, color: "#9B9B9B", marginTop: 10, fontWeight: 600 }}>AI가 먹음직스럽게 다듬고 있어요...</p>
            <LoadingSaveNote />
          </div>
        )}
        {error && (
