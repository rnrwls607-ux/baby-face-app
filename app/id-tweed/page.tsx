...
export default function IdTweedPage() {
  ...
  const COIN_GATED = LIVE_COIN_CONCEPTS.includes("idtweed");
  const COIN_COST = CONCEPTS.idtweed?.coinCost ?? 0;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/id-tweed", {
        method: "POST",
      ...
      void addToHistory(data.output, "증명사진 (반묶음 트위드)", Array.isArray(data.originalUrls) ? data.originalUrls : undefined);
    } catch (e: unknown) {
      clearTimeout(tid);
  ...
  const handleDownload = (url: string, idx: number) => { void saveImage(url, `id-tweed-${idx + 1}.png`); };
  const handleShare = (url: string, idx: number) => { void shareImage(url, `id-tweed-${idx + 1}.png`, "MOSPIC에서 만든 사진이에요 · mospic.com"); };
  ...
  return (
    <div style={{ maxWidth: 480, margin: "0 auto", minHeight: "100vh", background: "#F7F8FA", fontFamily: "var(--font-noto), 'Apple SD Gothic Neo', sans-serif" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "0 12px", height: 56, position: "sticky", top: 0, background: "#fff", zIndex: 10 }}>
        ...
        <span style={{ fontSize: 16, fontWeight: 800, color: "#191919" }}>반묶음 트위드 증명사진</span>
      </div>
      ...
      <div style={{ padding: "18px 18px 100px" }}>
        {results.length === 0 && (
          <>
            ...
            {BA_LIVE.includes("idtweed") ? (
              <div style={{ marginBottom: 8 }}>
                <BeforeAfterHero pairs={[1, 2, 3].flatMap(n => [
                  { before: `/examples/ba/idtweed-before-${n}.webp`, after: `/examples/ba/idtweed-after-${n}.webp` },
                  { before: `/examples/ba/idtweed-before.webp`, after: `/examples/ba/idtweed-after-${n}.webp` },
                ])} />
              </div>
            ) : (
