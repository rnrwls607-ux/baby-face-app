...
export default function BizManLightgrayPage() {
  ...
  const COIN_GATED = LIVE_COIN_CONCEPTS.includes("bizmlightgray");
  const COIN_COST = CONCEPTS.bizmlightgray?.coinCost ?? 0;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/biz-man-lightgray", {
        method: "POST",
      ...
      void addToHistory(data.output, "비즈프로필 (남성·라이트그레이 정장)", Array.isArray(data.originalUrls) ? data.originalUrls : undefined);
    } catch (e: unknown) {
      clearTimeout(tid);
  ...
  const handleDownload = (url: string, idx: number) => { void saveImage(url, `biz-man-lightgray-${idx + 1}.png`); };
  const handleShare = (url: string, idx: number) => { void shareImage(url, `biz-man-lightgray-${idx + 1}.png`, "MOSPIC에서 만든 사진이에요 · mospic.com"); };
      ...
      <div style={{ padding: "18px 18px 100px" }}>
        {results.length === 0 && (
          <>
            ...
            {BA_LIVE.includes("bizmlightgray") ? (
              <BeforeAfterHero pairs={[1, 2, 3, 4, 5, 6].flatMap(n => [
                { before: `/examples/ba/bizmlightgray-before-${n}.webp`, after: `/examples/ba/bizmlightgray-after-${n}.webp` },
                { before: `/examples/ba/bizmlightgray-before.webp`, after: `/examples/ba/bizmlightgray-after-${n}.webp` },
              ])} />
            ) : (
              <PreviewCard image="/details/bizmlightgray.webp" caption="남성 라이트그레이 정장, 미리 만나보세요" accent={ACCENT} />
            )}
