...
export default function IdGlassesGrayPage() {
  ...
  const COIN_GATED = LIVE_COIN_CONCEPTS.includes("idglasses");
  const COIN_COST = CONCEPTS.idglasses?.coinCost ?? 0;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/id-glasses-gray", {
        method: "POST",
  ...
  const handleDownload = (url: string, idx: number) => { void saveImage(url, `id-glasses-gray-${idx + 1}.png`); };
  const handleShare = (url: string, idx: number) => { void shareImage(url, `id-glasses-gray-${idx + 1}.png`, "MOSPIC에서 만든 사진이에요 · mospic.com"); };
  ...
  return (
    <div style={{ maxWidth: 480, margin: "0 auto", minHeight: "100vh", background: "#F7F8FA", fontFamily: "var(--font-noto), 'Apple SD Gothic Neo', sans-serif" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "0 12px", height: 56, position: "sticky", top: 0, background: "#fff", zIndex: 10 }}>
        ...
        <span style={{ fontSize: 16, fontWeight: 800, color: "#191919" }}>정장+안경 증명사진</span>
      </div>
      ...
      <div style={{ padding: "18px 18px 100px" }}>
        {results.length === 0 && (
          <>
            ...
            {BA_LIVE.includes("idglasses") ? (
              <BeforeAfterHero pairs={[1, 2, 3, 4, 5, 6].flatMap(n => [
                { before: `/examples/ba/idglasses-before-${n}.webp`, after: `/examples/ba/idglasses-after-${n}.webp` },
                { before: `/examples/ba/idglasses-before.webp`, after: `/examples/ba/idglasses-after-${n}.webp` },
              ])} />
            ) : (
              <PreviewCard image="/details/idglasses.webp" caption="정장+안경 증명사진, 미리 만나보세요" accent={ACCENT} />
            )}
