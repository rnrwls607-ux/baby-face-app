...
export default function GoodsPage() {
  ...
  const COIN_GATED = LIVE_COIN_CONCEPTS.includes("goods");
  const COIN_COST = CONCEPTS.goods?.coinCost ?? 0;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/goods", {
        ...
        body: JSON.stringify({ image: compressed, goodsType }),
        signal: ctrl.signal,
      });
      clearTimeout(tid);
  ...
  const handleDownload = () => { void saveImage(result, "goods.png"); };
  const handleShare = () => { void shareImage(result, "goods.png", "MOSPIC에서 만든 사진이에요 · mospic.com"); };
  return (
    <div style={{ maxWidth: 480, margin: "0 auto", minHeight: "100vh", background: "#F7F8FA", fontFamily: "var(--font-noto), 'Apple SD Gothic Neo', sans-serif" }}>
      ...
      <UploadGuide type="portrait_multi" />
      <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "0 12px", height: 56, position: "sticky", top: 0, background: "#fff", zIndex: 10 }}>
        ...
        <span style={{ fontSize: 16, fontWeight: 800, color: "#191919" }}>굿즈 미리보기</span>
      </div>
      <div style={{ padding: "18px 18px 100px" }}>
        {!result && (
          <>
            <PreviewCard placeholder="🔑" caption="나만의 아크릴 굿즈, 미리 만나보세요" />
            ...
            <div style={{ background: "#fff", borderRadius: 20, padding: "18px 18px", boxShadow: "0 2px 16px rgba(0,0,0,0.04)", marginBottom: 18 }}>
              <p style={{ fontSize: 13, fontWeight: 700, color: "#191919", marginBottom: 10, marginTop: 0 }}>어떤 굿즈로 만들까요?</p>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 8 }}>
                {GOODS_OPTIONS.map(o => (
                  <button key={o.key} onClick={() => setGoodsType(o.key)} style={chipStyle(goodsType === o.key)}>{o.label}</button>
                ))}
              </div>
            </div>
            <UploadZone
              ...
              onRemove={() => setImage("")}
            />
            <TipChips tips={[{ icon: "face", label: "얼굴 또렷하게" }, { icon: "sun", label: "밝은 곳에서" }, { icon: "expand", label: "대상 가까이" }]} />
            <PrivacyLine />
            <button onClick={handleSubmit} disabled={loading || !image || !goodsType}
              style={{ width: "100%", marginTop: 18, background: loading || !image || !goodsType ? "#E8E9ED" : "#FF4B7C", color: loading || !image || !goodsType ? "#AEB2BA" : "#fff", border: "none", borderRadius: 16, padding: "16px 0", fontSize: 16, fontWeight: 800, cursor: loading || !image || !goodsType ? "not-allowed" : "pointer", boxShadow: loading || !image || !goodsType ? "none" : "0 6px 18px rgba(255,75,124,0.32)" }}>
              {loading ? `만드는 중... (${elapsed}초)` : <>굿즈 만들기 ✨{COIN_GATED && COIN_COST > 0 && <span style={{ fontSize: 13, fontWeight: 700, opacity: 0.9 }}> · <CoinIcon size={14} onColor /> {COIN_COST}</span>}</>}
            </button>
          </>
        )}
        {loading && (
        ...
        {result && (
          <div>
            ...
            <div style={{ borderRadius: 20, overflow: "hidden", marginBottom: 14, boxShadow: "0 4px 20px rgba(0,0,0,0.08)" }}>
              <img src={result} alt="굿즈 미리보기" style={{ width: "100%", display: "block" }} />
            </div>
            <div style={{ display: "flex", gap: 10 }}>
              ...
              <button onClick={() => { setResult(""); setGoodsType(""); }}
                style={{ flex: 1, background: "#fff", color: "#191919", border: "1.5px solid #EFF0F3", borderRadius: 14, padding: "15px 0", fontSize: 14, fontWeight: 700, cursor: "pointer" }}>다른 굿즈로</button>
            </div>
            ...
            <p style={{ fontSize: 11, color: "#BFC3CB", textAlign: "center", marginTop: 14, lineHeight: 1.6 }}>※ 사진은 유지돼요. &quot;다른 굿즈로&quot;를 누르고 굿즈만 바꿔서 또 만들어보세요!</p>
          </div>
        )}
      </div>
    </div>
