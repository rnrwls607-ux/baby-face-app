...
const CAMPUS_OPTIONS = [
  { key: "ivy", label: "🌿 아이비 캠퍼스" },
  { key: "krspring", label: "🌸 봄날 한국 대학" },
  { key: "euclassic", label: "🏛️ 유럽 고전" },
  { key: "city", label: "🏙️ 도심 캠퍼스" },
];

export default function CampusgradPage() {
  const router = useRouter();
  // 코인 게이트 표시·가드 (coinCost는 표시 전용 — 요금의 진실원은 서버 withCoin) — ★스위치 날 벌크 앵커
  const COIN_GATED = LIVE_COIN_CONCEPTS.includes("campusgrad");
  const COIN_COST = CONCEPTS.campusgrad?.coinCost ?? 0;
  ...
  const [chip, setChip] = useState<string>("ivy"); // 기본값 아이비 — 칩을 안 골라도 바로 만들 수 있다
  ...
  // pick: 결과 화면에서 "다른 캠퍼스로" 누를 때 그 칩으로 바로 재생성 (칩 상태 갱신 포함)
  const handleSubmit = async (pick?: string) => {
    if (!image) { setError("사진을 올려주세요."); return; }
    const chosen = pick || chip;
    if (pick) setChip(pick);
    ...
    try {
      ...
      const res = await fetch("/api/campusgrad", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ image: compressed, chip: chosen }),
        signal: ctrl.signal,
      });
      clearTimeout(tid);
      ...
      void addToHistory(data.output, "캠퍼스 졸업사진", Array.isArray(data.originalUrls) ? data.originalUrls : undefined);
    } catch (e: unknown) {
      clearTimeout(tid);
  ...
  const handleDownload = () => { void saveImage(result, "campusgrad.png"); };
  const handleShare = () => { void shareImage(result, "campusgrad.png", "MOSPIC에서 만든 사진이에요 · mospic.com"); };
  return (
    <div style={{ maxWidth: 480, margin: "0 auto", minHeight: "100vh", background: "#fff", fontFamily: "var(--font-noto), 'Apple SD Gothic Neo', sans-serif" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "0 16px", height: 56, borderBottom: "1px solid #f0f0f0", position: "sticky", top: 0, background: "#fff", zIndex: 10 }}>
        ...
        <span style={{ fontSize: 16, fontWeight: 700, color: "#111" }}>캠퍼스 졸업사진</span>
      </div>
      <div style={{ padding: "20px 20px 100px" }}>
        {!result && (
          <>
            {/* 결과 예시 — BA_LIVE면 비포/애프터 라이브, 아니면 기존 PreviewCard (무변화 폴백) */}
            {BA_LIVE.includes("campusgrad") ? (
              <BeforeAfterHero pairs={[1, 2, 3].flatMap(n => [
                { before: `/examples/ba/campusgrad-before-${n}.webp`, after: `/examples/ba/campusgrad-after-${n}.webp` },
                { before: `/examples/ba/campusgrad-before.webp`, after: `/examples/ba/campusgrad-after-${n}.webp` },
              ])} />
            ) : (
              <PreviewCard placeholder="🎓" caption="캠퍼스 졸업사진, 미리 만나보세요" />
            )}
            ...
            <div style={{ background: "#fff", borderRadius: 20, padding: "18px 18px", boxShadow: "0 2px 16px rgba(0,0,0,0.04)", marginBottom: 18 }}>
              <p style={{ fontSize: 13, fontWeight: 700, color: "#191919", marginBottom: 10, marginTop: 0 }}>어떤 캠퍼스에서 찍을까요?</p>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 8 }}>
                {CAMPUS_OPTIONS.map(o => (
                  <button key={o.key} onClick={() => setChip(o.key)} style={chipStyle(chip === o.key)}>{o.label}</button>
                ))}
              </div>
            </div>
            <UploadZone
        ...
        {result && (
          <div>
            ...
            <div style={{ borderRadius: 16, overflow: "hidden", marginBottom: 12 }}>
              <img src={result} alt="캠퍼스 졸업사진" style={{ width: "100%", display: "block" }} />
            </div>
            {/* 다른 캠퍼스로 — 사진을 다시 올리지 않고 칩만 눌러 재생성 */}
            <div style={{ background: "#fff", borderRadius: 20, padding: "18px 18px", boxShadow: "0 2px 16px rgba(0,0,0,0.04)", margin: "0 0 12px" }}>
              <p style={{ fontSize: 13, fontWeight: 700, color: "#191919", marginBottom: 10, marginTop: 0 }}>다른 캠퍼스에서도 만들어볼까요?</p>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 8 }}>
                {CAMPUS_OPTIONS.map(o => (
                  <button key={o.key} onClick={() => (COIN_GATED && COIN_COST > 0 ? setPendingPick(o.key) : void handleSubmit(o.key))} disabled={loading} style={chipStyle(chip === o.key)}>{o.label}</button>
                ))}
              </div>
            </div>
      ...
      <RegenConfirmSheet open={pendingPick !== null} question="다른 캠퍼스로 새로 만들까요?" cost={COIN_COST}
