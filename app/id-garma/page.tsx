...
export default function IdGarmaPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      void addToHistory(data.output, "증명사진 (가르마컷 블랙정장)");
    } catch (e: unknown) {
      clearTimeout(tid);
    ...
    <div style={{ maxWidth: 480, margin: "0 auto", minHeight: "100vh", background: "#F7F8FA", fontFamily: "var(--font-noto), 'Apple SD Gothic Neo', sans-serif" }}>
      ...
      <div style={{ padding: "18px 18px 100px" }}>
        <div style={{ background: "#FFF0F5", borderRadius: 16, padding: "16px 18px", marginBottom: 22 }}>
          <p style={{ fontSize: 14, fontWeight: 800, color: ACCENT, margin: "0 0 5px" }}>🖤 가르마컷 블랙정장 증명사진</p>
          <p style={{ fontSize: 12.5, color: "#7A6A70", margin: 0, lineHeight: 1.55 }}>단정한 가르마컷과 블랙 정장, 차콜그레이 배경의 신뢰감 있는 프로필이에요. 얼굴이 정면으로 잘 보이는 사진을 3~6장 올리면, 가장 잘 나온 사진 3장을 만들어드려요.</p>
        </div>
