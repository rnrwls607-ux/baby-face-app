...
export default function GoldenhourPage() {
  ...
  const COIN_GATED = LIVE_COIN_CONCEPTS.includes("goldenhour");
  const COIN_COST = CONCEPTS.goldenhour?.coinCost ?? 0;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/goldenhour", {
        method: "POST",
      ...
      void addToHistory(data.output, "골든아워", Array.isArray(data.originalUrls) ? data.originalUrls : undefined);
    } catch (e: unknown) {
      clearTimeout(tid);
  ...
  const handleDownload = () => { void saveImage(result, "goldenhour.png"); };
  const handleShare = () => { void shareImage(result, "goldenhour.png", "MOSPIC에서 만든 사진이에요 · mospic.com"); };
  return (
      ...
      <div style={{ padding: "20px 20px 100px" }}>
        {!result && (
          <>
            ...
            {BA_LIVE.includes("goldenhour") ? (
              <BeforeAfterHero pairs={[1, 2, 3].flatMap(n => [
                { before: `/examples/ba/goldenhour-before-${n}.webp`, after: `/examples/ba/goldenhour-after-${n}.webp` },
                { before: `/examples/ba/goldenhour-before.webp`, after: `/examples/ba/goldenhour-after-${n}.webp` },
              ])} />
            ) : (
              <PreviewCard placeholder="🌅" caption="골든아워, 미리 만나보세요" />
            )}
            ...
            />
            <TipChips tips={[{ icon: "expand", label: "하늘이 보이게" }, { icon: "sun", label: "낮에 찍은 사진" }, { icon: "eye", label: "흔들리지 않게" }]} />
            <PrivacyLine />
            <ProCongestionHint concept="goldenhour" />
            <button onClick={handleSubmit} disabled={loading || !image}
              ...
              {loading ? `만드는 중... (${elapsed}초)` : <>골든아워 만들기 🌅{COIN_GATED && COIN_COST > 0 && <span style={{ fontSize: 13, fontWeight: 700, opacity: 0.9 }}> · <CoinIcon size={14} onColor /> {COIN_COST}</span>}</>}
            </button>
          </>
        )}
        {loading && (
        ...
        {error && (
          <div style={{ background: "#FFF0F3", border: "1px solid #FFD6E0", borderRadius: 12, padding: "12px 16px", marginTop: 16 }}>
            ...
            <ProCongestionError concept="goldenhour" error={error} />
        ...
        {result && (
          <div>
            ...
            <div style={{ borderRadius: 16, overflow: "hidden", marginBottom: 12 }}>
              <img src={result} alt="골든아워" style={{ width: "100%", display: "block" }} />
            </div>
