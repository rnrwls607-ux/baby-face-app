...
export default function HanbokcouplePage() {
  ...
  const COIN_GATED = LIVE_COIN_CONCEPTS.includes("hanbokcouple");
  const COIN_COST = CONCEPTS.hanbokcouple?.coinCost ?? 0;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      void addToHistory(data.output, "웨딩 한복 커플", Array.isArray(data.originalUrls) ? data.originalUrls : undefined);
    } catch (e: unknown) {
      clearTimeout(tid);
      ...
      <div style={{ padding: "18px 18px 100px" }}>
        ...
        {!result && (
          <>
            ...
            {BA_LIVE.includes("hanbokcouple") ? (
              <BeforeAfterHero pairs={[1, 2, 3].flatMap(n => [
                { before: `/examples/ba/hanbokcouple-before-${n}.webp`, after: `/examples/ba/hanbokcouple-after-${n}.webp` },
                { before: `/examples/ba/hanbokcouple-before.webp`, after: `/examples/ba/hanbokcouple-after-${n}.webp` },
              ])} />
            ) : (
              <>
              <div style={{ background: "#fff", borderRadius: 20, padding: 14, marginBottom: 8, boxShadow: "0 2px 16px rgba(0,0,0,0.04)" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 6, position: "relative" }}>
                    ...
                    <img src="/examples/hanbokcouple_b1.webp" alt="" loading="lazy" decoding="async" style={{ width: "100%", aspectRatio: "4/5", objectFit: "cover", borderRadius: 10, display: "block" }} />
                    <img src="/examples/hanbokcouple_b2.webp" alt="" loading="lazy" decoding="async" style={{ width: "100%", aspectRatio: "4/5", objectFit: "cover", borderRadius: 10, display: "block" }} />
                  </div>
                  ...
                  <div style={{ flex: 2, position: "relative" }}>
                    ...
                    <img src="/examples/hanbokcouple_a.webp" alt="" loading="lazy" decoding="async" style={{ width: "100%", aspectRatio: "4/5", objectFit: "cover", borderRadius: 14, display: "block" }} />
                  </div>
                </div>
                <p style={{ fontSize: 11.5, color: "#9B9B9B", textAlign: "center", margin: "10px 0 2px", fontWeight: 600 }}>각자 찍은 사진 두 장이, 웨딩 한복 화보로</p>
              </div>
              </>
            )}
            ...
            <PrivacyLine />
            <ProCongestionHint concept="hanbokcouple" />
            <button onClick={handleSubmit} disabled={!canSubmit}
              ...
              {loading ? `만드는 중... (${elapsed}초)` : <>웨딩 한복 화보 만들기 ✨{COIN_GATED && COIN_COST > 0 && <span style={{ fontSize: 13, fontWeight: 700, opacity: 0.9 }}> · <CoinIcon size={14} onColor /> {COIN_COST}</span>}</>}
            </button>
          </>
        )}
        {loading && (
        ...
        {error && (
          <div style={{ background: "#FFEAF1", border: "1px solid #FF4B7C33", borderRadius: 12, padding: "13px 16px", marginTop: 16 }}>
            ...
            <ProCongestionError concept="hanbokcouple" error={error} />
