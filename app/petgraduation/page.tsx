...
export default function PetgraduationPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      void addToHistory(data.output, "펫 졸업사진");
    } catch (e: unknown) {
      clearTimeout(tid);
        ...
        {!result && (
          <>
            ...
            <button onClick={handleSubmit} disabled={loading || !image}
              ...
              {loading ? `만드는 중... (${elapsed}초)` : <>졸업사진 만들기 ✨{COIN_GATED && COIN_COST > 0 && <span style={{ fontSize: 13, fontWeight: 700, opacity: 0.9 }}> · <CoinIcon size={14} /> {COIN_COST}</span>}</>}
            </button>
          </>
        )}
        {loading && (
