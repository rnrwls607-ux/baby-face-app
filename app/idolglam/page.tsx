...
const STYLE_OPTIONS = [
  { key: "pictorial", label: "📸 화보컷" },
  { key: "fancam", label: "🎥 직캠컷" },
];

export default function IdolglamPage() {
  const router = useRouter();
  // 코인 게이트 표시·가드 (coinCost는 표시 전용 — 요금의 진실원은 서버 withCoin) — ★스위치 날 벌크 앵커
  const COIN_GATED = LIVE_COIN_CONCEPTS.includes("idolglam");
  const COIN_COST = CONCEPTS.idolglam?.coinCost ?? 0;
  ...
  const handleSubmit = async (pick?: string) => {
    ...
    const tid = setTimeout(() => ctrl.abort(), 235000); // 서버 내부 컷 230초 + 여유 5초
    try {
      const compressed = await compress(image);
      const res = await fetch("/api/idolglam", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ image: compressed, style: chosen }),
        signal: ctrl.signal,
      });
      clearTimeout(tid);
      ...
      void addToHistory(data.output, "아이돌 글램", Array.isArray(data.originalUrls) ? data.originalUrls : undefined);
    } catch (e: unknown) {
      clearTimeout(tid);
  ...
  const handleDownload = () => { void saveImage(result, "idolglam.png"); };
  const handleShare = () => { void shareImage(result, "idolglam.png", "MOSPIC에서 만든 사진이에요 · mospic.com"); };
  return (
    <div style={{ maxWidth: 480, margin: "0 auto", minHeight: "100vh", background: "#fff", fontFamily: "var(--font-noto), 'Apple SD Gothic Neo', sans-serif" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "0 16px", height: 56, borderBottom: "1px solid #f0f0f0", position: "sticky", top: 0, background: "#fff", zIndex: 10 }}>
        ...
        <span style={{ fontSize: 16, fontWeight: 700, color: "#111" }}>아이돌 글램</span>
      </div>
      <div style={{ padding: "20px 20px 100px" }}>
        {!result && (
          <>
            {/* 결과 예시 — BA_LIVE면 비포/애프터 라이브, 아니면 기존 PreviewCard (무변화 폴백) */}
            {BA_LIVE.includes("idolglam") ? (
              <BeforeAfterHero pairs={[1, 2, 3, 4].flatMap(n => [
                { before: `/examples/ba/idolglam-before-${n}.webp`, after: `/examples/ba/idolglam-after-${n}.webp` },
                { before: `/examples/ba/idolglam-before.webp`, after: `/examples/ba/idolglam-after-${n}.webp` },
              ])} />
            ) : (
              <PreviewCard placeholder="🌟" caption="아이돌 글램, 미리 만나보세요" />
            )}
            ...
            <div style={{ background: "#fff", borderRadius: 20, padding: "18px 18px", boxShadow: "0 2px 16px rgba(0,0,0,0.04)", marginBottom: 18 }}>
              <p style={{ fontSize: 13, fontWeight: 700, color: "#191919", marginBottom: 10, marginTop: 0 }}>어떤 무드로 만들까요?</p>
            ...
            <p style={{ fontSize: 12, color: "#9B9B9B", margin: "10px 2px 0", lineHeight: 1.6 }}>여성 스타일링 중심 컨셉이에요.</p>
            <PrivacyLine />
            <ProCongestionHint concept="idolglam" />
            <button onClick={() => handleSubmit()} disabled={loading || !image}
              ...
              {loading ? `만드는 중... (${elapsed}초)` : <>아이돌 글램 만들기 🌟{COIN_GATED && COIN_COST > 0 && <span style={{ fontSize: 13, fontWeight: 700, opacity: 0.9 }}> · <CoinIcon size={14} onColor /> {COIN_COST}</span>}</>}
            </button>
          </>
        )}
        {loading && (
        ...
        {error && (
          <div style={{ background: "#FFF0F3", border: "1px solid #FFD6E0", borderRadius: 12, padding: "12px 16px", marginTop: 16 }}>
            ...
            <ProCongestionError concept="idolglam" error={error} />
        ...
        {result && (
          <div>
            ...
            <div style={{ borderRadius: 16, overflow: "hidden", marginBottom: 12 }}>
              <img src={result} alt="아이돌 글램" style={{ width: "100%", display: "block" }} />
            </div>
            {/* 다른 무드로 — 사진을 다시 올리지 않고 칩만 눌러 재생성 (엔진이 통째로 바뀐다) */}
            <div style={{ background: "#fff", borderRadius: 20, padding: "18px 18px", boxShadow: "0 2px 16px rgba(0,0,0,0.04)", margin: "0 0 12px" }}>
              <p style={{ fontSize: 13, fontWeight: 700, color: "#191919", marginBottom: 10, marginTop: 0 }}>다른 무드로도 만들어볼까요?</p>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 8 }}>
                {STYLE_OPTIONS.map(o => (
                  <button key={o.key} onClick={() => (COIN_GATED && COIN_COST > 0 ? setPendingPick(o.key) : void handleSubmit(o.key))} disabled={loading} style={chipStyle(style === o.key)}>{o.label}</button>
                ))}
              </div>
            </div>
      ...
      <RegenConfirmSheet open={pendingPick !== null} question="다른 무드로 새로 만들까요?" cost={COIN_COST}
