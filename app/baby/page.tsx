"use client";
// 아기 얼굴 — 라우트형 전환 1/3 (couple 페이지 복제 + baby 조정).
// 홈(app/page.tsx)의 showMakeScreen 화면과 당분간 공존한다. 홈 카드 링크 전환은 2단계.
// ★구형 게이트(/api/usage·FREE_LIMIT·limitReached)는 의도적으로 미이식 — 폐기 대상이고
// 그 자리를 코인 앵커가 대신한다(현재 LIVE_COIN_CONCEPTS에 baby가 없어 휴면).
...
export default function BabyPage() {
  ...
  const COIN_GATED = LIVE_COIN_CONCEPTS.includes("baby");
  const COIN_COST = CONCEPTS.baby?.coinCost ?? 0;
  ...
  const [images, setImages] = useState<string[]>([]); // 0 = 엄마, 1 = 아빠
  const [gender, setGender] = useState(""); // "girl" | "boy" — 아기 성별(1세트)
  ...
  const compress = (b64: string): Promise<string> => new Promise(res => {
    ...
    img.onload = () => {
      ...
      const M = 512; let { width: w, height: h } = img; // 홈 기존 핸들러와 동일 규격
  ...
  const handleSubmit = async () => {
    if (!images[0] || !images[1]) { setError("엄마와 아빠 사진을 모두 올려주세요."); return; }
    if (!gender) { setError("아기 성별을 선택해주세요."); return; }
    ...
    const tid = setTimeout(() => ctrl.abort(), 110000); // 서버 maxDuration 60·내부 50초보다 여유 있게
    try {
      const [c1, c2] = await Promise.all([compress(images[0]), compress(images[1])]);
      const res = await fetch("/api/generate", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ image1: c1, image2: c2, gender }),
        signal: ctrl.signal,
      });
      clearTimeout(tid);
      ...
      void addToHistory(data.output, "아기 얼굴", Array.isArray(data.originalUrls) ? data.originalUrls : undefined); // 과거 기록과 같은 라벨 유지
    } catch (e: unknown) {
      clearTimeout(tid);
  ...
  const handleDownload = () => { void saveImage(result, "baby.png"); };
  const handleShare = () => { void shareImage(result, "baby.png", "MOSPIC에서 만든 사진이에요 · mospic.com"); };
  const canSubmit = !!images[0] && !!images[1] && !!gender && !loading;
  // 성별 칩 — 딸=핑크·아들=스카이 컬러 코딩 (couple genderChipStyle 재활용)
  const genderChipStyle = (v: "girl" | "boy", active: boolean) => ({
    ...
    background: v === "girl" ? (active ? "#FF4B7C" : "#FFF0F5") : (active ? "#5AA7EA" : "#EBF5FE"),
    color: active ? "#FFFFFF" : v === "girl" ? "#E9548A" : "#4E96DC",
    boxShadow: active ? (v === "girl" ? "0 3px 10px rgba(255,75,124,0.28)" : "0 3px 10px rgba(90,167,234,0.28)") : "none",
  });
  return (
    <div style={{ maxWidth: 480, margin: "0 auto", minHeight: "100vh", background: "#F7F8FA", fontFamily: "var(--font-noto), 'Apple SD Gothic Neo', sans-serif" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "0 12px", height: 56, position: "sticky", top: 0, background: "#fff", zIndex: 10 }}>
        ...
        <span style={{ fontSize: 16, fontWeight: 800, color: "#191919" }}>우리 아기 얼굴은?</span>
      </div>
      <div style={{ padding: "18px 18px 100px" }}>
        <div style={{ background: "#FFEAF1", borderRadius: 16, padding: "16px 18px", marginBottom: 22 }}>
          <p style={{ fontSize: 14, fontWeight: 800, color: "#FF4B7C", margin: "0 0 5px" }}>👶 엄마·아빠 닮은 아기</p>
          <p style={{ fontSize: 12.5, color: "#B36B85", margin: 0, lineHeight: 1.55 }}>엄마와 아빠 사진을 한 장씩 올리면 두 사람을 자연스럽게 닮은 아기 얼굴을 만들어드려요.</p>
        </div>
        {!result && (
          <>
            {/* 결과 예시 — BA_LIVE면 비포/애프터 라이브, 아니면 기존 PreviewCard (무변화 폴백) */}
            {BA_LIVE.includes("baby") ? (
              <BeforeAfterHero pairs={[1, 2, 3].flatMap(n => [
                { before: `/examples/ba/baby-before-${n}.webp`, after: `/examples/ba/baby-after-${n}.webp` },
                { before: `/examples/ba/baby-before.webp`, after: `/examples/ba/baby-after-${n}.webp` },
              ])} />
            ) : (
              <PreviewCard placeholder="👶" caption="우리 아기 얼굴, 미리 만나보세요" />
            )}
            ...
            <div style={{ background: "#fff", borderRadius: 20, padding: "20px 18px", boxShadow: "0 2px 16px rgba(0,0,0,0.04)" }}>
              <p style={{ fontSize: 13, fontWeight: 700, color: "#191919", marginBottom: 10, marginTop: 0 }}>엄마·아빠 사진 (각 1장)</p>
              <div style={{ display: "flex", gap: 10 }}>
                {[0, 1].map(i => (
                  <label key={i} style={{ flex: 1, display: "block", cursor: "pointer" }}>
                    <div style={{ width: "100%", aspectRatio: "1", borderRadius: 14, border: images[i] ? "1.5px solid #FF4B7C" : "1.5px dashed #D9DCE2", background: images[i] ? "#fff" : "#F1F2F6", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", overflow: "hidden", gap: 3 }}>
                      {images[i]
                        ...
                        : <><span style={{ fontSize: 26, color: "#C2C6CE" }}>＋</span><span style={{ fontSize: 12, color: "#9B9B9B", fontWeight: 600 }}>{i === 0 ? "엄마 사진" : "아빠 사진"}</span></>}
                    </div>
              ...
              {/* 아기 성별 칩 1세트 — label 밖 배치(파일 선택 오작동 방지), setter 실호출 */}
              <p style={{ fontSize: 13, fontWeight: 700, color: "#191919", margin: "16px 0 8px" }}>아기 성별</p>
              <div style={{ display: "flex", gap: 6 }}>
                {([["girl", "딸"], ["boy", "아들"]] as const).map(([v, l]) => (
                  <button key={v} onClick={() => setGender(v)} style={genderChipStyle(v, gender === v)}>{l}</button>
                ))}
              </div>
            </div>
            ...
            <button onClick={handleSubmit} disabled={!canSubmit}
              ...
              {loading ? `예측 중... (${elapsed}초)` : <>아기 얼굴 만들기 ✨{COIN_GATED && COIN_COST > 0 && <span style={{ fontSize: 13, fontWeight: 700, opacity: 0.9 }}> · <CoinIcon size={14} onColor /> {COIN_COST}</span>}</>}
            </button>
          </>
        )}
        {loading && (
        ...
        {result && (
          <div>
            ...
            <p style={{ fontSize: 19, fontWeight: 900, color: "#191919", textAlign: "center", margin: "4px 0 18px" }}>{gender === "girl" ? "우리 딸 얼굴이에요! 👧" : "우리 아들 얼굴이에요! 👦"}</p>
            ...
            <div style={{ borderRadius: 20, overflow: "hidden", marginBottom: 14, boxShadow: "0 4px 20px rgba(0,0,0,0.08)" }}>
              <img src={result} alt="아기 얼굴" style={{ width: "100%", display: "block" }} />
            </div>
            <div style={{ display: "flex", gap: 10 }}>
