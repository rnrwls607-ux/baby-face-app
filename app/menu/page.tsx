...
export default function MenuPage() {
  ...
  const COIN_GATED = LIVE_COIN_CONCEPTS.includes("menu");
  const COIN_COST = CONCEPTS.menu?.coinCost ?? 0;
  ...
  const runGenerate = async (styleKey: string) => {
    ...
    try {
      ...
      void addToHistory(data.output, "메뉴판 비주얼", Array.isArray(data.originalUrls) ? data.originalUrls : undefined);
    } catch (e: unknown) {
      clearTimeout(tid);
        ...
        {!result && !loading && (
          <>
            ...
            <button onClick={() => runGenerate(style)} disabled={!image}
              ...
              메뉴판 사진 만들기 ✨{COIN_GATED && COIN_COST > 0 && <span style={{ fontSize: 13, fontWeight: 700, opacity: 0.9 }}> · <CoinIcon size={14} onColor /> {COIN_COST}</span>}
            </button>
          </>
        )}
        {loading && (
