...
// 여성 비즈프로필 (블루&그레이 미니원피스 + 밝은 회색 배경). {POSE} 자리에 포즈별 지시.
...
STYLING (women's business profile — blue & gray mini dress / bright light gray background)
- Clothing: an elegant, well-fitted color-block one-piece dress: a soft GRAY body paired with
  light SKY-BLUE sheer sleeves and a sky-blue ribbon/bow tie at the neckline — a refined, youthful,
  feminine silhouette, knee-length. A fresh, bright, put-together look. The dress looks smooth,
  fresh, and gracefully draped (soft natural drape, not messy creases). NO jacket needed.
...
- Lighting: soft, even, flattering professional studio lighting (gentle key + fill), polished
  ...
  the light background. The gray body and sky-blue sleeves both stay clear and distinct.
...
async function generateOneBizPhoto(imageDataUrls: string[], pose: string): Promise<string> {
  ...
  clearTimeout(timer);
  console.log(`[biz-bluegray-gray] status=${res.status} ${Date.now() - t0}ms`);

  if (!res.ok) throw new Error(await geminiFriendlyError(res, "biz-bluegray-gray"));
...
async function handler(request: NextRequest) {
  try {
    ...
    console.error("biz-bluegray-gray error:", err?.message);
    return NextResponse.json({ error: err?.message || "오류가 발생했습니다." }, { status: 500 });
  }
}

export const POST = withCoin("bizbluegray", 0, handler); // COIN_DORMANT: 실가격 9
