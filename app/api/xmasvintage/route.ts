...
// 빈티지 크리스마스 스냅 — person:pro (cheerglam 템플릿 복제, new-concept.mjs 생성)
...
async function generateXmasvintage(imageDataUrl: string): Promise<string> {
...
vintage-film indoor Christmas snap portrait of them in the scene described below.
...
THE SCENE — vintage film-look indoor Christmas snap:
- The person is captured at home on a cozy Christmas evening — a warmly
  lit living room, a decorated tree glowing softly behind them, ornaments
  and string lights melting into round bokeh: an intimate, nostalgic,
...
- Film rendering: the look of a well-exposed 35mm color film — a fine,
  ...
  grain is fine and uniform everywhere and must NEVER clump into spots,
  specks, or texture on the face — the face itself stays smooth, clean,
  and sharp beneath the film look.
- Wardrobe: a cozy knit look that suits the person — a soft cable-knit
  or mohair sweater or cardigan in deep red, cream, or forest green,
  styled so the face stays fully open and bright (nothing covering the
  forehead, eyebrows, or eyes; the hairline showing naturally). Clean
  fit around the neckline. No logos, no lettering, no printed words or
  slogans on the knit.
- Setting: a softly blurred home interior — the tree with glass
  ornaments and warm string lights as pure bokeh, a wrapped gift or two
  and a mug on a side table out of focus, lived-in cozy textures, like a
  real December evening at home, never a rendered backdrop or studio
  set. No readable text anywhere — no gift tags, no cards, no banners,
  no lettering on ornaments or packaging.
POSE — a natural cozy-evening moment: sitting or leaning near the tree,
looking toward the camera with a soft bright smile, or a candid glance
at an ornament held lightly in one hand.
...
- Face clean, fresh, and luminous — never tinted yellow or orange by the
  tree lights, and never speckled by the film grain?
- Does it read as a real home evening on film — not a studio set?
...
- A yellow, orange, or sallow-looking face tinted by the Christmas light.
- Film grain clumping into spots, specks, or noise on the face.
- A flat studio backdrop or photo-booth set — this must read as a real
  home interior.
- Readable gift tags, cards, banners, or lettering on any object.
...
vintage-film indoor Christmas snap portrait, at the absolute best of
their life. High resolution, no text, no watermark, no border.`;
  ...
  try {
    ...
    if ((e as { name?: string })?.name === "AbortError") {
      console.error(`[TIMEOUT][xmasvintage] 230초 무응답 ${Date.now() - t0}ms`);
      throw new Error("이미지 생성이 230초를 넘겨 중단했어요. 다시 시도해주세요.");
    }
    throw e;
  }
  clearTimeout(timer);
  console.log(`[xmasvintage] model=${GEMINI_MODEL} status=${res.status} ${Date.now() - t0}ms`);
  if (!res.ok) throw new Error(await geminiFriendlyError(res, "xmasvintage", "생성에 실패했어요. 다른 사진으로 다시 시도해주세요."));
  ...
  console.log(`[xmasvintage] finish=${cand?.finishReason || "-"} block=${data?.promptFeedback?.blockReason || "-"} parts=${respParts.length} img=${imgParts.length} ${Date.now() - t0}ms`);
  const chosen = (finalParts.length ? finalParts : imgParts).pop();
  const b64 = chosen?.inlineData?.data || chosen?.inline_data?.data;
  if (!b64) {
    const txt = respParts.find((p: { text?: string }) => p.text)?.text;
    console.error(`[xmasvintage] 이미지 없음 — finish=${cand?.finishReason || "-"} text=${(txt || "").slice(0, 500)}`);
...
async function handler(request: NextRequest) {
  try {
    ...
    const output = await generateXmasvintage(image);
    return NextResponse.json({ output: [output] });
  } catch (e: unknown) {
    const err = e as { message?: string };
    console.error("xmasvintage error:", err?.message);
    return NextResponse.json({ error: err?.message || "오류가 발생했습니다." }, { status: 500 });
  }
}

export const POST = withCoin("xmasvintage", 0, handler); // COIN_DORMANT: 실가격 3 · gemini-3-pro-image
