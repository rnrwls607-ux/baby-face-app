...
TASK
You are RETOUCHING a real photograph of one real person into a single clean, dandy profile-style ID photo. One to six photos of the SAME individual are provided. You may restyle hair and clothing to the concept below, but you must NOT redesign the face. This concept is for MEN only — always render a man.
...
- Hair: restyle into a natural, short men's crop cut with light texture on top, tidy around the ears and forehead, groomed and clean, natural black hair. The hair outline must be clean and smooth where it meets the background. (This overrides the source hairstyle, but NOT the face.)
- Clothing: a crisp ice-blue collared shirt layered under a fitted navy knit vest, sharp and dandy, clean lines.
- Background: solid, even, light sky-gray (bright cool gray with a faint blue hint), smooth and clean, no shadows, no props.

STYLING
- Expression: a gentle, natural, closed-lip smile, confident and refined. Eyes open, relaxed, on camera.
...
OUTPUT
- Vertical ID ratio (3.5:4.5), high-quality studio portrait, photorealistic, clean and dandy. Remember the two absolute rules: the SAME face under the new hair, inside the SAME fixed composition.`;
...
async function generateOneIdPhoto(imageDataUrls: string[]): Promise<string> {
  ...
  clearTimeout(timer);
  console.log(`[id-dandy] model=${GEMINI_MODEL} status=${res.status} ${Date.now() - t0}ms`);

  if (!res.ok) throw new Error(await geminiFriendlyError(res, "id-dandy"));
...
async function handler(request: NextRequest) {
  try {
    ...
    console.error("id-dandy error:", err?.message);
    return NextResponse.json({ error: err?.message || "오류가 발생했습니다." }, { status: 500 });
  }
}

export const POST = withCoin("iddandy", 0, handler); // COIN_DORMANT: 실가격 9
