...
// 여성 비즈프로필 (블랙 원피스 + 밝은 회색 배경). {POSE} 자리에 포즈별 지시.
...
STYLING (women's business profile — black dress / bright light gray background)
- Clothing: an elegant, well-fitted BLACK sheath dress (one-piece) with a clean, minimal,
  sophisticated silhouette — long or three-quarter sleeves, a refined neckline, knee-length.
  Chic, modern, and timeless; the dress looks sharp, smooth, and intentional. Freshly pressed
  and smooth, no messy wrinkles. NO jacket needed.
...
- Lighting: soft, even, flattering professional studio lighting (gentle key + fill), polished
  corporate-headshot quality, minimal harsh shadow. The backdrop is bright and light while the
  dress is dark, so keep the dark dress cleanly readable with gentle dimension through soft,
...
async function generateOneBizPhoto(imageDataUrls: string[], pose: string): Promise<string> {
  ...
  clearTimeout(timer);
  console.log(`[biz-blackdress-gray] status=${res.status} ${Date.now() - t0}ms`);

  if (!res.ok) throw new Error(await geminiFriendlyError(res, "biz-blackdress-gray"));
...
async function handler(request: NextRequest) {
  try {
    ...
    console.error("biz-blackdress-gray error:", err?.message);
    return NextResponse.json({ error: err?.message || "오류가 발생했습니다." }, { status: 500 });
  }
}

export const POST = withCoin("bizblackdress", 0, handler); // COIN_DORMANT: 실가격 9
