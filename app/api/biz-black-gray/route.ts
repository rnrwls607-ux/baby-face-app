...
// 비즈프로필 공통 프롬프트 (블랙 정장 + 밝은 회색 배경). {POSE} 자리에 포즈별 지시.
...
STYLING (business profile — black suit / light gray background)
- Clothing: a well-tailored BLACK business suit. For women: a sharp black blazer over a clean
  white blouse or shirt (skirt or trousers as needed for the framing). For men: a black suit
  jacket over a white dress shirt with a neat dark tie. Crisp, fitted, formal corporate style —
  classic and authoritative, the outfit should look sharp and intentional, a key part of the photo.
- Background: a clean, even studio backdrop in light neutral gray (a touch brighter than a
  mid-gray, so the black suit stands out clearly), softly and evenly lit, subtle gentle
  gradient, no harsh shadows — premium corporate studio feel.
...
async function handler(request: NextRequest) {
  try {
    ...
    console.error("biz-black-gray error:", err?.message);
    return NextResponse.json({ error: err?.message || "오류가 발생했습니다." }, { status: 500 });
  }
}

export const POST = withCoin("bizblack", 0, handler); // COIN_DORMANT: 실가격 9
