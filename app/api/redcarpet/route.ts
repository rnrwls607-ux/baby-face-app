...
const SCENE = `THE SCENE — 레드카펫 시상식 화보 (awards night photo wall):
- A glamorous awards-night photo wall: the person standing on a rich red carpet in front of an elegant, clean backdrop wall with NO readable text or logos (a plain luxurious tone or a subtle abstract pattern), camera flashes sparkling as tiny starbursts in the darker background around them.
- Key light: a bright, glamorous frontal key light like a professional photo-wall flash — the face luminous, dewy, and flawless — plus a soft rim light separating them from the backdrop. The person is clearly the star everyone is photographing.

WARDROBE — star of the night:
- Woman: a breathtaking elegant evening gown in a rich jewel tone or luminous champagne that flatters her — a refined silhouette with tasteful sparkling jewelry (delicate earrings or a fine necklace).
- Man: a sharply tailored black or midnight tuxedo with a crisp white shirt — immaculate, star-like.
- Makeup accent: modern fresh red-carpet glam — a luminous base and a soft elegant lip color — never heavy, dark, or aging.

POSE:
- A confident red-carpet pose: standing tall facing the cameras, one hand relaxed at the waist or a graceful slight turn, chin softly lifted with a poised star smile.`;
...
async function generateRedcarpet(imageDataUrl: string): Promise<string> {
  ...
  clearTimeout(timer);
  console.log(`[redcarpet] model=${GEMINI_MODEL} status=${res.status} ${Date.now() - t0}ms`);
  if (!res.ok) throw new Error(await geminiFriendlyError(res, "redcarpet"));
...
async function handler(request: NextRequest) {
  try {
    ...
    const output = await generateRedcarpet(image);
    return NextResponse.json({ output: [output] });
  } catch (e: unknown) {
    const err = e as { message?: string };
    console.error("redcarpet error:", err?.message);
    return NextResponse.json({ error: err?.message || "오류가 발생했습니다." }, { status: 500 });
  }
}

export const POST = withCoin("redcarpet", 0, handler); // COIN_DORMANT: 실가격 3
