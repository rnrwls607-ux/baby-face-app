...
// 2인 라인 Wave 2 — 나노바나나 Pro + 펫 확장 8곳 (2인 CORE 기준 원본 = couple, 공식 수정 시 couple 고치고 재이식)
...
You are RETOUCHING real photographs of two real people and their real pet — NOT generating new subjects. You are given 3 input images: Image 1 shows Person 1, Image 2 shows Person 2, Image 3 shows their beloved pet. These two people came to Seoul's most famous premium photo studio TOGETHER for the pictorial described in THE SCENE below. Your craft as the studio's master retoucher: everyone walks out looking like THEMSELVES on the best day of their life — flawless skin, perfect light, beautifully styled — visibly upgraded, never altered. Edit their photos into ONE single stunning family portrait of the two of them with their pet. THE HIGHEST PRIORITY, above everything else: each output face must be instantly recognizable as the SAME person side by side with their own source image. The output must be THESE two specific people — never a generic stock model face.
...
- The Pet = the animal in Image 3. Note its species (dog or cat), breed, size, coat color, fur length and texture, distinctive markings and their exact placement, ear shape, eye color, muzzle shape, and whether it wears a collar or accessory.
...
- EXACTLY two people and EXACTLY one pet: Person 1, Person 2, and the Pet — each appearing exactly once. Never add, remove, duplicate, or merge any subject. No extra animals, no extra people.
...
- PET IDENTITY LOCK: the pet is rendered from Image 3 alone — the SAME species, SAME breed, SAME size impression, SAME coat color, SAME markings in the SAME places, SAME ear shape, SAME eye color, SAME face. Never change its breed, color, or pattern. If it wears a collar in Image 3, keep the same collar; if not, add nothing. The owners must gasp "that's OUR baby" at first glance.
...
PET GROOMING — its cutest self, still itself:
- The pet looks like it just left the best groomer in Seoul: clean, fluffy, healthy coat with natural shine, bright clear eyes, a happy relaxed expression natural to this animal.
- Groom and brighten ONLY — never alter breed traits, markings, coat color, or proportions. No costumes, no bows, no accessories that aren't in Image 3.
...
- Discard the lighting of both original photos entirely. Re-light the two together with ONE unified, soft key light in the mood of THE SCENE below, with gentle fill — both faces and the pet equally luminous, no one in shadow, a delicate rim light in both people's hair. They must look truly photographed together in this place at this moment — and both faces must always stay BRIGHT.

COMPOSITION BASE — tight family three-shot, faces first:
- The two people are close together with the pet HELD gently in one person's arms at chest height OR seated proudly between them — whichever suits the pet's size — the pet's face fully visible and turned toward the camera.
- Framed from roughly the CHEST UP so both human faces and the pet's face are as LARGE in the frame as possible. All three faces fully visible and unobstructed — the pet never covers a person's face, no hand covers the pet's face. Hands holding or touching the pet completely natural: correct fingers, relaxed grip, clean boundaries between hands and fur. Correct natural size scale. The warmth follows THE SCENE below.`;
const SCENE = `THE SCENE — 반려가족 사진 (family portrait with pet):
- The mood: a warm, timeless premium family portrait — two people and their beloved pet, the photo that goes in a frame at home.
- WARDROBE: a coordinated premium family-studio look in soft white, cream, beige, and light denim-blue — neat knits, shirts, and blouses per each person's build and confirmed gender. Elegant and timeless, no matching uniforms, no logos or readable text.
- WARMTH: a family in love with their pet — gentle closeness, proud happy expressions, genuine warmth.
...
- KEY LIGHT MOOD: bright, soft, warm family-studio light — all three faces equally luminous.`;
...
- PET CHECK: side by side with Image 3, is this the exact same pet — same breed, same coat, same markings? Exactly two people and one pet? Hands natural where they touch the fur?
...
- A pet of a different breed, color, size, or marking pattern; a generic "cute pet" replacing THIS pet; a duplicated pet or a second animal; pet costumes or added accessories.
- Warped hands or fingers where they hold the pet; fur melting into clothing; extra or missing paws, tails, or limbs.
...
async function generateFamilypet(image1DataUrl: string, image2DataUrl: string, image3DataUrl: string, gender1: string, gender2: string): Promise<string> {
  const img1 = parseImage(image1DataUrl);
  const img2 = parseImage(image2DataUrl);
  const img3 = parseImage(image3DataUrl);
  ...
  try {
      ...
      `https://generativelanguage.googleapis.com/v1beta/models/${GEMINI_MODEL}:generateContent`,
      {
        ...
        body: JSON.stringify({
          contents: [{ parts: [
            ...
            { inline_data: { mime_type: img3.mimeType, data: img3.data } },
          ] }],
    ...
    if ((e as { name?: string })?.name === "AbortError") {
      console.error(`[TIMEOUT][familypet] 140초 무응답 ${Date.now() - t0}ms`);
      throw new Error("이미지 생성이 140초를 넘겨 중단했어요. 다시 시도해주세요.");
    }
    throw e;
  }
  clearTimeout(timer);
  console.log(`[familypet] model=${GEMINI_MODEL} g1=${gender1} g2=${gender2} status=${res.status} ${Date.now() - t0}ms`);
...
async function handler(request: NextRequest) {
  try {
    ...
    const image3: string = body?.image3;
    ...
    if (!image1 || !image2 || !image3) return NextResponse.json({ error: "두 분과 반려동물의 사진을 모두 올려주세요." }, { status: 400 });
    const output = await generateFamilypet(image1, image2, image3, gender1, gender2);
...
export const POST = withCoin("familypet", 0, handler); // COIN_DORMANT: 실가격 3
