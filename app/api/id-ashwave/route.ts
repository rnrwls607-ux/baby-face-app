...
- Hair: restyle into soft, loose shoulder-length waves in a cool ash-brown color, glossy and airy, with a natural part. The hair outline must be clean and smooth where it meets the background — a soft, even silhouette with no jagged, lumpy, or pixelated edges. (This overrides the source hairstyle, but NOT the face.)
- Clothing: a soft pastel-lavender knit top, clean and gentle, simple neckline.
- Background: solid, even, soft neutral light gray, smooth and clean, no shadows, no props.

STYLING
- Expression: a gentle, natural, closed-lip smile, soft and serene. Eyes open, relaxed, on camera.
...
async function generateOneIdPhoto(imageDataUrls: string[]): Promise<string> {
  ...
  clearTimeout(timer);
  console.log(`[id-ashwave] model=${GEMINI_MODEL} status=${res.status} ${Date.now() - t0}ms`);

  if (!res.ok) throw new Error(await geminiFriendlyError(res, "id-ashwave"));
...
async function handler(request: NextRequest) {
  try {
    ...
    console.error("id-ashwave error:", err?.message);
    return NextResponse.json({ error: err?.message || "오류가 발생했습니다." }, { status: 500 });
  }
}

export const POST = withCoin("idashwave", 0, handler); // COIN_DORMANT: 실가격 9
