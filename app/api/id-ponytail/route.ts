...
- Hair: restyle into a sleek LOW PONYTAIL — hair pulled back smoothly and tied low at the nape, clean and professional, no flyaways, forehead and ears visible. (This overrides the source hairstyle, but NOT the face.)
- Clothing: a sharp navy formal suit jacket over a clean white top, professional.
- Background: solid cool light gray tone, smooth and even, no shadows, no props.
...
async function generateOneIdPhoto(imageDataUrls: string[]): Promise<string> {
  ...
  clearTimeout(timer);
  console.log(`[id-ponytail] model=${GEMINI_MODEL} status=${res.status} ${Date.now() - t0}ms`);

  if (!res.ok) throw new Error(await geminiFriendlyError(res, "id-ponytail"));
...
async function handler(request: NextRequest) {
  try {
    ...
    console.error("id-ponytail error:", err?.message);
    return NextResponse.json({ error: err?.message || "오류가 발생했습니다." }, { status: 500 });
  }
}

export const POST = withCoin("idponytail", 0, handler); // COIN_DORMANT: 실가격 9
