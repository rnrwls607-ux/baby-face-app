...
- Hair: restyle into a chin-to-shoulder length bob with soft inward C-curl ends, glossy and bouncy, fresh and youthful, with a soft natural part. (This overrides the source hairstyle, but NOT the face.)
- Clothing: a soft light-beige knit top, clean and cozy, simple neckline.
- Background: solid soft butter-yellow tone, smooth and even, no shadows, no props.
...
async function generateOneIdPhoto(imageDataUrls: string[]): Promise<string> {
  ...
  clearTimeout(timer);
  console.log(`[id-wavebob] model=${GEMINI_MODEL} status=${res.status} ${Date.now() - t0}ms`);

  if (!res.ok) throw new Error(await geminiFriendlyError(res, "id-wavebob"));
...
async function handler(request: NextRequest) {
  try {
    ...
    console.error("id-wavebob error:", err?.message);
    return NextResponse.json({ error: err?.message || "오류가 발생했습니다." }, { status: 500 });
  }
}

export const POST = withCoin("idwavebob", 0, handler); // COIN_DORMANT: 실가격 9
