...
// 배치 2-A — 관상 화보 (Pro 단일입력 route 구조 복제, 크롭 없음 = 원본 비율 유지)
...
async function generateFortunecard(imageDataUrl: string): Promise<string> {
...
oriental fortune portrait of them in the scene described below.
...
- Hair: a trendy modern Korean hairstyle that suits them, in their true
  ...
  naturally if they wear them, and naturally with any traditional
  styling the scene requires). Never a dated style that ages them.
...
THE SCENE — auspicious oriental fortune portrait:
- The person is portrayed as the subject of a precious "fortune-filled"
  oriental portrait — the mood of a traditional physiognomy portrait
  that says "this face carries great fortune," rendered as a luxurious
  modern photograph.
- Light (the face comes FIRST): flawless beauty lighting on the person —
  ...
  idol-grade luminous, the skin tone kept fresh, clear, and true —
  while the warm candle-amber glow and soft drifting golden particles
  stay in the BACKGROUND as atmosphere. ★The amber light must NEVER
  tint the face yellow, sallow, or dull — the face is always the
  brightest, freshest, most flattering element in the frame; a serene,
  auspicious, blessed mood.
- Wardrobe: an elegant modernized hanbok-inspired outfit in auspicious
  tones that suit the person — deep red, jade green, or ivory silk with
  subtle gold sheen, refined and luxurious, clean fit around the
  neckline. No lettering, no embroidery that forms characters.
- Setting: a softly blurred traditional backdrop — hanji-paper texture,
  a hint of ink-wash mountain painting, a branch of blossom, drifting
  gold dust — every element BARE of text: no calligraphy, no hanja, no
  hangul, no characters of any kind, even blurred character-like brush
  shapes are a failure.
- The overall feel: "a face blessed with fortune" — dignified, warm,
  and radiant, like a treasured family portrait.
POSE — a poised, serene pose: relaxed shoulders, a gentle confident
gaze toward the camera, quiet dignity.
...
the background melting into soft creamy bokeh. Rich, warm, film-like
...
- Face bright, fresh, and clean-toned — never yellowed by the amber
  light?
...
- A yellow, sallow, or dull-toned face from the amber light.
- Any calligraphy, hanja, hangul, or character-like brush shapes
  anywhere.
...
- Plastic waxy skin, dead flat lighting, murky shadows on the face,
  oversaturated HDR.
- Any shamanistic, occult, or religious imagery — this is an elegant
  pictorial, not a ritual.
...
oriental fortune portrait, at the absolute best of their life. High
resolution, no text, no watermark, no border.`;
  ...
  try {
    ...
    if ((e as { name?: string })?.name === "AbortError") {
      console.error(`[TIMEOUT][fortunecard] 230초 무응답 ${Date.now() - t0}ms`);
      throw new Error("이미지 생성이 230초를 넘겨 중단했어요. 다시 시도해주세요.");
    }
    throw e;
  }
  clearTimeout(timer);
  console.log(`[fortunecard] model=${GEMINI_MODEL} status=${res.status} ${Date.now() - t0}ms`);
  if (!res.ok) throw new Error(await geminiFriendlyError(res, "fortunecard", "생성에 실패했어요. 다른 사진으로 다시 시도해주세요."));
  ...
  console.log(`[fortunecard] finish=${cand?.finishReason || "-"} block=${data?.promptFeedback?.blockReason || "-"} parts=${respParts.length} img=${imgParts.length} ${Date.now() - t0}ms`);
  const chosen = (finalParts.length ? finalParts : imgParts).pop();
  const b64 = chosen?.inlineData?.data || chosen?.inline_data?.data;
  if (!b64) {
    const txt = respParts.find((p: { text?: string }) => p.text)?.text;
    console.error(`[fortunecard] 이미지 없음 — finish=${cand?.finishReason || "-"} text=${(txt || "").slice(0, 500)}`);
...
async function handler(request: NextRequest) {
  try {
    ...
    const output = await generateFortunecard(image);
    return NextResponse.json({ output: [output] });
  } catch (e: unknown) {
    const err = e as { message?: string };
    console.error("fortunecard error:", err?.message);
    return NextResponse.json({ error: err?.message || "오류가 발생했습니다." }, { status: 500 });
  }
}

export const POST = withCoin("fortunecard", 0, handler); // COIN_DORMANT: 실가격 3 · gemini-3-pro-image
