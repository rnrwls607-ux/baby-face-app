...
const SCENE = `THE SCENE — 웨딩 한복 커플 (Korean traditional wedding pictorial):
- The mood: a breathtaking traditional Korean wedding pictorial — a couple in exquisite wedding hanbok, romantic and regal. This is THEM wearing wedding hanbok — never a stock bride or groom model. Their faces and hair stay exactly theirs (per the laws above); only the clothing, setting, and light are the wedding.
- WARDROBE: magnificent wedding-grade hanbok appropriate to each person's confirmed gender — for a female person, a luminous bridal hanbok with delicate embroidery and graceful flowing lines; for a male person, a dignified groom's hanbok with an elegant silk overcoat in deep tasteful tones. The two hanbok in DIFFERENT harmonizing festive colors — glowing silks, tasteful and regal, never costume-cheap. Crisp collars, clean goreum, no tangled fabric.
- HAIR RULE: each person's OWN hair exactly as the HAIR IDENTITY LAW — own color, length, and silhouette, softly polished. NO headpieces, NO crowns, NO traditional hair ornaments, NO updos — nothing on or around the hair.
- WARMTH: romantic and tender — facing each other slightly or side by side, hands gently joined or a soft arm link; expressions warm, deeply affectionate, softly smiling.
- BACKDROP: a beautiful hanok courtyard dressed for a wedding — warm wood, hanji doors, soft silk drapery and delicate floral touches melting into golden bokeh. Regal and romantic, never busy. All elements completely TEXT-FREE.
- KEY LIGHT MOOD: warm, golden, celebratory light — both faces luminous.`;
...
async function generateHanbokcouple(image1DataUrl: string, image2DataUrl: string, gender1: string, gender2: string): Promise<string> {
  ...
  try {
    ...
    if ((e as { name?: string })?.name === "AbortError") {
      console.error(`[TIMEOUT][hanbokcouple] 140초 무응답 ${Date.now() - t0}ms`);
      throw new Error("이미지 생성이 140초를 넘겨 중단했어요. 다시 시도해주세요.");
    }
    throw e;
  }
  clearTimeout(timer);
  console.log(`[hanbokcouple] model=${GEMINI_MODEL} g1=${gender1} g2=${gender2} status=${res.status} ${Date.now() - t0}ms`);
...
async function handler(request: NextRequest) {
  try {
    ...
    const output = await generateHanbokcouple(image1, image2, gender1, gender2);
...
export const POST = withCoin("hanbokcouple", 0, handler); // COIN_DORMANT: 실가격 3
