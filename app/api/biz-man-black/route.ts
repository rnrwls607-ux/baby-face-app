...
- Clothing: a well-tailored BLACK men's business suit jacket over a crisp white dress shirt, with a neat, tasteful necktie (solid black, deep gray, or burgundy), properly knotted and centered. Sharp, formal, high-end — the outfit clean and intentional.
...
async function generateOneBizPhoto(imageDataUrls: string[], pose: string): Promise<string> {
  ...
  clearTimeout(timer);
  console.log(`[biz-man-black] status=${res.status} ${Date.now() - t0}ms`);

  if (!res.ok) throw new Error(await geminiFriendlyError(res, "biz-man-black"));
...
async function handler(request: NextRequest) {
  try {
    ...
    console.error("biz-man-black error:", err?.message);
    return NextResponse.json({ error: err?.message || "오류가 발생했습니다." }, { status: 500 });
  }
}

export const POST = withCoin("bizmblack", 0, handler); // COIN_DORMANT: 실가격 9
