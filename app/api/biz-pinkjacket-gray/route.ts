...
// 여성 비즈프로필 (핑크 트위드 재킷 셋업 + 밝은 회색 배경). {POSE} 자리에 포즈별 지시.
...
STYLING (women's business profile — pink tweed jacket set / bright light gray background)
- Clothing: a refined, well-fitted soft PINK tweed jacket (Chanel-style bouclé tweed) worn over a
  clean white or ivory blouse/shell, paired with a matching pink tweed skirt or neutral trousers
  suited to the framing. An elegant, feminine, youthful premium look — structured but soft, bright
  and graceful. The tweed texture is subtle and high-end; the outfit looks smooth and intentional,
  no messy wrinkles.
...
async function generateOneBizPhoto(imageDataUrls: string[], pose: string): Promise<string> {
  ...
  clearTimeout(timer);
  console.log(`[biz-pinkjacket-gray] status=${res.status} ${Date.now() - t0}ms`);

  if (!res.ok) throw new Error(await geminiFriendlyError(res, "biz-pinkjacket-gray"));
...
async function handler(request: NextRequest) {
  try {
    ...
    console.error("biz-pinkjacket-gray error:", err?.message);
    return NextResponse.json({ error: err?.message || "오류가 발생했습니다." }, { status: 500 });
  }
}

export const POST = withCoin("bizpinkjacket", 0, handler); // COIN_DORMANT: 실가격 9
