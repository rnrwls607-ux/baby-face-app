...
const SCENE = `THE SCENE — 호캉스 풀빌라 화보 (luxury hotel pool):
- A luxurious hotel infinity pool on a brilliant summer day: clear turquoise water sparkling behind them, sleek resort architecture and sun loungers softly blurred, a bright blue sky with soft clouds — five-star vacation glamour.
- Key light: bright, clean, flattering sunlight with a soft bounce from the water — the face luminous and fresh, eyes relaxed and open, never squinting or harshly shadowed.

WARDROBE — chic resort look (not swimwear):
- Woman: an elegant summer resort outfit — a chic flowing sundress or a refined linen set in white or a soft luminous tone; optionally a wide-brim sunhat held in hand (never worn shading the face).
- Man: a crisp open-collar resort shirt in white or a light tone with tailored light shorts or trousers — clean, expensive-looking, effortless.

POSE:
- A relaxed luxury-vacation pose: leaning lightly on the pool edge or a lounger, strolling along the poolside, or glancing back with a soft confident smile.`;
...
async function generateHocance(imageDataUrl: string): Promise<string> {
  ...
  clearTimeout(timer);
  console.log(`[hocance] model=${GEMINI_MODEL} status=${res.status} ${Date.now() - t0}ms`);
  if (!res.ok) throw new Error(await geminiFriendlyError(res, "hocance"));
...
async function handler(request: NextRequest) {
  try {
    ...
    const output = await generateHocance(image);
    return NextResponse.json({ output: [output] });
  } catch (e: unknown) {
    const err = e as { message?: string };
    console.error("hocance error:", err?.message);
    return NextResponse.json({ error: err?.message || "오류가 발생했습니다." }, { status: 500 });
  }
}

export const POST = withCoin("hocance", 0, handler); // COIN_DORMANT: 실가격 3
