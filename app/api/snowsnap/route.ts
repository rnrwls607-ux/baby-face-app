...
// 배치 2-A — 첫눈 스냅 (Pro 단일입력 route 구조 복제, 크롭 없음 = 원본 비율 유지)
...
async function generateSnowsnap(imageDataUrl: string): Promise<string> {
...
first-snow winter snap portrait of them in the scene described below.
...
- Hair: a trendy modern Korean hairstyle that suits them, in their true
  ...
  naturally if they wear them, and naturally with any winter headwear
  the scene requires). Never a dated style that ages them.
...
THE SCENE — romantic first-snow winter snap:
- The person is captured in a softly falling snow scene — the dreamy
  first-snow moment: gentle snowflakes drifting around them, cheeks
  fresh, eyes bright, the quiet magic of the season's first snow.
- Light (the face comes FIRST): flawless beauty lighting on the person —
  ...
  crisp — while the soft overcast winter daylight and the warm amber of
  distant street lamps stay in the BACKGROUND as atmosphere. ★The cool
  blue winter tones must NEVER make the face look pale, gray, or cold —
  the face stays warm, rosy, and luminous, the brightest element in the
  frame; a romantic, hushed, snow-globe mood.
- Snow rendering: soft mid-air snowflakes in gentle focus falloff — a
  few crisp flakes near the person, the rest melting into dreamy bokeh;
  a delicate dusting on the shoulders and hair, never clumpy or fake.
- Wardrobe: a chic winter look that suits the person — a beautiful wool
  coat or padded jacket in a flattering tone, a soft knit scarf or
  beanie styled so the face stays fully open and bright (nothing
  covering the forehead, eyebrows, or eyes; the hairline showing
  naturally). Clean fit around the neckline. No logos, no lettering.
- Setting: a softly blurred snowy street or park at dusk — bare branches
  catching snow, warm lamp glow, distant city lights — lived-in winter
  textures, like a real snowy evening, never a rendered backdrop. No
  readable text on any sign or surface.
POSE — a natural winter-moment pose: looking toward the camera with a
soft bright smile, or a candid glance upward at the falling snow.
...
- Face warm, rosy, and luminous — never paled or grayed by the winter
  tones?
...
- A pale, gray, or cold-looking face washed out by the winter tones.
- Clumpy, fake-looking snow blobs on the face or lens.
...
first-snow winter snap portrait, at the absolute best of their life.
High resolution, no text, no watermark, no border.`;
  ...
  try {
    ...
    if ((e as { name?: string })?.name === "AbortError") {
      console.error(`[TIMEOUT][snowsnap] 230초 무응답 ${Date.now() - t0}ms`);
      throw new Error("이미지 생성이 230초를 넘겨 중단했어요. 다시 시도해주세요.");
    }
    throw e;
  }
  clearTimeout(timer);
  console.log(`[snowsnap] model=${GEMINI_MODEL} status=${res.status} ${Date.now() - t0}ms`);
  if (!res.ok) throw new Error(await geminiFriendlyError(res, "snowsnap", "생성에 실패했어요. 다른 사진으로 다시 시도해주세요."));
  ...
  console.log(`[snowsnap] finish=${cand?.finishReason || "-"} block=${data?.promptFeedback?.blockReason || "-"} parts=${respParts.length} img=${imgParts.length} ${Date.now() - t0}ms`);
  const chosen = (finalParts.length ? finalParts : imgParts).pop();
  const b64 = chosen?.inlineData?.data || chosen?.inline_data?.data;
  if (!b64) {
    const txt = respParts.find((p: { text?: string }) => p.text)?.text;
    console.error(`[snowsnap] 이미지 없음 — finish=${cand?.finishReason || "-"} text=${(txt || "").slice(0, 500)}`);
...
async function handler(request: NextRequest) {
  try {
    ...
    const output = await generateSnowsnap(image);
    return NextResponse.json({ output: [output] });
  } catch (e: unknown) {
    const err = e as { message?: string };
    console.error("snowsnap error:", err?.message);
    return NextResponse.json({ error: err?.message || "오류가 발생했습니다." }, { status: 500 });
  }
}

export const POST = withCoin("snowsnap", 0, handler); // COIN_DORMANT: 실가격 3 · gemini-3-pro-image
