...
- The flat mineral-pigment coloring applies to the costume, background, and motifs — never to replace the fur's real color (a gray cat stays gray, not indigo or jade).
...
- Watermarks, borders, frames.`;
  ...
  try {
    ...
    if ((e as { name?: string })?.name === "AbortError") {
      console.error(`[TIMEOUT][petminhwa] 230초 무응답 ${Date.now() - t0}ms`);
  ...
  if (!res.ok) throw new Error(await geminiFriendlyError(res, "petminhwa", "생성에 실패했어요. 다른 사진으로 다시 시도해주세요."));
...
export const POST = withCoin("petminhwa", 0, handler); // COIN_DORMANT: 실가격 3
