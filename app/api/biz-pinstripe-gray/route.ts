...
// 여성 비즈프로필 (핀스트라이프 수트 + 밝은 회색 배경). {POSE} 자리에 포즈별 지시.
...
STYLING (women's business profile — pinstripe suit / bright light gray background)
- Clothing: a well-tailored PINSTRIPE women's business suit — a navy-and-white (or charcoal-and-
  white) fine pinstriped blazer over a clean white blouse or shell, paired with matching pinstriped
  trousers or a pencil skirt suited to the framing. Sharp, fitted, classic corporate style with a
  confident, polished pattern; intentional and professional. Freshly pressed and smooth, no messy
  wrinkles.
...
- Lighting: soft, even, flattering professional studio lighting (gentle key + fill), polished
  ...
  the light background. Keep the pinstripe pattern crisp and even, not blurry or warped.
...
async function generateOneBizPhoto(imageDataUrls: string[], pose: string): Promise<string> {
  ...
  clearTimeout(timer);
  console.log(`[biz-pinstripe-gray] status=${res.status} ${Date.now() - t0}ms`);

  if (!res.ok) throw new Error(await geminiFriendlyError(res, "biz-pinstripe-gray"));
...
async function handler(request: NextRequest) {
  try {
    ...
    console.error("biz-pinstripe-gray error:", err?.message);
    return NextResponse.json({ error: err?.message || "오류가 발생했습니다." }, { status: 500 });
  }
}

export const POST = withCoin("bizpinstripe", 0, handler); // COIN_DORMANT: 실가격 9
