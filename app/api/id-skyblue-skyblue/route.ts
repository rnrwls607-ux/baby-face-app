...
STANDARD LOOK
- Background: solid, even, pastel light-blue, no shadows.
- Clothing: a light-blue collared dress shirt, no jacket.
...
async function handler(request: NextRequest) {
  try {
    ...
    console.error("id-skyblue error:", err?.message);
    return NextResponse.json({ error: err?.message || "오류가 발생했습니다." }, { status: 500 });
  }
}

export const POST = withCoin("idskyblue", 0, handler); // COIN_DORMANT: 실가격 9
