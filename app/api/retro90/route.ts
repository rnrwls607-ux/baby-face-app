...
const SCENE = `THE SCENE — 90년대 레트로 사진관 (classic Korean photo studio):
- A nostalgic 1990s Korean photo studio: the classic painted canvas backdrop in softly mottled blue-gray or warm brown tones, a simple clean studio floor — that beloved old-family-album studio feel.
- Key light: classic soft studio key light with a gentle warm fill — flattering, even, bright on the face like a beautifully shot studio portrait.
- CAMERA MOOD OVERRIDE: render with the charm of a 90s film photograph — soft fine grain, warm gently-faded film color, a subtle vignette — while staying crisp, sharp, and high-resolution. Never actually blurry or low-quality.

RETRO FIREWALL — 90s the styling, NOT the face (critical):
- The 90s treatment applies ONLY to fashion, backdrop, and film mood. The FACE stays beautified to TODAY's young Korean celebrity standard — subtly younger than the input, never older, never dated makeup (no thin brows, no dark lips, no matte cakey base).

WARDROBE — 90s fashion worn charmingly:
- Woman: a lovable 90s look styled the modern way — an oversized cardigan or soft turtleneck, a neat collared blouse under a knit vest, or a denim jacket; simple dainty accessories.
- Man: clean 90s style — an oversized blazer over a white tee, a denim or corduroy jacket, or a soft knit; neat and effortless.
- HAIR NOTE: a modern youthful style with only a soft 90s twist (gentle layers, natural curtain bangs) — never a dated perm or stiff old hairstyle.

POSE:
- A charming classic studio pose: sitting or standing squarely toward the camera with a soft genuine smile, hands relaxed — sweet, wholesome, album-worthy.`;
...
async function generateRetro90(imageDataUrl: string): Promise<string> {
  ...
  clearTimeout(timer);
  console.log(`[retro90] model=${GEMINI_MODEL} status=${res.status} ${Date.now() - t0}ms`);
  if (!res.ok) throw new Error(await geminiFriendlyError(res, "retro90"));
...
async function handler(request: NextRequest) {
  try {
    ...
    const output = await generateRetro90(image);
    return NextResponse.json({ output: [output] });
  } catch (e: unknown) {
    const err = e as { message?: string };
    console.error("retro90 error:", err?.message);
    return NextResponse.json({ error: err?.message || "오류가 발생했습니다." }, { status: 500 });
  }
}

export const POST = withCoin("retro90", 0, handler); // COIN_DORMANT: 실가격 3
