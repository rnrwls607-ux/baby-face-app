...
TASK
You are RETOUCHING a real photograph of one real person into a single clean, elegant profile-style ID photo. One to six photos of the SAME individual are provided. You may restyle hair and clothing to the concept below, but you must NOT redesign the face. This concept is for WOMEN only — always render a woman.
...
- Keep the apparent age and natural bone structure as in the source, and the person's TRUE skin tone (correct any warm/cool color cast from the source lighting, but never lighten, darken, or shift the actual skin tone).
...
- Hair: restyle into a neat, chin-length BOB haircut, smooth and glossy, tucked slightly behind the ears, with a soft natural center or side part. Keep it tidy and elegant. (This overrides the source hairstyle, but NOT the face.)
- Clothing: an elegant off-shoulder top in clean white/ivory, bare shoulders and collarbones visible, simple and classy.
- Background: solid soft lavender (pale purple) gradient, smooth and clean, no shadows, no props.
...
- Do NOT copy the input's zoom. Always produce a head-and-shoulders composition (for off-shoulder, include the bare shoulders and upper chest line) — never more of the face, no matter how large it fills the frame in the source, and no matter how tightly cropped the input already is.
- Centered, facing straight at the camera, symmetric, portrait-lens perspective (~85mm, no distortion).
- Head from top of hair to chin fills about 45% of the frame height, small even margin above the head, eyes slightly above the middle. Shoulders reach both edges.
...
OUTPUT
- Vertical ID ratio (3.5:4.5), high-quality studio portrait, photorealistic, clean and elegant. Remember the two absolute rules: the SAME face under the new hair, inside the SAME fixed composition.`;
...
async function generateOneIdPhoto(imageDataUrls: string[]): Promise<string> {
  ...
  clearTimeout(timer);
  console.log(`[id-offshoulder] model=${GEMINI_MODEL} status=${res.status} ${Date.now() - t0}ms`);

  if (!res.ok) throw new Error(await geminiFriendlyError(res, "id-offshoulder"));
...
async function handler(request: NextRequest) {
  try {
    ...
    console.error("id-offshoulder error:", err?.message);
    return NextResponse.json({ error: err?.message || "오류가 발생했습니다." }, { status: 500 });
  }
}

export const POST = withCoin("idoffshoulder", 0, handler); // COIN_DORMANT: 실가격 9
