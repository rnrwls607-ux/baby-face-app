...
// 배치 2-A — 흑백 배우 프로필 (Pro 단일입력 route 구조 복제, 크롭 없음 = 원본 비율 유지)
...
async function generateMonoactor(imageDataUrl: string): Promise<string> {
...
black-and-white actor profile portrait of them in the scene described below.
...
- Hair: a trendy modern Korean hairstyle that suits them, with its tone
  faithfully mapped to monochrome (dark hair stays dark, light stays
  light), styled beautifully for the scene (around the glasses
...
THE SCENE — timeless black-and-white actor profile:
- The entire photo is a masterful BLACK AND WHITE portrait — the classic
  actor-profile look used by casting directors and film magazines:
  timeless, intense, quietly charismatic.
- Light: flawless beauty lighting on the person — a sculpting soft key
  light from about 45 degrees with delicate catchlights, gentle fill
  keeping the shadow side alive, and a clean rim light separating them
  from the background, clearly BEAUTIFYING, every feature crisp and
  luminous, the face glowing as the brightest element in the frame —
  while the monochrome world around them falls into a rich gradient of
  deep charcoal to silver; a serious, magnetic, editorial mood.
- Monochrome mastery: deep true blacks, glowing highlights, a full
  silky tonal range with fine film-like grain — never flat gray, never
  murky.
- Wardrobe: a simple timeless piece that reads beautifully in
  monochrome — a dark turtleneck, crisp shirt, or structured jacket
  with a clean neckline. No logos, no lettering.
- Backdrop: a seamless deep-toned studio background with subtle
  gradient falloff. No readable text anywhere.
...
the background melting into soft creamy bokeh. Rich, film-like
monochrome grade. Photorealistic, high resolution.
...
- Glasses exactly as the source? Hair tone faithful in monochrome?
ABSOLUTELY AVOID:
...
- Plastic waxy skin, dead flat lighting, murky shadows on the face,
  oversaturated HDR.
- Flat lifeless gray tones or murky muddy shadows.
...
black-and-white actor profile portrait, at the absolute best of their
life. High resolution, no text, no watermark, no border.`;
  ...
  try {
    ...
    if ((e as { name?: string })?.name === "AbortError") {
      console.error(`[TIMEOUT][monoactor] 230초 무응답 ${Date.now() - t0}ms`);
      throw new Error("이미지 생성이 230초를 넘겨 중단했어요. 다시 시도해주세요.");
    }
    throw e;
  }
  clearTimeout(timer);
  console.log(`[monoactor] model=${GEMINI_MODEL} status=${res.status} ${Date.now() - t0}ms`);
  if (!res.ok) throw new Error(await geminiFriendlyError(res, "monoactor", "생성에 실패했어요. 다른 사진으로 다시 시도해주세요."));
  ...
  console.log(`[monoactor] finish=${cand?.finishReason || "-"} block=${data?.promptFeedback?.blockReason || "-"} parts=${respParts.length} img=${imgParts.length} ${Date.now() - t0}ms`);
  const chosen = (finalParts.length ? finalParts : imgParts).pop();
  const b64 = chosen?.inlineData?.data || chosen?.inline_data?.data;
  if (!b64) {
    const txt = respParts.find((p: { text?: string }) => p.text)?.text;
    console.error(`[monoactor] 이미지 없음 — finish=${cand?.finishReason || "-"} text=${(txt || "").slice(0, 500)}`);
...
async function handler(request: NextRequest) {
  try {
    ...
    const output = await generateMonoactor(image);
    return NextResponse.json({ output: [output] });
  } catch (e: unknown) {
    const err = e as { message?: string };
    console.error("monoactor error:", err?.message);
    return NextResponse.json({ error: err?.message || "오류가 발생했습니다." }, { status: 500 });
  }
}

export const POST = withCoin("monoactor", 0, handler); // COIN_DORMANT: 실가격 3 · gemini-3-pro-image
