...
- Extra or missing paws, ears, tails; watermarks, borders.`;
  ...
  try {
    ...
    if ((e as { name?: string })?.name === "AbortError") {
      console.error(`[TIMEOUT][petbirthday] 230초 무응답 ${Date.now() - t0}ms`);
  ...
  if (!res.ok) throw new Error(await geminiFriendlyError(res, "petbirthday", "생성에 실패했어요. 다른 사진으로 다시 시도해주세요."));
  ...
  console.log(`[petbirthday] finish=${cand?.finishReason || "-"} block=${data?.promptFeedback?.blockReason || "-"} parts=${respParts.length} img=${imgParts.length} ${Date.now() - t0}ms`);
  const chosen = (finalParts.length ? finalParts : imgParts).pop();
  const b64 = chosen?.inlineData?.data || chosen?.inline_data?.data;
  if (!b64) {
    const txt = respParts.find((p: { text?: string }) => p.text)?.text;
    console.error(`[petbirthday] 이미지 없음 — finish=${cand?.finishReason || "-"} text=${(txt || "").slice(0, 500)}`);
...
export const POST = withCoin("petbirthday", 0, handler); // COIN_DORMANT: 실가격 3
