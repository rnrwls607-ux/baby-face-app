...
// 여성 비즈프로필 (쉬폰 블라우스 + 파스텔 스커트 + 밝은 회색 배경). {POSE} 자리에 포즈별 지시.
...
STYLING (women's business profile — chiffon blouse + pastel skirt / bright light gray background)
- Clothing: an elegant, well-fitted IVORY (soft off-white) CHIFFON blouse — a softly draping
  long-sleeved blouse with a refined, feminine silhouette — neatly tucked into a well-fitted soft
  PASTEL (light peach or blush) pencil skirt suited to the framing. NO blazer, NO outer layer. The
  blouse looks smooth, fresh, and gracefully draped (soft natural drape, not messy creases), bright
  and clean.
...
async function generateOneBizPhoto(imageDataUrls: string[], pose: string): Promise<string> {
  ...
  clearTimeout(timer);
  console.log(`[biz-chiffon-gray] status=${res.status} ${Date.now() - t0}ms`);

  if (!res.ok) throw new Error(await geminiFriendlyError(res, "biz-chiffon-gray"));
...
async function handler(request: NextRequest) {
  try {
    ...
    console.error("biz-chiffon-gray error:", err?.message);
    return NextResponse.json({ error: err?.message || "오류가 발생했습니다." }, { status: 500 });
  }
}

export const POST = withCoin("bizchiffon", 0, handler); // COIN_DORMANT: 실가격 9
