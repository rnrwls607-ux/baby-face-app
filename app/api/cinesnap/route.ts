...
// 배치 2-A — 시네필름 스냅 (Pro 단일입력 route 구조 복제, 크롭 없음 = 원본 비율 유지)
...
async function generateCinesnap(imageDataUrl: string): Promise<string> {
...
cinematic film still portrait of them in the scene described below.
...
1. SMALL FACE (most important): Slim the jawline into a soft, elegant
   ...
   should read about 15% smaller and more compact than the input — a
...
- Beautify in the aesthetic of TODAY's young Korean celebrities — fresh,
  ...
  never older.
- AIM HIGH: the finished face should read like a top-tier celebrity
  visual — the kind of face people stop scrolling for — while still
  being unmistakably this same person.
...
THE SCENE — one frame from a beautiful film:
- The photo must feel like a single still cut from a critically-acclaimed
  movie — quiet, emotional, and expensive-looking, as if an OST is
  playing somewhere.
- Light: flawless beauty lighting on the person — a bright soft key
  ...
  while the scene around them glows with soft window side-light and
  distant blurred neon and street lights melting into warm and teal
  bokeh; a quiet, cinematic, slightly wistful mood.
- Setting: a narrow atmospheric city alley at dusk, or beside a large
  cafe window — softly blurred behind the person with lived-in textures,
  like a real filming location, never a rendered backdrop. Any signs,
  posters, or storefront lettering are rendered only as soft unreadable
  glowing shapes — NO readable characters of any kind anywhere.
- Cinematic grade: anamorphic-lens feel with shallow depth of field and
  soft oval bokeh, a refined teal-and-orange color grade, gentle film
  grain and subtle halation on highlights — the polish of a movie teaser
  poster, while the face itself stays clean, bright, and flawless.
- Wardrobe: an effortlessly stylish, slightly cinematic everyday outfit
  that suits the person — simple refined pieces with a clean fit around
  the neckline and shoulders so the face stays the hero. No logos, no
  brand marks, no lettering on any clothing.
POSE — a quiet candid pose: standing or leaning naturally, gaze toward
the camera or slightly past it, like a protagonist caught mid-scene.
...
hero of the frame.
CAMERA — shot like an anamorphic cinema lens portrait: the person
tack-sharp, the background melting into soft creamy cinematic bokeh.
Rich, film-like color grade. Photorealistic, high resolution.
...
- Plastic waxy skin, dead flat lighting, murky shadows on the face,
  oversaturated HDR.
- Copying any specific existing movie's poster, title design, or iconic
  scene.
...
cinematic film still portrait, at the absolute best of their life. High
resolution, no text, no watermark, no border.`;
  ...
  try {
    ...
    if ((e as { name?: string })?.name === "AbortError") {
      console.error(`[TIMEOUT][cinesnap] 230초 무응답 ${Date.now() - t0}ms`);
      throw new Error("이미지 생성이 230초를 넘겨 중단했어요. 다시 시도해주세요.");
    }
    throw e;
  }
  clearTimeout(timer);
  console.log(`[cinesnap] model=${GEMINI_MODEL} status=${res.status} ${Date.now() - t0}ms`);
  if (!res.ok) throw new Error(await geminiFriendlyError(res, "cinesnap", "생성에 실패했어요. 다른 사진으로 다시 시도해주세요."));
  ...
  console.log(`[cinesnap] finish=${cand?.finishReason || "-"} block=${data?.promptFeedback?.blockReason || "-"} parts=${respParts.length} img=${imgParts.length} ${Date.now() - t0}ms`);
  const chosen = (finalParts.length ? finalParts : imgParts).pop();
  const b64 = chosen?.inlineData?.data || chosen?.inline_data?.data;
  if (!b64) {
    const txt = respParts.find((p: { text?: string }) => p.text)?.text;
    console.error(`[cinesnap] 이미지 없음 — finish=${cand?.finishReason || "-"} text=${(txt || "").slice(0, 500)}`);
...
async function handler(request: NextRequest) {
  try {
    ...
    const output = await generateCinesnap(image);
    return NextResponse.json({ output: [output] });
  } catch (e: unknown) {
    const err = e as { message?: string };
    console.error("cinesnap error:", err?.message);
    return NextResponse.json({ error: err?.message || "오류가 발생했습니다." }, { status: 500 });
  }
}

export const POST = withCoin("cinesnap", 0, handler); // COIN_DORMANT: 실가격 3 · gemini-3-pro-image
