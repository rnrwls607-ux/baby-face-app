...
async function generateInterior(imageDataUrl: string): Promise<string> {
  const img = parseImage(imageDataUrl);
  const prompt = `You are an elite interior-design and real-estate staging artist. Take this photo of a room and transform it into a beautifully furnished, styled, magazine-quality modern interior - while keeping it unmistakably the SAME room. Be a confident designer: the result should look professionally styled and aspirational, not a timid edit. Suit a modern Korean home (apartment / villa / house) or commercial space - natural and livable, not oversized or cluttered.
...
=== STYLE BOLDLY (transform the look like a pro designer) ===
- Furniture and decor: add or replace with realistic, well-proportioned modern pieces in a balanced, intentional layout with a clear focal point. Ground everything correctly - real scale, contact shadows, matching perspective. Tasteful and livable; do NOT overcrowd the room or add floating or duplicated items.
- Cohesive design: choose one harmonious palette and style (e.g. warm minimal, scandinavian, natural/cozy, modern). Add tasteful accents - a few plants, soft textiles, simple art, layered lighting.
- Surfaces: refresh flooring, walls, and ceiling to clean modern finishes that suit the style, following the existing surfaces (no structural change).
...
- High-resolution, sharp, and professional. No watermark, no text overlay, no added borders or logos. The same room - beautifully styled and inviting.`;
  ...
  clearTimeout(timer);
  console.log(`[interior] model=${GEMINI_MODEL} status=${res.status} ${Date.now() - t0}ms`);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const output = await generateInterior(image);
