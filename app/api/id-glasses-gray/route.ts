...
// 증명사진 프롬프트 (정장 + 안경 친화 + 밝은 회색 배경 / 남녀공용)
...
- Clothing: a sharp dark formal suit jacket over a clean white collared shirt.
...
- Eyewear & accessories: this is a WITH-GLASSES ID photo — the final image MUST show the person wearing clean, clear eyeglasses, WITHOUT EXCEPTION. If the person already wears eyeglasses in the source, keep the SAME glasses with their real shape and frame color. If the source has NO glasses, you MUST add well-fitting eyeglasses with a simple, modern frame that suits the face. In every case: sit the glasses straight and level, and render the lenses perfectly clear and transparent with NO glare, NO reflections, and NO distortion, so both eyes are fully visible through the lenses. Render the ears clean — remove all earrings and ear jewelry even if present in the source. No other jewelry or accessories.
...
async function generateOneIdPhoto(imageDataUrls: string[]): Promise<string> {
  ...
  clearTimeout(timer);
  console.log(`[id-glasses-gray] model=${GEMINI_MODEL} status=${res.status} ${Date.now() - t0}ms`);

  if (!res.ok) throw new Error(await geminiFriendlyError(res, "id-glasses-gray"));
...
async function handler(request: NextRequest) {
  try {
    ...
    console.error("id-glasses-gray error:", err?.message);
    return NextResponse.json({ error: err?.message || "오류가 발생했습니다." }, { status: 500 });
  }
}

export const POST = withCoin("idglasses", 0, handler); // COIN_DORMANT: 실가격 9
