...
// 증명사진 프롬프트 (터틀넥 니트 + 밝은 회색 배경 / 남녀공용)
...
- Clothing: a clean, modern turtleneck knit top in a calm neutral tone.
...
async function generateOneIdPhoto(imageDataUrls: string[]): Promise<string> {
  ...
  clearTimeout(timer);
  console.log(`[id-turtleneck-gray] model=${GEMINI_MODEL} status=${res.status} ${Date.now() - t0}ms`);

  if (!res.ok) throw new Error(await geminiFriendlyError(res, "id-turtleneck-gray"));
...
async function handler(request: NextRequest) {
  try {
    ...
    console.error("id-turtleneck-gray error:", err?.message);
    return NextResponse.json({ error: err?.message || "오류가 발생했습니다." }, { status: 500 });
  }
}

export const POST = withCoin("idturtleneck", 0, handler); // COIN_DORMANT: 실가격 9
