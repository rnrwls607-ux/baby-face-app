...
// 여성 비즈프로필 (타이 블라우스 + 버건디 슬랙스 + 밝은 회색 배경). {POSE} 자리에 포즈별 지시.
...
STYLING (women's business profile — tie blouse + burgundy trousers / bright light gray background)
- Clothing: an elegant IVORY pussy-bow tie blouse with a soft ribbon tied at the neck, neatly
  tucked into well-fitted BURGUNDY (deep wine) dress trousers. A confident, warm, sophisticated
  combination. NO blazer, NO outer layer. The blouse is smooth and crisp, freshly pressed, no messy
  wrinkles; the burgundy trousers a rich, even color.
...
async function generateOneBizPhoto(imageDataUrls: string[], pose: string): Promise<string> {
  ...
  clearTimeout(timer);
  console.log(`[biz-burgundy-gray] status=${res.status} ${Date.now() - t0}ms`);

  if (!res.ok) throw new Error(await geminiFriendlyError(res, "biz-burgundy-gray"));
...
async function handler(request: NextRequest) {
  try {
    ...
    console.error("biz-burgundy-gray error:", err?.message);
    return NextResponse.json({ error: err?.message || "오류가 발생했습니다." }, { status: 500 });
  }
}

export const POST = withCoin("bizburgundy", 0, handler); // COIN_DORMANT: 실가격 9
