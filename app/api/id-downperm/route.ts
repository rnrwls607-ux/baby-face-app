...
TASK
You are RETOUCHING a real photograph of one real person into a single clean, fresh profile-style ID photo. One to six photos of the SAME individual are provided. You may restyle hair and clothing to the concept below, but you must NOT redesign the face. This concept is for MEN only — always render a man.
...
- Hair: restyle into a soft, natural men's down-perm with a gentle fringe lightly resting on the forehead, relaxed but tidy, natural black hair. The hair outline must be clean and smooth where it meets the background. (This overrides the source hairstyle, but NOT the face.)
- Clothing: a crisp clean white collared shirt, fresh and neat, no tie.
- Background: solid, even, soft light sky-blue, smooth and clean, no shadows, no props.

STYLING
- Expression: a gentle, natural, closed-lip smile, bright and friendly. Eyes open, relaxed, on camera.
...
OUTPUT
- Vertical ID ratio (3.5:4.5), high-quality studio portrait, photorealistic, fresh and clean. Remember the two absolute rules: the SAME face under the new hair, inside the SAME fixed composition.`;
...
async function generateOneIdPhoto(imageDataUrls: string[]): Promise<string> {
  ...
  clearTimeout(timer);
  console.log(`[id-downperm] model=${GEMINI_MODEL} status=${res.status} ${Date.now() - t0}ms`);

  if (!res.ok) throw new Error(await geminiFriendlyError(res, "id-downperm"));
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error("id-downperm error:", err?.message);
