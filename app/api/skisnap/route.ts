...
// 스키장 겨울 스냅 — person:pro (cheerglam 템플릿 복제, new-concept.mjs 생성)
...
async function generateSkisnap(imageDataUrl: string): Promise<string> {
...
ski resort winter snap portrait of them in the scene described below.
...
- Hair: a trendy modern Korean hairstyle that suits them, in their true
  ...
  naturally if they wear them, and falling naturally from under the
  beanie with the hairline and front hair clearly showing). Never a dated
  style that ages them.
...
THE SCENE — a bright, clear winter day at the top of a ski slope on a
snow-covered mountain.
- Setting: sparkling fresh snow all around, a groomed slope falling away
  behind the person, snow-laden pine trees at mid-distance, majestic white
  mountain ridges and a deep, vivid blue sky far behind — all of it
  softening into gentle bokeh so the person stays the clear hero. No
  other skiers or people anywhere in the frame. No lift towers, signs, or
  banners.
- Wardrobe: a stylish, well-fitted modern ski outfit in a fresh color
  that flatters the person's skin tone — a puffer jacket or ski jacket in
  ivory, sky blue, lilac, or coral with a clean, structured collar that
  frames the face and neck; a soft knit beanie in a matching or neutral
  tone; slim ski gloves. Fabrics look real: matte technical shell, soft
  knit, subtle quilting. Nothing bulky or shapeless around the neck and
  jaw.
- HEADWEAR RULE (critical): the beanie sits BACK on the head so it NEVER
  shadows or covers the forehead, eyebrows, or eyes — the full face stays
  bright, open, and clearly visible, with the hairline and front hair
  showing naturally. The ski goggles are HEADWEAR here, NOT eyewear: they
  rest pushed up on TOP of the beanie, strap around the hat, lenses
  facing up and away — never over the eyes, never on the forehead, never
  casting a shadow or reflecting onto the face. Their lenses are plain
  dark or mirrored with no visible reflections of anything. If the person
  wears glasses, the glasses stay on the face exactly as the source and
  the goggles stay on top of the beanie — the two never overlap.
- Light: flawless beauty lighting on the person — a bright, CLEAN,
  ...
  them glows with brilliant high-altitude winter sunlight, dazzling white
  snow and crisp blue sky; fresh, exhilarating, joyful winter-holiday mood.
  SNOW-GLARE ISOLATION (critical): the intense reflected light from the
  snow lives in the BACKGROUND only — it must NEVER flatten the face pale,
  gray, washed-out, or glare-blown, and must never cast a blue or cyan
  tint on the skin. The face keeps healthy warm color, soft dimensional
  shadows, and rosy warmth on the cheeks, as if lit by its own beauty
  light — never a chalky, overexposed, or cold-blue face.
- Every surface BARE — no readable text, letters, numbers, logos, brand
  marks, patches, or signage anywhere on the jacket, beanie, goggles,
  gloves, poles, or in the background. Even illegible text shapes are a
  failure.
POSE — standing on the slope facing the camera, relaxed and radiant, a
bright natural winter smile or a fresh, cool half-smile; one gloved hand
lightly touching the goggles on top of the beanie or resting casually on
a ski pole held to the side, body turned slightly for a flattering line.
...
the snowy slope and mountains melting into soft, bright, creamy bokeh.
Clean, crisp, film-like winter color grade — pure whites, vivid blue sky,
and a warm, luminous, healthy face. Photorealistic, high resolution — real
snow texture, real fabric, real skin.
...
- Are the goggles on TOP of the beanie — eyes, eyebrows, forehead, and
  hairline all fully visible and unshadowed?
- Is the face warm, dimensional, and healthy — not pale, gray, blue-tinted,
  or glare-washed — while the snow behind glows brilliantly white?
- Zero readable text, logos, or patches on any gear?
ABSOLUTELY AVOID:
...
- Goggles worn over the eyes, goggles on the forehead, a helmet, a hood,
  or any headwear covering or shadowing the forehead, eyebrows, or eyes.
...
- Plastic waxy skin, dead flat lighting, murky shadows on the face,
  oversaturated HDR.
- A pale, chalky, gray, or blue-tinted face; blown-out glare on the skin;
  a face flattened by snow reflection.
- Crowds or other people in the background.
- Lift towers, signage, banners, brand patches, or a bulky shapeless
  outfit swallowing the neck and jaw.
- Any readable text, signs, or watermarks anywhere.
Output: one photorealistic photo — the same person in a complete ski resort winter snap portrait,
...
`;
  ...
  try {
    ...
    if ((e as { name?: string })?.name === "AbortError") {
      console.error(`[TIMEOUT][skisnap] 230초 무응답 ${Date.now() - t0}ms`);
      throw new Error("이미지 생성이 230초를 넘겨 중단했어요. 다시 시도해주세요.");
    }
    throw e;
  }
  clearTimeout(timer);
  console.log(`[skisnap] model=${GEMINI_MODEL} status=${res.status} ${Date.now() - t0}ms`);
  if (!res.ok) throw new Error(await geminiFriendlyError(res, "skisnap", "생성에 실패했어요. 다른 사진으로 다시 시도해주세요."));
  ...
  console.log(`[skisnap] finish=${cand?.finishReason || "-"} block=${data?.promptFeedback?.blockReason || "-"} parts=${respParts.length} img=${imgParts.length} ${Date.now() - t0}ms`);
  const chosen = (finalParts.length ? finalParts : imgParts).pop();
  const b64 = chosen?.inlineData?.data || chosen?.inline_data?.data;
  if (!b64) {
    const txt = respParts.find((p: { text?: string }) => p.text)?.text;
    console.error(`[skisnap] 이미지 없음 — finish=${cand?.finishReason || "-"} text=${(txt || "").slice(0, 500)}`);
...
async function handler(request: NextRequest) {
  try {
    ...
    const output = await generateSkisnap(image);
    return NextResponse.json({ output: [output] });
  } catch (e: unknown) {
    const err = e as { message?: string };
    console.error("skisnap error:", err?.message);
    return NextResponse.json({ error: err?.message || "오류가 발생했습니다." }, { status: 500 });
  }
}

export const POST = withCoin("skisnap", 0, handler); // COIN_DORMANT: 실가격 3 · gemini-3-pro-image
