...
// 감성 캠핑 스냅 — person:pro (cheerglam 템플릿 복제, new-concept.mjs 생성)
...
async function generateCampsnap(imageDataUrl: string): Promise<string> {
...
cozy evening camping snap portrait of them in the scene described below.
...
THE SCENE — cozy blue-hour camping snap:
- The person is captured at a campsite in the blue hour — the sky deep
  navy, pine silhouettes behind, a lantern glowing on the table and a
...
- Props: a warm camping lantern, a folding camp chair, a small enamel
  ...
  no brand logos, no printed lettering on any item.
- Wardrobe: a cozy outdoor look that suits the person — a soft flannel
  or plaid shirt layered over a knit or tee, or a fleece, in flattering
  warm-neutral tones, styled so the face stays fully open and bright
  (nothing covering the forehead, eyebrows, or eyes; the hairline
  showing naturally). Clean fit around the neckline. No logos, no
  lettering.
- Setting: a softly blurred blue-hour campground — deep blue sky, pine
  trees, the lantern and campfire melting into warm bokeh, lived-in
  outdoor textures, like a real camping evening, never a rendered
  backdrop. No readable text on any sign, gear, or surface.
POSE — a natural camping-evening moment: seated relaxed in the camp
chair, looking toward the camera with a soft bright smile, or a candid
glance at the lantern with the mug held lightly in both hands.
...
- Face clean, fresh, and luminous — never tinted orange or yellow by
  the firelight or lantern?
...
- An orange, yellow, or sallow-looking face tinted by the campfire or
  lantern, or fire-colored light flickering on the skin.
- Flames, smoke, or haze in front of or touching the face.
- Brand logos or printed lettering on any camping gear.
...
cozy evening camping snap portrait, at the absolute best of their life.
High resolution, no text, no watermark, no border.`;
  ...
  try {
    ...
    if ((e as { name?: string })?.name === "AbortError") {
      console.error(`[TIMEOUT][campsnap] 230초 무응답 ${Date.now() - t0}ms`);
      throw new Error("이미지 생성이 230초를 넘겨 중단했어요. 다시 시도해주세요.");
    }
    throw e;
  }
  clearTimeout(timer);
  console.log(`[campsnap] model=${GEMINI_MODEL} status=${res.status} ${Date.now() - t0}ms`);
  if (!res.ok) throw new Error(await geminiFriendlyError(res, "campsnap", "생성에 실패했어요. 다른 사진으로 다시 시도해주세요."));
  ...
  console.log(`[campsnap] finish=${cand?.finishReason || "-"} block=${data?.promptFeedback?.blockReason || "-"} parts=${respParts.length} img=${imgParts.length} ${Date.now() - t0}ms`);
  const chosen = (finalParts.length ? finalParts : imgParts).pop();
  const b64 = chosen?.inlineData?.data || chosen?.inline_data?.data;
  if (!b64) {
    const txt = respParts.find((p: { text?: string }) => p.text)?.text;
    console.error(`[campsnap] 이미지 없음 — finish=${cand?.finishReason || "-"} text=${(txt || "").slice(0, 500)}`);
...
async function handler(request: NextRequest) {
  try {
    ...
    const output = await generateCampsnap(image);
    return NextResponse.json({ output: [output] });
  } catch (e: unknown) {
    const err = e as { message?: string };
    console.error("campsnap error:", err?.message);
    return NextResponse.json({ error: err?.message || "오류가 발생했습니다." }, { status: 500 });
  }
}

export const POST = withCoin("campsnap", 0, handler); // COIN_DORMANT: 실가격 3 · gemini-3-pro-image
