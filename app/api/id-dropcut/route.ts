...
TASK
You are RETOUCHING a real photograph of one real person into a single clean, formal ID photo. One to six photos of the SAME individual are provided. You may restyle hair and clothing to the concept below, but you must NOT redesign the face. This concept is for MEN only — always render a man.
...
- Hair: restyle into a soft Korean "드랍컷" (drop cut) — a natural, slightly textured men's cut with gentle fringe softly covering part of the forehead, relaxed but tidy, natural black hair. (This overrides the source hairstyle, but NOT the face.)
- Clothing: a clean light-blue collared dress shirt, fresh and neat, top button area tidy, no tie.
- Background: solid soft gray-blue tone, smooth and even, no shadows, no props.
...
OUTPUT
- Vertical ID ratio (3.5:4.5), studio passport-photo quality, photorealistic, clean. Remember the two absolute rules: the SAME face under the new hair, inside the SAME fixed composition.`;
...
async function generateOneIdPhoto(imageDataUrls: string[]): Promise<string> {
  ...
  clearTimeout(timer);
  console.log(`[id-dropcut] model=${GEMINI_MODEL} status=${res.status} ${Date.now() - t0}ms`);

  if (!res.ok) throw new Error(await geminiFriendlyError(res, "id-dropcut"));
...
async function handler(request: NextRequest) {
  try {
    ...
    console.error("id-dropcut error:", err?.message);
    return NextResponse.json({ error: err?.message || "오류가 발생했습니다." }, { status: 500 });
  }
}

export const POST = withCoin("iddropcut", 0, handler); // COIN_DORMANT: 실가격 9
