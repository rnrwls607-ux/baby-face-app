...
// 여성 비즈프로필 (파스텔 스카이블루 블라우스 + 베이지/화이트 스커트 + 밝은 회색 배경). {POSE} 자리에 포즈별 지시.
...
STYLING (women's business profile — pastel sky-blue blouse + beige/white skirt / bright light gray background)
- Clothing: an elegant, well-fitted soft PASTEL SKY-BLUE blouse — a refined long-sleeved blouse
  with a clean, feminine silhouette, fresh and bright — neatly tucked into a well-fitted light
  BEIGE or off-white pencil skirt suited to the framing. A clean, youthful, approachable
  combination. NO blazer, NO outer layer. The blouse looks smooth, fresh, and softly draped,
  bright and clean, no messy wrinkles.
...
- Lighting: soft, even, flattering professional studio lighting (gentle key + fill), polished
  ...
  the light background. Keep light-colored garments bright and clean, but NOT blown out or overexposed.
...
async function generateOneBizPhoto(imageDataUrls: string[], pose: string): Promise<string> {
  ...
  clearTimeout(timer);
  console.log(`[biz-skyblouse-gray] status=${res.status} ${Date.now() - t0}ms`);

  if (!res.ok) throw new Error(await geminiFriendlyError(res, "biz-skyblouse-gray"));
...
async function handler(request: NextRequest) {
  try {
    ...
    console.error("biz-skyblouse-gray error:", err?.message);
    return NextResponse.json({ error: err?.message || "오류가 발생했습니다." }, { status: 500 });
  }
}

export const POST = withCoin("bizskyblouse", 0, handler); // COIN_DORMANT: 실가격 9
