...
- Clothing: a well-tailored BEIGE or CAMEL (warm light tan) men's suit jacket over a crisp white dress shirt, with a neat necktie in a soft coordinating tone (brown, navy, or muted blue), properly knotted. Keep the light suit clearly distinct in tone and brightness from the gray studio background so they do NOT merge. Warm, approachable, refined — sharp and intentional.
...
async function generateOneBizPhoto(imageDataUrls: string[], pose: string): Promise<string> {
  ...
  clearTimeout(timer);
  console.log(`[biz-man-beige] status=${res.status} ${Date.now() - t0}ms`);

  if (!res.ok) throw new Error(await geminiFriendlyError(res, "biz-man-beige"));
...
async function handler(request: NextRequest) {
  try {
    ...
    console.error("biz-man-beige error:", err?.message);
    return NextResponse.json({ error: err?.message || "오류가 발생했습니다." }, { status: 500 });
  }
}

export const POST = withCoin("bizmbeige", 0, handler); // COIN_DORMANT: 실가격 9
