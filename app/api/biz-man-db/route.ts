...
- Clothing: a well-tailored NAVY DOUBLE-BREASTED men's suit jacket (with the double-breasted button front clearly visible) over a crisp white dress shirt with a neat necktie (navy or burgundy), properly knotted. Fashionable, confident, strong silhouette — sharp and intentional.
...
async function generateOneBizPhoto(imageDataUrls: string[], pose: string): Promise<string> {
  ...
  clearTimeout(timer);
  console.log(`[biz-man-db] status=${res.status} ${Date.now() - t0}ms`);

  if (!res.ok) throw new Error(await geminiFriendlyError(res, "biz-man-db"));
...
async function handler(request: NextRequest) {
  try {
    ...
    console.error("biz-man-db error:", err?.message);
    return NextResponse.json({ error: err?.message || "오류가 발생했습니다." }, { status: 500 });
  }
}

export const POST = withCoin("bizmdb", 0, handler); // COIN_DORMANT: 실가격 9
