...
// 여성 비즈프로필 (네이비 블라우스 + 베이지 스커트 + 밝은 회색 배경). {POSE} 자리에 포즈별 지시.
...
STYLING (women's business profile — navy blouse + beige skirt / bright light gray background)
- Clothing: an elegant, well-fitted NAVY blouse — a refined long-sleeved blouse with a clean,
  feminine silhouette — neatly tucked into a well-fitted BEIGE (warm sand) pencil skirt suited to
  the framing. A classic, tidy, put-together combination. NO blazer, NO outer layer. The blouse is
  smooth and crisp, freshly pressed, no messy wrinkles.
...
async function generateOneBizPhoto(imageDataUrls: string[], pose: string): Promise<string> {
  ...
  clearTimeout(timer);
  console.log(`[biz-navyblouse-gray] status=${res.status} ${Date.now() - t0}ms`);

  if (!res.ok) throw new Error(await geminiFriendlyError(res, "biz-navyblouse-gray"));
...
async function handler(request: NextRequest) {
  try {
    ...
    console.error("biz-navyblouse-gray error:", err?.message);
    return NextResponse.json({ error: err?.message || "오류가 발생했습니다." }, { status: 500 });
  }
}

export const POST = withCoin("biznavyblouse", 0, handler); // COIN_DORMANT: 실가격 9
