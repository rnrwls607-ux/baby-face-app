...
async function generateOneBizPhoto(imageDataUrls: string[], pose: string): Promise<string> {
  ...
  clearTimeout(timer);
  console.log(`[biz-man-lightgray] status=${res.status} ${Date.now() - t0}ms`);

  if (!res.ok) throw new Error(await geminiFriendlyError(res, "biz-man-lightgray"));
...
async function handler(request: NextRequest) {
  try {
    ...
    console.error("biz-man-lightgray error:", err?.message);
    return NextResponse.json({ error: err?.message || "오류가 발생했습니다." }, { status: 500 });
  }
}

export const POST = withCoin("bizmlightgray", 0, handler); // COIN_DORMANT: 실가격 9
