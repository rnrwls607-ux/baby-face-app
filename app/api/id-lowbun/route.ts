...
- Hair: restyle into a sleek, low bun in a dark-brown color — hair pulled back smoothly and gathered into a clean low chignon at the nape, no flyaways, forehead and ears visible. The hairline and bun outline must be clean and smooth. (This overrides the source hairstyle, but NOT the face.)
- Clothing: a minimal black turtleneck knit, clean and chic.
- Background: solid, even, soft light rose-beige tone (bright and clean, not dull or muddy), smooth and even, no shadows, no props.

STYLING
- Expression: a gentle, natural, closed-lip smile, refined and confident. Eyes open, relaxed, on camera.
...
async function generateOneIdPhoto(imageDataUrls: string[]): Promise<string> {
  ...
  clearTimeout(timer);
  console.log(`[id-lowbun] model=${GEMINI_MODEL} status=${res.status} ${Date.now() - t0}ms`);

  if (!res.ok) throw new Error(await geminiFriendlyError(res, "id-lowbun"));
...
async function handler(request: NextRequest) {
  try {
    ...
    console.error("id-lowbun error:", err?.message);
    return NextResponse.json({ error: err?.message || "오류가 발생했습니다." }, { status: 500 });
  }
}

export const POST = withCoin("idlowbun", 0, handler); // COIN_DORMANT: 실가격 9
