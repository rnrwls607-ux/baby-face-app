...
// 증명사진 프롬프트 (블랙 정장 + 넥타이 + 밝은 회색 배경 / 남녀공용)
...
- Clothing: a sharp formal suit jacket in TRUE DEEP BLACK (pure black fabric — NOT navy, NOT dark blue, NOT charcoal) over a clean white collared shirt, with a neat dark necktie.
...
async function generateOneIdPhoto(imageDataUrls: string[]): Promise<string> {
  ...
  clearTimeout(timer);
  console.log(`[id-blacktie-gray] model=${GEMINI_MODEL} status=${res.status} ${Date.now() - t0}ms`);

  if (!res.ok) throw new Error(await geminiFriendlyError(res, "id-blacktie-gray"));
...
async function handler(request: NextRequest) {
  try {
    ...
    console.error("id-blacktie-gray error:", err?.message);
    return NextResponse.json({ error: err?.message || "오류가 발생했습니다." }, { status: 500 });
  }
}

export const POST = withCoin("idblacktie", 0, handler); // COIN_DORMANT: 실가격 9
