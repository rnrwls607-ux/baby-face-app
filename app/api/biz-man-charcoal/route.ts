...
- Clothing: a well-tailored CHARCOAL GRAY men's business suit jacket over a crisp white or very light blue dress shirt, with a neat, tasteful necktie (deep burgundy, navy, or silver-gray), properly knotted and centered. Crisp, fitted, modern corporate style — sharp and intentional.
...
async function generateOneBizPhoto(imageDataUrls: string[], pose: string): Promise<string> {
  ...
  clearTimeout(timer);
  console.log(`[biz-man-charcoal] status=${res.status} ${Date.now() - t0}ms`);

  if (!res.ok) throw new Error(await geminiFriendlyError(res, "biz-man-charcoal"));
...
async function handler(request: NextRequest) {
  try {
    ...
    console.error("biz-man-charcoal error:", err?.message);
    return NextResponse.json({ error: err?.message || "오류가 발생했습니다." }, { status: 500 });
  }
}

export const POST = withCoin("bizmcharcoal", 0, handler); // COIN_DORMANT: 실가격 9
