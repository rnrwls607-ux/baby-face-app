...
// 여성 비즈프로필 (화이트 셔츠 + 블루 스커트 + 밝은 회색 배경). {POSE} 자리에 포즈별 지시.
...
STYLING (women's business profile — white shirt + blue skirt / bright light gray background)
- Clothing: a crisp WHITE shirt/blouse with a clean collar and softly rolled sleeves, neatly
  tucked into a well-fitted dusty BLUE (soft cornflower blue) pencil skirt suited to the framing.
  A clean, fresh, professional combination. NO blazer, NO outer layer. The shirt is freshly pressed
  and smooth, wrinkle-free, bright and clean.
...
async function generateOneBizPhoto(imageDataUrls: string[], pose: string): Promise<string> {
  ...
  clearTimeout(timer);
  console.log(`[biz-blueskirt-gray] status=${res.status} ${Date.now() - t0}ms`);

  if (!res.ok) throw new Error(await geminiFriendlyError(res, "biz-blueskirt-gray"));
...
async function handler(request: NextRequest) {
  try {
    ...
    console.error("biz-blueskirt-gray error:", err?.message);
    return NextResponse.json({ error: err?.message || "오류가 발생했습니다." }, { status: 500 });
  }
}

export const POST = withCoin("bizblueskirt", 0, handler); // COIN_DORMANT: 실가격 9
