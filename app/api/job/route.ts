...
const JOB_PROMPTS: Record<string, string> = {
  pilot: `${JOB_CORE}\n\nTHE SCENE — 직업 변신: 파일럿 (airline captain):
- A bright modern airport: a softly blurred jet bridge or terminal window with an aircraft glowing in clean daylight behind them — polished, aspirational, cinematic.
- Key light: bright, clean, professional daylight — the face crisp and luminous.

WARDROBE — airline captain:
- An immaculate captain's uniform: a tailored dark navy jacket with gold sleeve stripes, a crisp white shirt and tie, and a captain's hat either worn tilted back so the ENTIRE face stays fully lit and visible, or carried smartly under one arm. NO readable name tags, airline logos, or text anywhere on the uniform.

POSE:
- A confident, trustworthy captain's pose: standing tall with a composed warm smile, or a poised slight turn with the hat under one arm.\n\n${JOB_FINISH}`,
  doctor: `${JOB_CORE}\n\nTHE SCENE — 직업 변신: 의사 (doctor):
- A bright, modern hospital: a clean softly-blurred corridor or consultation room in airy white-and-mint tones with large windows of soft daylight — trustworthy and premium.
- Key light: bright, clean, even daylight — the face fresh and luminous.

WARDROBE — doctor:
- A crisp white doctor's coat over neat professional attire, a stethoscope draped naturally around the neck. NO readable name tags, hospital logos, or text anywhere.

POSE:
- A warm, capable doctor's pose: arms lightly crossed with a reassuring smile, or standing composed with the stethoscope, looking toward the camera with kind, confident eyes.\n\n${JOB_FINISH}`,
  ceo: `${JOB_CORE}\n\nTHE SCENE — 직업 변신: CEO (successful executive):
- A stunning high-rise executive office: floor-to-ceiling windows with a softly blurred city skyline in golden late-afternoon light, sleek modern furniture — success made visible.
- Key light: warm directional window light sculpting the face brightly, with a soft interior fill — luminous and powerful.

WARDROBE — top executive:
- Woman: a sharply tailored premium suit or an elegant executive dress in a refined tone, with minimal fine jewelry.
- Man: a perfectly tailored premium suit in charcoal or navy with a crisp shirt — details understated and expensive.

POSE:
- A powerful, composed pose: standing by the window with the skyline behind, arms lightly crossed or one hand in a pocket, a confident assured expression.\n\n${JOB_FINISH}`,
};
async function generateJob(imageDataUrl: string, option: string): Promise<string> {
  const img = parseImage(imageDataUrl);
  const prompt = JOB_PROMPTS[option] || JOB_PROMPTS.pilot;
  ...
  clearTimeout(timer);
  console.log(`[job] model=${GEMINI_MODEL} option=${option} status=${res.status} ${Date.now() - t0}ms`);
  if (!res.ok) throw new Error(await geminiFriendlyError(res, "job"));
...
async function handler(request: NextRequest) {
  try {
    ...
    const option: string = typeof body?.option === "string" ? body.option : "pilot";
    if (!image) return NextResponse.json({ error: "사진을 올려주세요." }, { status: 400 });
    const output = await generateJob(image, option);
...
export const POST = withCoin("job", 0, handler); // COIN_DORMANT: 실가격 3
