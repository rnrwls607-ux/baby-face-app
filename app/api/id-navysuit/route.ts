...
// 가르마 네이비수트 (남성 전용 / 라이트 웜그레이 배경, 어두운 정장 대비 밝게)
...
- Hair: restyle into a neat side-parted men's cut with light natural volume on top, tidy around the ears and forehead, groomed and professional, natural black hair. The hair outline must be clean and smooth where it meets the background. (This overrides the source hairstyle, but NOT the face.)
- Clothing: a sharp navy formal suit jacket over a clean white collared shirt with a neat tie.
- Background: solid, even, light warm gray (clearly lighter than the navy suit and dark hair, so the person stands out crisply against it), smooth and clean, no shadows, no props.
...
async function generateOneIdPhoto(imageDataUrls: string[]): Promise<string> {
  ...
  clearTimeout(timer);
  console.log(`[id-navysuit] model=${GEMINI_MODEL} status=${res.status} ${Date.now() - t0}ms`);

  if (!res.ok) throw new Error(await geminiFriendlyError(res, "id-navysuit"));
...
async function handler(request: NextRequest) {
  try {
    ...
    console.error("id-navysuit error:", err?.message);
    return NextResponse.json({ error: err?.message || "오류가 발생했습니다." }, { status: 500 });
  }
}

export const POST = withCoin("idnavysuit", 0, handler); // COIN_DORMANT: 실가격 9
