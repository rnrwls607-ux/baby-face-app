...
// 여성 비즈프로필 (핑크 트위드 원피스 + 밝은 회색 배경). {POSE} 자리에 포즈별 지시.
...
STYLING (women's business profile — pink tweed dress / bright light gray background)
- Clothing: an elegant, well-fitted soft PINK TWEED dress (one-piece) in a refined Chanel-style
  bouclé tweed — structured, graceful, and sophisticated, with a clean neckline and three-quarter
  or long sleeves, knee-length. Polished, feminine, and premium; the tweed texture is subtle and
  high-end. The dress looks smooth and intentional, no messy wrinkles. NO jacket needed.
...
async function generateOneBizPhoto(imageDataUrls: string[], pose: string): Promise<string> {
  ...
  clearTimeout(timer);
  console.log(`[biz-pinktweed-gray] status=${res.status} ${Date.now() - t0}ms`);

  if (!res.ok) throw new Error(await geminiFriendlyError(res, "biz-pinktweed-gray"));
...
async function handler(request: NextRequest) {
  try {
    ...
    console.error("biz-pinktweed-gray error:", err?.message);
    return NextResponse.json({ error: err?.message || "오류가 발생했습니다." }, { status: 500 });
  }
}

export const POST = withCoin("bizpinktweed", 0, handler); // COIN_DORMANT: 실가격 9
