...
- Hair: restyle into a neat, elegant UPDO — hair smoothly pulled up and back into a clean bun/chignon, no loose flyaways, forehead and ears visible, refined and professional. (This overrides the source hairstyle, but NOT the face.)
- Clothing: a clean ivory blouse with a smooth collar or soft neckline, elegant and professional.
- Background: solid pale mint (soft light green) tone, smooth and even, no shadows, no props.
...
async function generateOneIdPhoto(imageDataUrls: string[]): Promise<string> {
  ...
  clearTimeout(timer);
  console.log(`[id-updo] model=${GEMINI_MODEL} status=${res.status} ${Date.now() - t0}ms`);

  if (!res.ok) throw new Error(await geminiFriendlyError(res, "id-updo"));
...
async function handler(request: NextRequest) {
  try {
    ...
    console.error("id-updo error:", err?.message);
    return NextResponse.json({ error: err?.message || "오류가 발생했습니다." }, { status: 500 });
  }
}

export const POST = withCoin("idupdo", 0, handler); // COIN_DORMANT: 실가격 9
