...
// 트렌치코트 가을 화보 — person:pro (cheerglam 템플릿 복제, new-concept.mjs 생성)
...
async function generateTrenchlook(imageDataUrl: string): Promise<string> {
...
autumn trench-coat street pictorial of them in the scene described below.
...
- Hair: a trendy modern Korean hairstyle that suits them, in their true
  ...
  naturally if they wear them, and moving naturally as they walk).
  Never a dated style that ages them.
...
THE SCENE — autumn trench-coat street pictorial:
- The person is captured mid-walk on a city street strewn with fallen
...
- Wardrobe: a long trench coat that drapes beautifully, worn open over a
  ...
  and bright (nothing covering the forehead, eyebrows, or eyes; the
  hairline showing naturally). No logos, no lettering.
- Setting: a softly blurred autumn city street — leaf-strewn pavement,
  bare-branched street trees, shop fronts melting into bokeh; lived-in
  urban textures, like a real street caught in passing, never a rendered
  backdrop. No readable text on any sign, shop, or surface.
POSE — a candid walking moment: caught mid-stride and turning toward the
camera, or glancing back over the shoulder, coat moving naturally.
...
- A yellow, orange, or sallow-looking face tinted by the street light.
- A stiff, planted, studio-style stance — this must read as a walking moment.
...
autumn trench-coat street pictorial, at the absolute best of their life.
High resolution, no text, no watermark, no border.`;
  ...
  try {
    ...
    if ((e as { name?: string })?.name === "AbortError") {
      console.error(`[TIMEOUT][trenchlook] 230초 무응답 ${Date.now() - t0}ms`);
      throw new Error("이미지 생성이 230초를 넘겨 중단했어요. 다시 시도해주세요.");
    }
    throw e;
  }
  clearTimeout(timer);
  console.log(`[trenchlook] model=${GEMINI_MODEL} status=${res.status} ${Date.now() - t0}ms`);
  if (!res.ok) throw new Error(await geminiFriendlyError(res, "trenchlook", "생성에 실패했어요. 다른 사진으로 다시 시도해주세요."));
  ...
  console.log(`[trenchlook] finish=${cand?.finishReason || "-"} block=${data?.promptFeedback?.blockReason || "-"} parts=${respParts.length} img=${imgParts.length} ${Date.now() - t0}ms`);
  const chosen = (finalParts.length ? finalParts : imgParts).pop();
  const b64 = chosen?.inlineData?.data || chosen?.inline_data?.data;
  if (!b64) {
    const txt = respParts.find((p: { text?: string }) => p.text)?.text;
    console.error(`[trenchlook] 이미지 없음 — finish=${cand?.finishReason || "-"} text=${(txt || "").slice(0, 500)}`);
...
async function handler(request: NextRequest) {
  try {
    ...
    const output = await generateTrenchlook(image);
    return NextResponse.json({ output: [output] });
  } catch (e: unknown) {
    const err = e as { message?: string };
    console.error("trenchlook error:", err?.message);
    return NextResponse.json({ error: err?.message || "오류가 발생했습니다." }, { status: 500 });
  }
}

export const POST = withCoin("trenchlook", 0, handler); // COIN_DORMANT: 실가격 3 · gemini-3-pro-image
