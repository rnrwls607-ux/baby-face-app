...
SELF-CHECK before finishing: zero new moles, freckles, or painted marks anywhere? · every original mole still in place? · glasses exactly as the original (or still absent)? · double eyelids and face structure untouched? · same person at a glance, despite the updo and uniform? · hairline natural with the new updo? · same pose, same hands with no props added, same background? · uniform free of logos, wing pins, name tags, and lettering, and not matching any real airline? · does it read "cabin crew" instantly? Only then is the work complete.

Output: one photorealistic photo, identical in pose and background to the input — the same person in complete cabin-crew styling. High resolution, no text, no watermark, no border — and absolutely zero new moles or marks anywhere: default skin is clear.`;
