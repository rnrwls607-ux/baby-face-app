...
// 여성 비즈프로필 (베이지 정장 + 밝은 회색 배경). {POSE} 자리에 포즈별 지시.
...
STYLING (women's business profile — beige suit / bright light gray background)
- Clothing: a well-tailored BEIGE (warm taupe/sand) women's business suit — a beige blazer over
  a clean white blouse or shell, paired with matching beige trousers or a beige pencil skirt
  suited to the framing. Crisp, fitted, modern corporate style in a warm, soft, premium tone;
  the outfit looks sharp, elegant, and intentional. Freshly pressed and smooth, no messy wrinkles.
...
async function generateOneBizPhoto(imageDataUrls: string[], pose: string): Promise<string> {
  ...
  clearTimeout(timer);
  console.log(`[biz-beige-gray] status=${res.status} ${Date.now() - t0}ms`);

  if (!res.ok) throw new Error(await geminiFriendlyError(res, "biz-beige-gray"));
...
async function handler(request: NextRequest) {
  try {
    ...
    console.error("biz-beige-gray error:", err?.message);
    return NextResponse.json({ error: err?.message || "오류가 발생했습니다." }, { status: 500 });
  }
}

export const POST = withCoin("bizbeige", 0, handler); // COIN_DORMANT: 실가격 9
