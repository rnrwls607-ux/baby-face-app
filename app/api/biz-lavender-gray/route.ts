...
// 여성 비즈프로필 (라벤더 파스텔 정장 + 밝은 회색 배경). {POSE} 자리에 포즈별 지시.
...
STYLING (women's business profile — pastel lavender suit / bright light gray background)
- Clothing: a well-tailored soft PASTEL LAVENDER women's business suit — a light lavender blazer
  over a clean white blouse or shell, paired with matching lavender trousers or a lavender pencil
  skirt suited to the framing. Fitted, modern, and elegant in a fresh, gentle pastel tone that
  feels bright and approachable yet professional. Freshly pressed and smooth, no messy wrinkles.
...
async function generateOneBizPhoto(imageDataUrls: string[], pose: string): Promise<string> {
  ...
  clearTimeout(timer);
  console.log(`[biz-lavender-gray] status=${res.status} ${Date.now() - t0}ms`);

  if (!res.ok) throw new Error(await geminiFriendlyError(res, "biz-lavender-gray"));
...
async function handler(request: NextRequest) {
  try {
    ...
    console.error("biz-lavender-gray error:", err?.message);
    return NextResponse.json({ error: err?.message || "오류가 발생했습니다." }, { status: 500 });
  }
}

export const POST = withCoin("bizlavender", 0, handler); // COIN_DORMANT: 실가격 9
