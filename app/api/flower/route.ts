...
const SCENE = `THE SCENE — 플라워 스튜디오 화보 (flower-filled studio):
- A breathtaking flower studio set: lush fresh blooms — roses, tulips, baby's breath — in soft pastel and cream tones filling the edges of the frame and softly blurred in the foreground, an airy light backdrop glowing behind. Dreamy, romantic, luminous. The flowers frame the person but NEVER cover or shadow the face.
- Key light: bright, soft, diffused studio light like a gentle morning glow — the face radiant and dewy among the blooms.

WARDROBE — romantic and luminous:
- Woman: a romantic soft dress in white, cream, or a delicate pastel that harmonizes with the flowers; optionally holding a small elegant bouquet below the chest line.
- Man: a soft clean look — a light knit or an open-collar shirt in white or a warm light tone — gentle and refined.

POSE:
- A soft, dreamy pose: gazing at the camera with a gentle smile among the flowers, lightly holding the bouquet, or a serene slight turn with eyes softly forward.`;
...
async function generateFlower(imageDataUrl: string): Promise<string> {
  ...
  clearTimeout(timer);
  console.log(`[flower] model=${GEMINI_MODEL} status=${res.status} ${Date.now() - t0}ms`);
  if (!res.ok) throw new Error(await geminiFriendlyError(res, "flower"));
...
async function handler(request: NextRequest) {
  try {
    ...
    const output = await generateFlower(image);
...
export const POST = withCoin("flower", 0, handler); // COIN_DORMANT: 실가격 3
