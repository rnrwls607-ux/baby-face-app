...
- Clothing: a well-tailored NAVY or GRAY sport jacket over a light OXFORD dress shirt (white or light blue) with a textured KNIT tie (navy, burgundy, or forest), neatly knotted. Semi-formal, youthful-professional, approachable — clean and intentional.
...
async function generateOneBizPhoto(imageDataUrls: string[], pose: string): Promise<string> {
  ...
  clearTimeout(timer);
  console.log(`[biz-man-knittie] status=${res.status} ${Date.now() - t0}ms`);

  if (!res.ok) throw new Error(await geminiFriendlyError(res, "biz-man-knittie"));
...
async function handler(request: NextRequest) {
  try {
    ...
    console.error("biz-man-knittie error:", err?.message);
    return NextResponse.json({ error: err?.message || "오류가 발생했습니다." }, { status: 500 });
  }
}

export const POST = withCoin("bizmknittie", 0, handler); // COIN_DORMANT: 실가격 9
