...
// 여성 비즈프로필 (카키 수트 + 밝은 회색 배경). {POSE} 자리에 포즈별 지시.
...
STYLING (women's business profile — khaki suit / bright light gray background)
- Clothing: a well-tailored KHAKI / soft OLIVE women's business suit — a khaki blazer over a
  clean white blouse or shell, paired with matching khaki trousers or a khaki pencil skirt suited
  to the framing. Crisp, fitted, modern corporate style in a warm, earthy, premium tone; sharp,
  elegant, and intentional. Freshly pressed and smooth, no messy wrinkles.
...
async function generateOneBizPhoto(imageDataUrls: string[], pose: string): Promise<string> {
  ...
  clearTimeout(timer);
  console.log(`[biz-khaki-gray] status=${res.status} ${Date.now() - t0}ms`);

  if (!res.ok) throw new Error(await geminiFriendlyError(res, "biz-khaki-gray"));
...
async function handler(request: NextRequest) {
  try {
    ...
    console.error("biz-khaki-gray error:", err?.message);
    return NextResponse.json({ error: err?.message || "오류가 발생했습니다." }, { status: 500 });
  }
}

export const POST = withCoin("bizkhaki", 0, handler); // COIN_DORMANT: 실가격 9
