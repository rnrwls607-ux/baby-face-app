...
// 비즈프로필 공통 프롬프트 (네이비 정장 + 회색 배경). {POSE} 자리에 포즈별 지시가 들어감.
...
STYLING (business profile — navy suit / gray background)
- Clothing: a well-tailored NAVY business suit. For women: a navy blazer over a clean white
  blouse or shirt (skirt or trousers as needed for the framing). For men: a navy suit jacket
  over a white dress shirt with a neat dark tie. Crisp, fitted, modern corporate style — the
  outfit should look sharp and intentional, a key part of the photo.
...
// 사진 여러 장 + 특정 포즈로 비즈프로필 1장 생성 (독립 1회 호출)
async function generateOneBizPhoto(imageDataUrls: string[], pose: string): Promise<string> {
  ...
  clearTimeout(timer);
  console.log(`[biz-navy-gray] status=${res.status} ${Date.now() - t0}ms`);

  if (!res.ok) throw new Error(await geminiFriendlyError(res, "biz-navy-gray"));
...
async function handler(request: NextRequest) {
  try {
    ...
    // ★독립 3회 동시 호출 — 각 호출에 서로 다른 포즈(팔짱/손모으기/3-4각도)
    ...
    console.error("biz-navy-gray error:", err?.message);
    return NextResponse.json({ error: err?.message || "오류가 발생했습니다." }, { status: 500 });
  }
}

export const POST = withCoin("biznavy", 0, handler); // COIN_DORMANT: 실가격 9
