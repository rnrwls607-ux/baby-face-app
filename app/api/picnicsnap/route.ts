...
// 한강 피크닉 스냅 — person:pro (cheerglam 템플릿 복제, new-concept.mjs 생성)
...
async function generatePicnicsnap(imageDataUrl: string): Promise<string> {
...
riverside picnic snap portrait of them in the scene described below.
...
THE SCENE — bright riverside picnic snap:
- The person is captured on a sunny afternoon picnic by the river — a
  wide grassy riverbank, a picnic mat spread out, a wicker basket of
  fresh fruit beside them, a bicycle leaning nearby, the river and a
...
- Props: a simple picnic mat, a wicker basket with whole fresh fruit,
  ...
  no labels, no printed cups or boxes, no logos on the bicycle or bag,
  no signs, no banners, not even illegible text-like shapes anywhere
  in the frame.
- Wardrobe: a light, breezy casual look that suits the person — a linen
  shirt, a soft blouse, or a light knit in fresh clean tones, styled so
  the face stays fully open and bright (nothing covering the forehead,
  eyebrows, or eyes; the hairline showing naturally). Clean fit around
  the neckline. No logos, no lettering.
- Setting: a softly blurred green riverside park in clear daylight —
  real grass, a blue sky with a few soft clouds, the river and bridge
  and far skyline melting into bokeh, lived-in outdoor textures, like a
  real afternoon by the river, never a rendered backdrop. No readable
  text on any sign, surface, or object.
POSE — a natural picnic moment: sitting relaxed on the mat, looking
toward the camera with a bright open smile, or a candid laugh with one
hand resting near the basket.
...
- Clean bright daylight — no squinting, no harsh shadows, no blown
  highlights on the skin, and the face never tinted yellow or orange?
- ZERO readable characters anywhere — no labels, wrappers, packaging,
  signs, banners, or text-like shapes on any object?
...
- Harsh midday light — squinting, hard nose or brow shadows, or blown
  highlights on the face.
- ANY labels, wrappers, packaging print, logos, signs, banners, or
  text-like shapes — on food, cups, the bicycle, or anywhere in frame.
...
riverside picnic snap portrait, at the absolute best of their life.
High resolution, no text, no watermark, no border.`;
  ...
  try {
    ...
    if ((e as { name?: string })?.name === "AbortError") {
      console.error(`[TIMEOUT][picnicsnap] 230초 무응답 ${Date.now() - t0}ms`);
      throw new Error("이미지 생성이 230초를 넘겨 중단했어요. 다시 시도해주세요.");
    }
    throw e;
  }
  clearTimeout(timer);
  console.log(`[picnicsnap] model=${GEMINI_MODEL} status=${res.status} ${Date.now() - t0}ms`);
  if (!res.ok) throw new Error(await geminiFriendlyError(res, "picnicsnap", "생성에 실패했어요. 다른 사진으로 다시 시도해주세요."));
  ...
  console.log(`[picnicsnap] finish=${cand?.finishReason || "-"} block=${data?.promptFeedback?.blockReason || "-"} parts=${respParts.length} img=${imgParts.length} ${Date.now() - t0}ms`);
  const chosen = (finalParts.length ? finalParts : imgParts).pop();
  const b64 = chosen?.inlineData?.data || chosen?.inline_data?.data;
  if (!b64) {
    const txt = respParts.find((p: { text?: string }) => p.text)?.text;
    console.error(`[picnicsnap] 이미지 없음 — finish=${cand?.finishReason || "-"} text=${(txt || "").slice(0, 500)}`);
...
async function handler(request: NextRequest) {
  try {
    ...
    const output = await generatePicnicsnap(image);
    return NextResponse.json({ output: [output] });
  } catch (e: unknown) {
    const err = e as { message?: string };
    console.error("picnicsnap error:", err?.message);
    return NextResponse.json({ error: err?.message || "오류가 발생했습니다." }, { status: 500 });
  }
}

export const POST = withCoin("picnicsnap", 0, handler); // COIN_DORMANT: 실가격 3 · gemini-3-pro-image
