...
const SCENE = (ATTIRE: string) => `THE SCENE — 셀프웨딩 화보 (self-wedding pictorial):
- The mood: the couple's own dreamy wedding photoshoot, on their own terms — bright, luminous, the beginning of everything.
- WARDROBE: ${ATTIRE}
- WARMTH: radiant and in love — facing each other at a slight angle or side by side, a hand resting tenderly on an arm or hands gently joined over the bouquet; expressions glowing, genuinely in love, softly smiling or laughing lightly.
- BACKDROP: an airy, luminous premium wedding studio — soft white and cream tones, elegant sheer drapery, delicate floral arrangements and a whisper of golden light melting into dreamy soft-focus depth. Bright, romantic, expensive, timeless. All decorations completely TEXT-FREE.
- KEY LIGHT MOOD: bright, soft, romantic beauty light — both faces equally luminous.`;
...
async function generateSelfwedding(image1DataUrl: string, image2DataUrl: string, gender1: string, gender2: string): Promise<string> {
  ...
  try {
    ...
    if ((e as { name?: string })?.name === "AbortError") {
      console.error(`[TIMEOUT][selfwedding] 140초 무응답 ${Date.now() - t0}ms`);
      throw new Error("이미지 생성이 140초를 넘겨 중단했어요. 다시 시도해주세요.");
    }
    throw e;
  }
  clearTimeout(timer);
  console.log(`[selfwedding] model=${GEMINI_MODEL} g1=${gender1} g2=${gender2} status=${res.status} ${Date.now() - t0}ms`);
...
async function handler(request: NextRequest) {
  try {
    ...
    const output = await generateSelfwedding(image1, image2, gender1, gender2);
    return NextResponse.json({ output: [output] });
  } catch (e: unknown) {
    const err = e as { message?: string };
    console.error("selfwedding error:", err?.message);
    return NextResponse.json({ error: err?.message || "오류가 발생했습니다." }, { status: 500 });
  }
}

export const POST = withCoin("selfwedding", 0, handler); // COIN_DORMANT: 실가격 3
