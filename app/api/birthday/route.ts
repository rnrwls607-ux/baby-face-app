...
const SCENE = `THE SCENE — 생일 화보 (dreamy birthday party set):
- A dreamy birthday studio set: soft pastel and cream balloons floating around, a pretty cake with softly glowing candles on a side table, delicate flowers and warm fairy-light bokeh — sweet, festive, and airy. All decorations completely TEXT-FREE: no letter banners, no writing on the cake, no readable characters anywhere.
- Key light: a warm, soft, celebratory glow — a bright flattering key light on the face blended with the cozy candle and fairy-light warmth. The face stays bright and radiant.

WARDROBE — the birthday star:
- Woman: a lovely party dress in a soft pastel, cream, or white tone; optionally a dainty birthday crown headband or a small bouquet held — charming, never childish or costume-cheap.
- Man: a clean celebratory smart-casual look — a soft knit or a neat shirt in a light warm tone; optionally holding a small bouquet.

POSE:
- A joyful, radiant pose: a bright genuine smile toward the camera, softly holding the bouquet, or leaning in near the glowing candles with sparkling eyes.`;
...
async function generateBirthday(imageDataUrl: string): Promise<string> {
  ...
  clearTimeout(timer);
  console.log(`[birthday] model=${GEMINI_MODEL} status=${res.status} ${Date.now() - t0}ms`);
  if (!res.ok) throw new Error(await geminiFriendlyError(res, "birthday"));
...
async function handler(request: NextRequest) {
  try {
    ...
    const output = await generateBirthday(image);
...
export const POST = withCoin("birthday", 0, handler); // COIN_DORMANT: 실가격 3
