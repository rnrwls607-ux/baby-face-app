...
// 여성 비즈프로필 (셔링 블라우스 + 베이지 스커트 + 밝은 회색 배경). {POSE} 자리에 포즈별 지시.
...
STYLING (women's business profile — shirring blouse + beige skirt / bright light gray background)
- Clothing: an elegant, well-fitted IVORY blouse with soft shirring and gently puffed shoulders —
  a refined, feminine silhouette with delicate gathered detailing — neatly tucked into a well-fitted
  BEIGE (warm sand) pencil skirt suited to the framing. NO blazer, NO outer layer. The blouse looks
  smooth, fresh, and softly structured (soft natural drape, not messy creases), bright and clean.
...
async function generateOneBizPhoto(imageDataUrls: string[], pose: string): Promise<string> {
  ...
  clearTimeout(timer);
  console.log(`[biz-shirring-gray] status=${res.status} ${Date.now() - t0}ms`);

  if (!res.ok) throw new Error(await geminiFriendlyError(res, "biz-shirring-gray"));
...
async function handler(request: NextRequest) {
  try {
    ...
    console.error("biz-shirring-gray error:", err?.message);
    return NextResponse.json({ error: err?.message || "오류가 발생했습니다." }, { status: 500 });
  }
}

export const POST = withCoin("bizshirring", 0, handler); // COIN_DORMANT: 실가격 9
