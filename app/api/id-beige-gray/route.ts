...
// 증명사진 프롬프트 (베이지 정장 + 밝은 회색 배경 / 남녀공용)
...
- Clothing: a soft beige formal suit jacket over a clean white collared shirt.
...
async function generateOneIdPhoto(imageDataUrls: string[]): Promise<string> {
  ...
  clearTimeout(timer);
  console.log(`[id-beige-gray] model=${GEMINI_MODEL} status=${res.status} ${Date.now() - t0}ms`);

  if (!res.ok) throw new Error(await geminiFriendlyError(res, "id-beige-gray"));
...
async function handler(request: NextRequest) {
  try {
    ...
    console.error("id-beige-gray error:", err?.message);
    return NextResponse.json({ error: err?.message || "오류가 발생했습니다." }, { status: 500 });
  }
}

export const POST = withCoin("idbeige", 0, handler); // COIN_DORMANT: 실가격 9
