...
// 여성 비즈프로필 (카멜 니트 원피스 + 밝은 회색 배경). {POSE} 자리에 포즈별 지시.
...
STYLING (women's business profile — camel knit dress / bright light gray background)
- Clothing: an elegant, well-fitted CAMEL (warm tan) fine-knit one-piece dress with a soft,
  refined, feminine silhouette — long sleeves, a clean neckline, knee-length, in a smooth
  fine-gauge knit. A warm, cozy yet polished look. The knit dress looks soft and clean with fine
  even texture, smooth and neat, no messy wrinkles. NO jacket needed.
...
- Lighting: soft, even, flattering professional studio lighting (gentle key + fill), polished
  ...
  the light background. Keep the fine knit texture soft and even.
...
async function generateOneBizPhoto(imageDataUrls: string[], pose: string): Promise<string> {
  ...
  clearTimeout(timer);
  console.log(`[biz-knitdress-gray] status=${res.status} ${Date.now() - t0}ms`);

  if (!res.ok) throw new Error(await geminiFriendlyError(res, "biz-knitdress-gray"));
...
async function handler(request: NextRequest) {
  try {
    ...
    console.error("biz-knitdress-gray error:", err?.message);
    return NextResponse.json({ error: err?.message || "오류가 발생했습니다." }, { status: 500 });
  }
}

export const POST = withCoin("bizknitdress", 0, handler); // COIN_DORMANT: 실가격 9
