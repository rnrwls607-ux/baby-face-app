...
- Clothing: an elegant, well-fitted CREAM (soft ivory) one-piece dress with a soft ribbon/bow tie
  at the neckline and gently sheer or three-quarter sleeves — a refined, graceful, feminine
  silhouette conveying quiet elegance and trustworthiness. The dress looks smooth, fresh, and
  gracefully tied, bright and clean (soft natural drape, not messy creases). NO jacket needed.
...
async function generateOneBizPhoto(imageDataUrls: string[], pose: string): Promise<string> {
  ...
  clearTimeout(timer);
  console.log(`[biz-creamdress-gray] status=${res.status} ${Date.now() - t0}ms`);

  if (!res.ok) throw new Error(await geminiFriendlyError(res, "biz-creamdress-gray"));
...
async function handler(request: NextRequest) {
  try {
    ...
    console.error("biz-creamdress-gray error:", err?.message);
    return NextResponse.json({ error: err?.message || "오류가 발생했습니다." }, { status: 500 });
  }
}

export const POST = withCoin("bizcreamdress", 0, handler); // COIN_DORMANT: 실가격 9
