...
// 여성 비즈프로필 (니트 가디건 + 블라우스 + 밝은 회색 배경). {POSE} 자리에 포즈별 지시.
...
STYLING (women's business profile — knit cardigan + blouse / bright light gray background)
- Clothing: a soft, well-fitted light BEIGE or CREAM knit cardigan worn over a clean white blouse
  with a small neat collar, paired with well-fitted neutral (beige or black) trousers or a skirt
  suited to the framing. A gentle, feminine, smart-casual professional look — approachable and
  youthful yet tidy and competent. The knit looks soft and clean with fine texture, the blouse
  fresh and crisp; smooth and neat, no messy wrinkles.
...
async function generateOneBizPhoto(imageDataUrls: string[], pose: string): Promise<string> {
  ...
  clearTimeout(timer);
  console.log(`[biz-knit-gray] status=${res.status} ${Date.now() - t0}ms`);

  if (!res.ok) throw new Error(await geminiFriendlyError(res, "biz-knit-gray"));
...
async function handler(request: NextRequest) {
  try {
    ...
    console.error("biz-knit-gray error:", err?.message);
    return NextResponse.json({ error: err?.message || "오류가 발생했습니다." }, { status: 500 });
  }
}

export const POST = withCoin("bizknit", 0, handler); // COIN_DORMANT: 실가격 9
