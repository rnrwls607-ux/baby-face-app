...
TASK
You are RETOUCHING a real photograph of one real person into a single clean, casual profile-style photo. One to six photos of the SAME individual are provided. You may restyle hair and clothing to the concept below, but you must NOT redesign the face. This concept is for MEN only — always render a man.
...
- Hair: restyle into a clean, fresh neat short-to-medium men's cut with light natural texture on top, forehead lightly visible, clean around the ears, groomed and youthful, natural black hair. (This overrides the source hairstyle, but NOT the face.)
- Clothing: a plain clean white crew-neck t-shirt, simple and casual.
- Background: solid warm sand-beige tone, smooth and even, no shadows, no props.

STYLING
- Expression: a gentle, natural, closed-lip smile, easy and friendly. Eyes open, relaxed, on camera.
...
OUTPUT
- Vertical ID ratio (3.5:4.5), high-quality clean portrait, photorealistic, fresh and casual. Remember the two absolute rules: the SAME face under the new hair, inside the SAME fixed composition.`;
...
async function generateOneIdPhoto(imageDataUrls: string[]): Promise<string> {
  ...
  clearTimeout(timer);
  console.log(`[id-perm] model=${GEMINI_MODEL} status=${res.status} ${Date.now() - t0}ms`);

  if (!res.ok) throw new Error(await geminiFriendlyError(res, "id-perm"));
...
async function handler(request: NextRequest) {
  try {
    ...
    console.error("id-perm error:", err?.message);
    return NextResponse.json({ error: err?.message || "오류가 발생했습니다." }, { status: 500 });
  }
}

export const POST = withCoin("idperm", 0, handler); // COIN_DORMANT: 실가격 9
