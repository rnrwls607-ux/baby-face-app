...
// 배치 2-A — 교복 컨셉 스냅 (Pro 단일입력 route 구조 복제, 크롭 없음 = 원본 비율 유지)
...
async function generateSchoolsnap(imageDataUrl: string): Promise<string> {
...
school-uniform concept snap portrait of them in the scene described below.
...
THE SCENE — Korean school-uniform concept studio snap:
- The person is styled in the popular Korean "school uniform concept
  photo" fashion — the trendy adult photo-studio experience, playful and
  nostalgic, clearly a stylish concept shoot.
- Light: flawless beauty lighting on the person — a bright soft key
  ...
  while the scene around them glows with bright, airy daylight; a fresh,
  youthful, slightly nostalgic campus mood.
- Wardrobe: a beautifully fitted GENERIC school-uniform-style outfit
  that suits the person — for a woman, e.g. a neat blazer or cardigan
  over a white shirt with a ribbon or slim tie and a pleated skirt; for
  a man, a clean blazer over a white shirt with a slim tie and tailored
  trousers. Refined fabric, clean modern fit around the neckline and
  shoulders. ★The uniform is an ORIGINAL generic design: NO school
  emblem, NO crest, NO name tag, NO lettering or badge of any kind on
  the clothing.
- Setting: a sunlit campus walkway with soft greenery, or a tidy bright
  classroom by a large window — softly blurred behind the person. No
  readable text on any board, sign, or surface.
...
- Any real school's uniform design, emblem, crest, or name tag.
...
school-uniform concept snap portrait, at the absolute best of their
life. High resolution, no text, no watermark, no border.`;
  ...
  try {
    ...
    if ((e as { name?: string })?.name === "AbortError") {
      console.error(`[TIMEOUT][schoolsnap] 230초 무응답 ${Date.now() - t0}ms`);
      throw new Error("이미지 생성이 230초를 넘겨 중단했어요. 다시 시도해주세요.");
    }
    throw e;
  }
  clearTimeout(timer);
  console.log(`[schoolsnap] model=${GEMINI_MODEL} status=${res.status} ${Date.now() - t0}ms`);
  if (!res.ok) throw new Error(await geminiFriendlyError(res, "schoolsnap", "생성에 실패했어요. 다른 사진으로 다시 시도해주세요."));
  ...
  console.log(`[schoolsnap] finish=${cand?.finishReason || "-"} block=${data?.promptFeedback?.blockReason || "-"} parts=${respParts.length} img=${imgParts.length} ${Date.now() - t0}ms`);
  const chosen = (finalParts.length ? finalParts : imgParts).pop();
  const b64 = chosen?.inlineData?.data || chosen?.inline_data?.data;
  if (!b64) {
    const txt = respParts.find((p: { text?: string }) => p.text)?.text;
    console.error(`[schoolsnap] 이미지 없음 — finish=${cand?.finishReason || "-"} text=${(txt || "").slice(0, 500)}`);
...
async function handler(request: NextRequest) {
  try {
    ...
    const output = await generateSchoolsnap(image);
    return NextResponse.json({ output: [output] });
  } catch (e: unknown) {
    const err = e as { message?: string };
    console.error("schoolsnap error:", err?.message);
    return NextResponse.json({ error: err?.message || "오류가 발생했습니다." }, { status: 500 });
  }
}

export const POST = withCoin("schoolsnap", 0, handler); // COIN_DORMANT: 실가격 3 · gemini-3-pro-image
