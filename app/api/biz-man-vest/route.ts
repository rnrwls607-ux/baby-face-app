...
- Clothing: a well-tailored CHARCOAL GRAY three-piece men's suit — jacket over a matching buttoned WAISTCOAT (vest) and a crisp white dress shirt, with a neat necktie (burgundy or navy), properly knotted. The waistcoat is clearly visible. Executive, premium, sharp and intentional.
...
async function generateOneBizPhoto(imageDataUrls: string[], pose: string): Promise<string> {
  ...
  clearTimeout(timer);
  console.log(`[biz-man-vest] status=${res.status} ${Date.now() - t0}ms`);

  if (!res.ok) throw new Error(await geminiFriendlyError(res, "biz-man-vest"));
...
async function handler(request: NextRequest) {
  try {
    ...
    console.error("biz-man-vest error:", err?.message);
    return NextResponse.json({ error: err?.message || "오류가 발생했습니다." }, { status: 500 });
  }
}

export const POST = withCoin("bizmvest", 0, handler); // COIN_DORMANT: 실가격 9
