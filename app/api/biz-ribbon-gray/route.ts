...
// 여성 전용 비즈프로필 (아이보리 리본 타이 블라우스 + 블랙 스커트 + 밝은 회색 배경). {POSE} 자리에 포즈별 지시.
...
STYLING (women's business profile — ivory pussy-bow blouse + black skirt / bright light gray background)
- Clothing: an elegant, well-fitted IVORY (soft off-white) PUSSY-BOW BLOUSE — a long-sleeved
  blouse with a soft ribbon/bow tie at the neck, a refined and feminine professional signature
  look — neatly TUCKED IN, paired with a well-fitted BLACK pencil skirt (or tailored black
  bottoms suited to the framing). NO blazer, NO suit jacket, NO outer layer — just the blouse
  and black skirt, clean and put-together. The blouse must look freshly IRONED and SMOOTH —
  wrinkle-free, crisp and neat, with a clean pressed surface and a gracefully tied bow. Do NOT
  add creases, crinkles, or visible wrinkles in the fabric; keep it sleek and tidy.
...
- Lighting: soft, even, flattering professional studio lighting (gentle key + fill), polished
  ...
  keep the ivory blouse clearly readable and cleanly SEPARATED from it: give the blouse gentle
...
async function generateOneBizPhoto(imageDataUrls: string[], pose: string): Promise<string> {
  ...
  clearTimeout(timer);
  console.log(`[biz-ribbon-gray] status=${res.status} ${Date.now() - t0}ms`);

  if (!res.ok) throw new Error(await geminiFriendlyError(res, "biz-ribbon-gray"));
...
async function handler(request: NextRequest) {
  try {
    ...
    console.error("biz-ribbon-gray error:", err?.message);
    return NextResponse.json({ error: err?.message || "오류가 발생했습니다." }, { status: 500 });
  }
}

export const POST = withCoin("bizribbon", 0, handler); // COIN_DORMANT: 실가격 9
