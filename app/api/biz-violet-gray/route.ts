...
// 여성 비즈프로필 (화이트 셔츠 + 바이올렛 스커트 + 밝은 회색 배경). {POSE} 자리에 포즈별 지시.
...
STYLING (women's business profile — white shirt + violet skirt / bright light gray background)
- Clothing: a crisp WHITE shirt/blouse with a clean collar, neatly tucked into a well-fitted soft
  VIOLET (muted purple) pencil skirt suited to the framing. A clean, elegant, modern combination.
  NO blazer, NO outer layer. The shirt is freshly pressed and smooth, wrinkle-free, bright and clean.
...
async function generateOneBizPhoto(imageDataUrls: string[], pose: string): Promise<string> {
  ...
  clearTimeout(timer);
  console.log(`[biz-violet-gray] status=${res.status} ${Date.now() - t0}ms`);

  if (!res.ok) throw new Error(await geminiFriendlyError(res, "biz-violet-gray"));
...
async function handler(request: NextRequest) {
  try {
    ...
    console.error("biz-violet-gray error:", err?.message);
    return NextResponse.json({ error: err?.message || "오류가 발생했습니다." }, { status: 500 });
  }
}

export const POST = withCoin("bizviolet", 0, handler); // COIN_DORMANT: 실가격 9
