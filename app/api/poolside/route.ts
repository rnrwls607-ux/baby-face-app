...
// 배치 2-A — 풀사이드 호캉스 (Pro 단일입력 route 구조 복제, 크롭 없음 = 원본 비율 유지)
...
async function generatePoolside(imageDataUrl: string): Promise<string> {
...
poolside resort pictorial portrait of them in the scene described below.
...
THE SCENE — luxury poolside resort pictorial:
- The person is photographed at a beautiful resort infinity pool — the
  dreamy vacation pictorial: relaxed, sun-kissed, effortlessly
  glamorous.
- Light (the face comes FIRST): flawless beauty lighting on the person —
  ...
  idol-grade luminous, the skin fresh and glowing, every feature crisp —
  while the crisp resort daylight and sparkling turquoise water
  reflections stay in the BACKGROUND as atmosphere. ★The face must
  NEVER be washed out by glare, flattened by harsh sun, or tinted by
  the water — it stays the brightest, freshest, most flattering element
  in the frame; a breezy, sun-drenched vacation mood.
- Wardrobe: a tasteful resort look that suits the person — for a woman,
  e.g. an elegant summer dress, or a chic swimsuit with a light linen
  cover-up or shirt, tasteful and magazine-worthy, never revealing or
  vulgar; for a man, a relaxed open linen shirt over swim shorts or
  light resort wear. Clean fit around the neckline and shoulders. No
  logos, no lettering.
- Setting: an infinity pool edge or a stylish lounger with a straw hat
  or a glass of fruit juice nearby — softly blurred resort architecture,
  palm leaves, and sky behind; lived-in vacation textures, like a real
  five-star resort, never a rendered backdrop. No readable text on any
  towel, sign, or surface.
POSE — a relaxed vacation pose: leaning lightly at the pool edge or
seated gracefully on the lounger, a soft easy smile toward the camera.
...
- Face bright and clean-toned — never washed out by glare or sun?
...
- A washed-out or glare-flattened face.
- Revealing, suggestive, or vulgar styling — this is an elegant
  magazine pictorial.
...
poolside resort pictorial portrait, at the absolute best of their life.
High resolution, no text, no watermark, no border.`;
  ...
  try {
    ...
    if ((e as { name?: string })?.name === "AbortError") {
      console.error(`[TIMEOUT][poolside] 230초 무응답 ${Date.now() - t0}ms`);
      throw new Error("이미지 생성이 230초를 넘겨 중단했어요. 다시 시도해주세요.");
    }
    throw e;
  }
  clearTimeout(timer);
  console.log(`[poolside] model=${GEMINI_MODEL} status=${res.status} ${Date.now() - t0}ms`);
  if (!res.ok) throw new Error(await geminiFriendlyError(res, "poolside", "생성에 실패했어요. 다른 사진으로 다시 시도해주세요."));
  ...
  console.log(`[poolside] finish=${cand?.finishReason || "-"} block=${data?.promptFeedback?.blockReason || "-"} parts=${respParts.length} img=${imgParts.length} ${Date.now() - t0}ms`);
  const chosen = (finalParts.length ? finalParts : imgParts).pop();
  const b64 = chosen?.inlineData?.data || chosen?.inline_data?.data;
  if (!b64) {
    const txt = respParts.find((p: { text?: string }) => p.text)?.text;
    console.error(`[poolside] 이미지 없음 — finish=${cand?.finishReason || "-"} text=${(txt || "").slice(0, 500)}`);
...
async function handler(request: NextRequest) {
  try {
    ...
    const output = await generatePoolside(image);
    return NextResponse.json({ output: [output] });
  } catch (e: unknown) {
    const err = e as { message?: string };
    console.error("poolside error:", err?.message);
    return NextResponse.json({ error: err?.message || "오류가 발생했습니다." }, { status: 500 });
  }
}

export const POST = withCoin("poolside", 0, handler); // COIN_DORMANT: 실가격 3 · gemini-3-pro-image
