...
TASK
You are RETOUCHING a real photograph of one real person into a single clean, classic formal ID photo. One to six photos of the SAME individual are provided. You may restyle hair and clothing to the concept below, but you must NOT redesign the face. This concept is for MEN only — always render a man.
...
- Hair: restyle into a neat POMADE slicked-back style — hair combed back cleanly with a glossy finish, classic and polished, forehead visible, natural black hair. (This overrides the source hairstyle, but NOT the face.)
- Clothing: a classic vintage-toned suit — a warm brown or muted tweed jacket over a shirt with a retro patterned tie, giving a nostalgic 1990s formal-portrait feel.
- Background: solid deep sepia-brown tone with a soft vintage studio feel, smooth and even, no props.
...
OUTPUT
- Vertical ID ratio (3.5:4.5), classic studio portrait with a nostalgic tone, photorealistic. Remember the two absolute rules: the SAME face under the new hair, inside the SAME fixed composition.`;
...
async function generateOneIdPhoto(imageDataUrls: string[]): Promise<string> {
  ...
  clearTimeout(timer);
  console.log(`[id-pomade] model=${GEMINI_MODEL} status=${res.status} ${Date.now() - t0}ms`);

  if (!res.ok) throw new Error(await geminiFriendlyError(res, "id-pomade"));
...
async function handler(request: NextRequest) {
  try {
    ...
    console.error("id-pomade error:", err?.message);
    return NextResponse.json({ error: err?.message || "오류가 발생했습니다." }, { status: 500 });
  }
}

export const POST = withCoin("idpomade", 0, handler); // COIN_DORMANT: 실가격 9
