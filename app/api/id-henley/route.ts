...
// 투블럭 헨리넥 (남성 전용 / 라이트 세이지 그레이 배경, 검은 상의 대비)
...
TASK
You are RETOUCHING a real photograph of one real person into a single clean, modern profile-style ID photo. One to six photos of the SAME individual are provided. You may restyle hair and clothing to the concept below, but you must NOT redesign the face. This concept is for MEN only — always render a man.
...
- Hair: restyle into a clean short two-block men's cut — trimmed neatly around the sides with natural texture on top, groomed, natural black hair. The hair outline must be clean and smooth where it meets the background. (This overrides the source hairstyle, but NOT the face.)
- Clothing: a black henley top (with a short buttoned placket at the neck), simple and modern.
- Background: solid, even, light sage gray-green (a soft, muted, LIGHT tone — clearly lighter than the black top and hair, so the person stands out crisply against it), smooth and clean, no shadows, no props.

STYLING
- Expression: a gentle, natural, closed-lip smile, cool and composed. Eyes open, relaxed, on camera.
...
OUTPUT
- Vertical ID ratio (3.5:4.5), high-quality studio portrait, photorealistic, clean and modern. Remember the two absolute rules: the SAME face under the new hair, inside the SAME fixed composition.`;
...
async function generateOneIdPhoto(imageDataUrls: string[]): Promise<string> {
  ...
  clearTimeout(timer);
  console.log(`[id-henley] model=${GEMINI_MODEL} status=${res.status} ${Date.now() - t0}ms`);

  if (!res.ok) throw new Error(await geminiFriendlyError(res, "id-henley"));
...
async function handler(request: NextRequest) {
  try {
    ...
    console.error("id-henley error:", err?.message);
    return NextResponse.json({ error: err?.message || "오류가 발생했습니다." }, { status: 500 });
  }
}

export const POST = withCoin("idhenley", 0, handler); // COIN_DORMANT: 실가격 9
