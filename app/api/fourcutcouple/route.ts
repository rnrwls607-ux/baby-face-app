...
COMPOSITION — the four-cut strip (critical structure):
- The output is ONE vertical photo strip: FOUR stacked frames of equal size separated by a clean cream-white border, like a premium Korean photo booth print.
- ALL FOUR frames show the SAME two people together, chest-up, both faces large, fully visible, and clearly recognizable in EVERY frame.
- Each frame has a DIFFERENT pose and energy: (1) both smiling warmly at the camera, (2) cheek to cheek, (3) a playful pose — V-signs or finger hearts, (4) a candid burst of laughter. Natural hands, correct fingers, in every frame.
- IDENTITY ACROSS FRAMES: each person looks EXACTLY the same person in all four frames — same face, same hair, same outfit throughout the strip. Eight face instances total; every single one must pass the side-by-side test with its source.
- Inside each frame: a clean bright photo-booth backdrop in a soft warm tone; crisp, even booth lighting on both faces.
- The strip and its borders are completely TEXT-FREE: no logos, no dates, no booth branding, no letters anywhere.`;
const SCENE = `THE SCENE — 커플 네컷 (photo-booth strip):
- The mood: a fun, affectionate Korean photo-booth session — the strip couples pin on their wall.
- WARDROBE: trendy everyday couple looks in clean tones per each person's confirmed gender — casual, current, no logos or readable text.
- The booth structure and poses follow the COMPOSITION section above.`;
...
- STRIP CHECK: exactly FOUR frames? The same two people in every frame, identical across frames? No text, dates, or logos on the strip?
Only when every check passes is the portrait complete.

FRAMING:
- ONE tall vertical strip of four equal frames with clean cream borders, both faces tack-sharp in every frame. Photorealistic, high resolution.

ABSOLUTELY AVOID (equally important):
- A frame count other than four; different-looking people between frames; changed outfits or hair between frames; any text, date stamp, or booth logo on the strip.
...
async function generateFourcutcouple(image1DataUrl: string, image2DataUrl: string, gender1: string, gender2: string): Promise<string> {
  ...
  try {
    ...
    if ((e as { name?: string })?.name === "AbortError") {
      console.error(`[TIMEOUT][fourcutcouple] 140초 무응답 ${Date.now() - t0}ms`);
      throw new Error("이미지 생성이 140초를 넘겨 중단했어요. 다시 시도해주세요.");
    }
    throw e;
  }
  clearTimeout(timer);
  console.log(`[fourcutcouple] model=${GEMINI_MODEL} g1=${gender1} g2=${gender2} status=${res.status} ${Date.now() - t0}ms`);
...
async function handler(request: NextRequest) {
  try {
    ...
    const output = await generateFourcutcouple(image1, image2, gender1, gender2);
