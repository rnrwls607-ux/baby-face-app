...
export default function AgePage() {
  ...
  const COIN_GATED = LIVE_COIN_CONCEPTS.includes("age");
  const COIN_COST = CONCEPTS.age?.coinCost ?? 0;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      void addToHistory(data.output, mode === "baby" ? "베이비 변환" : "노년 변환", Array.isArray(data.originalUrls) ? data.originalUrls : undefined);
    } catch (e: unknown) {
      clearTimeout(tid);
        ...
        {!result && (
          <>
            ...
            <button onClick={handleSubmit} disabled={loading || !image}
              ...
              {loading ? `만드는 중... (${elapsed}초)` : mode === "baby" ? "아기 모습 보기 ✨" : <>노년 모습 보기 ✨{COIN_GATED && COIN_COST > 0 && <span style={{ fontSize: 13, fontWeight: 700, opacity: 0.9 }}> · <CoinIcon size={14} onColor /> {COIN_COST}</span>}</>}
            </button>
          </>
        )}
        {loading && (
