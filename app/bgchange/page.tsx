...
export default function BgchangePage() {
  ...
  const COIN_GATED = LIVE_COIN_CONCEPTS.includes("bgchange");
  const COIN_COST = CONCEPTS.bgchange?.coinCost ?? 0;
  ...
  const handleSubmit = async (pick?: string) => {
    ...
    try {
      ...
      void addToHistory(data.output, "배경 교체", Array.isArray(data.originalUrls) ? data.originalUrls : undefined);
    } catch (e: unknown) {
      clearTimeout(tid);
  ...
  return (
    <div style={{ maxWidth: 480, margin: "0 auto", minHeight: "100vh", background: "#fff", fontFamily: "var(--font-noto), 'Apple SD Gothic Neo', sans-serif" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "0 16px", height: 56, borderBottom: "1px solid #f0f0f0", position: "sticky", top: 0, background: "#fff", zIndex: 10 }}>
        ...
        <span style={{ fontSize: 16, fontWeight: 700, color: "#111" }}>배경 교체</span>
      </div>
      <div style={{ padding: "20px 20px 100px" }}>
        {!result && (
          <>
            {/* 결과 예시 — BA_LIVE면 비포/애프터 라이브, 아니면 기존 PreviewCard (무변화 폴백) */}
            {BA_LIVE.includes("bgchange") ? (
              <BeforeAfterHero pairs={[1, 2, 3].flatMap(n => [
                { before: `/examples/ba/bgchange-before-${n}.webp`, after: `/examples/ba/bgchange-after-${n}.webp` },
                { before: `/examples/ba/bgchange-before.webp`, after: `/examples/ba/bgchange-after-${n}.webp` },
              ])} />
            ) : (
        ...
        {result && (
          <div>
            ...
            <div style={{ background: "#fff", borderRadius: 20, padding: "18px 18px", boxShadow: "0 2px 16px rgba(0,0,0,0.04)", margin: "0 0 12px" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 8 }}>
                {BG_OPTIONS.map((o, i) => (
                  <button key={o.key} onClick={() => (COIN_GATED && COIN_COST > 0 ? setPendingPick(o.key) : void handleSubmit(o.key))} disabled={loading} style={{ ...chipStyle(bg === o.key), ...(i === BG_OPTIONS.length - 1 ? { gridColumn: "1 / -1" } : {}) }}>{o.label}</button>
                ))}
              </div>
            </div>
