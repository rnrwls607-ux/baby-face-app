...
export default function IdPhotoPage() {
  ...
  const handleShare = () => { void shareImage(results[selected], "id_photo.png", "MOSPIC에서 만든 사진이에요 · mospic.com"); };
  ...
  return (
    <div style={{ maxWidth: 480, margin: "0 auto", minHeight: "100vh", background: UI.pageBg, fontFamily: "var(--font-noto), 'Apple SD Gothic Neo', sans-serif" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "0 12px", height: 56, borderBottom: `1px solid ${UI.line}`, position: "sticky", top: 0, background: UI.surface, zIndex: 10 }}>
        <button onClick={() => { if (results.length) { setResults([]); return; } if (window.history.length > 1 + backCloseGhostCount()) router.back(); else router.push("/"); }} style={{ background: "none", border: "none", fontSize: 26, cursor: "pointer", color: UI.text, padding: "4px 8px", lineHeight: 1 }}>‹</button>
