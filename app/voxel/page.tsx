...
export default function VoxelPage() {
  ...
  const COIN_GATED = LIVE_COIN_CONCEPTS.includes("voxel");
  const COIN_COST = CONCEPTS.voxel?.coinCost ?? 0;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      void addToHistory(data.output, "복셀 아트", Array.isArray(data.originalUrls) ? data.originalUrls : undefined);
    } catch (e: unknown) {
      clearTimeout(tid);
  ...
  const handleDownload = () => { void saveImage(result, "voxel.png"); };
  const handleShare = () => { void shareImage(result, "voxel.png", "MOSPIC에서 만든 사진이에요 · mospic.com"); };
  return (
      ...
      <div style={{ padding: "20px 20px 100px" }}>
        {!result && (
          <>
            ...
            {BA_LIVE.includes("voxel") ? (
              <BeforeAfterHero pairs={[1, 2, 3].flatMap(n => [
                { before: `/examples/ba/voxel-before-${n}.webp`, after: `/examples/ba/voxel-after-${n}.webp` },
                { before: `/examples/ba/voxel-before.webp`, after: `/examples/ba/voxel-after-${n}.webp` },
              ])} />
            ) : (
            ...
            <button onClick={handleSubmit} disabled={loading || !image}
              ...
              {loading ? `만드는 중... (${elapsed}초)` : <>복셀 아트 만들기 ✨{COIN_GATED && COIN_COST > 0 && <span style={{ fontSize: 13, fontWeight: 700, opacity: 0.9 }}> · <CoinIcon size={14} onColor /> {COIN_COST}</span>}</>}
            </button>
          </>
        )}
        {loading && (
