...
export default function SelfweddingPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/selfwedding", {
        method: "POST",
      ...
      void addToHistory(data.output, "셀프웨딩 화보", Array.isArray(data.originalUrls) ? data.originalUrls : undefined);
    } catch (e: unknown) {
      clearTimeout(tid);
  ...
  const handleDownload = () => { void saveImage(result, "selfwedding.png"); };
  const handleShare = () => { void shareImage(result, "selfwedding.png", "MOSPIC에서 만든 사진이에요 · mospic.com"); };
    ...
    <div style={{ maxWidth: 480, margin: "0 auto", minHeight: "100vh", background: "#F7F8FA", fontFamily: "var(--font-noto), 'Apple SD Gothic Neo', sans-serif" }}>
      ...
      <div style={{ padding: "18px 18px 100px" }}>
        <div style={{ background: "#FFEAF1", borderRadius: 16, padding: "16px 18px", marginBottom: 22 }}>
          <p style={{ fontSize: 14, fontWeight: 800, color: "#FF4B7C", margin: "0 0 5px" }}>💍 우리끼리, 셀프웨딩</p>
          <p style={{ fontSize: 12.5, color: "#B36B85", margin: 0, lineHeight: 1.55 }}>두 사람의 사진을 한 장씩 올리면 스튜디오 셀프웨딩 화보를 만들어드려요. 드레스·턱시도까지 완벽하게.</p>
        </div>
        {!result && (
          <>
            {/* 결과 예시 — 각자 셀카 → 함께 화보 (2인 라인 전용 구성) */}
            {BA_LIVE.includes("selfwedding") ? (
              <BeforeAfterHero pairs={[1, 2, 3].flatMap(n => [
                { before: `/examples/ba/selfwedding-before-${n}.webp`, after: `/examples/ba/selfwedding-after-${n}.webp` },
                { before: `/examples/ba/selfwedding-before.webp`, after: `/examples/ba/selfwedding-after-${n}.webp` },
              ])} />
            ) : (
              <>
            ...
            <PrivacyLine />
            <ProCongestionHint concept="selfwedding" />
            <button onClick={handleSubmit} disabled={!canSubmit}
              ...
              {loading ? `만드는 중... (${elapsed}초)` : <>셀프웨딩 화보 만들기 ✨{COIN_GATED && COIN_COST > 0 && <span style={{ fontSize: 13, fontWeight: 700, opacity: 0.9 }}> · <CoinIcon size={14} onColor /> {COIN_COST}</span>}</>}
            </button>
          </>
        )}
        {loading && (
        ...
        {error && (
          <div style={{ background: "#FFEAF1", border: "1px solid #FF4B7C33", borderRadius: 12, padding: "13px 16px", marginTop: 16 }}>
            ...
            <ProCongestionError concept="selfwedding" error={error} />
        ...
        {result && (
          <div>
            ...
            <div style={{ borderRadius: 20, overflow: "hidden", marginBottom: 14, boxShadow: "0 4px 20px rgba(0,0,0,0.08)" }}>
              <img src={result} alt="셀프웨딩 화보" style={{ width: "100%", display: "block" }} />
            </div>
            <div style={{ display: "flex", gap: 10 }}>
