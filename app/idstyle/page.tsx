...
export default function IdStylePage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      void addToHistory(data.output, `증명사진 ${STYLE_LABEL}`, Array.isArray(data.originalUrls) ? data.originalUrls : undefined);
    } catch (e: unknown) {
      clearTimeout(tid);
  ...
  const handleDownload = () => { void saveImage(result, "idphoto.png"); };
  const handleShare = () => { void shareImage(result, "idphoto.png", "MOSPIC에서 만든 사진이에요 · mospic.com"); };
