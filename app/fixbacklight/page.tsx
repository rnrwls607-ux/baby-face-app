...
export default function FixbacklightPage() {
  const router = useRouter();
  // 코인 게이트 표시·가드 (coinCost는 표시 전용 — 요금의 진실원은 서버 withCoin) — ★스위치 날 벌크 앵커
  const COIN_GATED = LIVE_COIN_CONCEPTS.includes("fixbacklight");
  const COIN_COST = CONCEPTS.fixbacklight?.coinCost ?? 0;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/fixbacklight", {
        method: "POST",
      ...
      void addToHistory(data.output, "역광 구제", Array.isArray(data.originalUrls) ? data.originalUrls : undefined);
    } catch (e: unknown) {
      clearTimeout(tid);
  ...
  const handleDownload = () => { void saveImage(result, "fixbacklight.png"); };
  const handleShare = () => { void shareImage(result, "fixbacklight.png", "MOSPIC에서 만든 사진이에요 · mospic.com"); };
  return (
    <div style={{ maxWidth: 480, margin: "0 auto", minHeight: "100vh", background: "#fff", fontFamily: "var(--font-noto), 'Apple SD Gothic Neo', sans-serif" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "0 16px", height: 56, borderBottom: "1px solid #f0f0f0", position: "sticky", top: 0, background: "#fff", zIndex: 10 }}>
        ...
        <span style={{ fontSize: 16, fontWeight: 700, color: "#111" }}>역광 구제</span>
      </div>
      <div style={{ padding: "20px 20px 100px" }}>
        {!result && (
          <>
            {/* 결과 예시 — BA_LIVE면 비포/애프터 라이브, 아니면 기존 PreviewCard (무변화 폴백) */}
            {BA_LIVE.includes("fixbacklight") ? (
              <BeforeAfterHero pairs={[1, 2, 3].flatMap(n => [
                { before: `/examples/ba/fixbacklight-before-${n}.webp`, after: `/examples/ba/fixbacklight-after-${n}.webp` },
                { before: `/examples/ba/fixbacklight-before.webp`, after: `/examples/ba/fixbacklight-after-${n}.webp` },
              ])} />
            ) : (
              <PreviewCard placeholder="🔆" caption="역광 구제, 미리 만나보세요" />
            )}
            ...
            />
            <TipChips tips={[{ icon: "expand", label: "역광으로 어두운 사진" }, { icon: "sun", label: "얼굴이 보이기만 하면 OK" }, { icon: "eye", label: "흔들리지 않게" }]} />
            <PrivacyLine />
            <button onClick={handleSubmit} disabled={loading || !image}
              ...
              {loading ? `만드는 중... (${elapsed}초)` : <>역광 구제하기 🔆{COIN_GATED && COIN_COST > 0 && <span style={{ fontSize: 13, fontWeight: 700, opacity: 0.9 }}> · <CoinIcon size={14} onColor /> {COIN_COST}</span>}</>}
            </button>
          </>
        )}
        {loading && (
          <div style={{ marginTop: 24, textAlign: "center" }}>
            ...
            <p style={{ fontSize: 14, color: "#888", marginTop: 8 }}>AI가 얼굴에 빛을 되돌리고 있어요...</p>
            <LoadingSaveNote />
          </div>
        )}
        {error && (
        ...
        {result && (
          <div>
            ...
            <div style={{ borderRadius: 16, overflow: "hidden", marginBottom: 12 }}>
              <img src={result} alt="역광 구제" style={{ width: "100%", display: "block" }} />
            </div>
