...
export default function PettwoPage() {
  const router = useRouter();
  // 코인 게이트 표시·가드 (coinCost는 표시 전용 — 요금의 진실원은 서버 withCoin) — ★스위치 날 벌크 앵커
  const COIN_GATED = LIVE_COIN_CONCEPTS.includes("pettwo");
  const COIN_COST = CONCEPTS.pettwo?.coinCost ?? 0;
  ...
  const handleUpload = async (file: File, slot: 0 | 1) => { const b64 = await toBase64(file); if (slot === 0) setImage1(b64); else setImage2(b64); };
  const handleSubmit = async () => {
    if (!image1 || !image2) { setError("두 아이의 사진을 모두 올려주세요."); return; }
    ...
    try {
      ...
      const res = await fetch("/api/pettwo", {
        method: "POST",
      ...
      void addToHistory(data.output, "펫 둘이서", Array.isArray(data.originalUrls) ? data.originalUrls : undefined);
    } catch (e: unknown) {
      clearTimeout(tid);
  ...
  const handleDownload = () => { void saveImage(result, "pettwo.png"); };
  const handleShare = () => { void shareImage(result, "pettwo.png", "MOSPIC에서 만든 사진이에요 · mospic.com"); };
  return (
    <div style={{ maxWidth: 480, margin: "0 auto", minHeight: "100vh", background: "#F7F8FA", fontFamily: "var(--font-noto), 'Apple SD Gothic Neo', sans-serif" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "0 12px", height: 56, position: "sticky", top: 0, background: "#fff", zIndex: 10 }}>
        ...
        <span style={{ fontSize: 16, fontWeight: 800, color: "#191919" }}>펫 둘이서</span>
      </div>
      <div style={{ padding: "18px 18px 100px" }}>
        {!result && (
          <>
            {/* 결과 예시 — BA_LIVE면 비포/애프터 라이브, 아니면 기존 PreviewCard (무변화 폴백) */}
            {BA_LIVE.includes("pettwo") ? (
              <BeforeAfterHero pairs={[1, 2, 3].flatMap(n => [
                { before: `/examples/ba/pettwo-before-${n}.webp`, after: `/examples/ba/pettwo-after-${n}.webp` },
                { before: `/examples/ba/pettwo-before.webp`, after: `/examples/ba/pettwo-after-${n}.webp` },
              ])} />
            ) : (
              <PreviewCard placeholder="🐾" caption="펫 둘이서, 미리 만나보세요" />
            )}
            ...
            <UploadZone
              ...
              max={1}
              onPick={files => handleUpload(files[0], 0)}
              onRemove={() => setImage1("")}
            />
            <UploadZone
              label="둘째 아이"
              ...
              max={1}
              onPick={files => handleUpload(files[0], 1)}
              onRemove={() => setImage2("")}
            />
            <TipChips tips={[{ icon: "face", label: "얼굴 정면" }, { icon: "sun", label: "밝은 곳에서" }, { icon: "eye", label: "얼굴 또렷하게" }]} />
            <PrivacyLine />
            <ProCongestionHint concept="pettwo" />

            <button onClick={handleSubmit} disabled={loading || !image1 || !image2}
              style={{ width: "100%", marginTop: 18, background: loading || !image1 || !image2 ? "#E8E9ED" : "#FF4B7C", color: loading || !image1 || !image2 ? "#AEB2BA" : "#fff", border: "none", borderRadius: 16, padding: "16px 0", fontSize: 16, fontWeight: 800, cursor: loading || !image1 || !image2 ? "not-allowed" : "pointer", boxShadow: loading || !image1 || !image2 ? "none" : "0 6px 18px rgba(255,75,124,0.32)" }}>
              {loading ? `만드는 중... (${elapsed}초)` : <>둘이서 화보 만들기 ✨{COIN_GATED && COIN_COST > 0 && <span style={{ fontSize: 13, fontWeight: 700, opacity: 0.9 }}> · <CoinIcon size={14} onColor /> {COIN_COST}</span>}</>}
            </button>
          </>
        )}
        {loading && (
          <div style={{ marginTop: 28, textAlign: "center" }}>
            ...
            <p style={{ fontSize: 14, color: "#9B9B9B", marginTop: 10, fontWeight: 600 }}>AI가 두 아이를 한 장에 담고 있어요...</p>
            <LoadingSaveNote />
          </div>
        )}
        {error && (
          <div style={{ background: "#FFEAF1", border: "1px solid #FF4B7C33", borderRadius: 12, padding: "13px 16px", marginTop: 16 }}>
            ...
            <ProCongestionError concept="pettwo" error={error} />
        ...
        {result && (
          <div>
            ...
            <div style={{ borderRadius: 20, overflow: "hidden", marginBottom: 14, boxShadow: "0 4px 20px rgba(0,0,0,0.08)" }}>
              <img src={result} alt="펫 둘이서" style={{ width: "100%", display: "block" }} />
            </div>
            <div style={{ display: "flex", gap: 10 }}>
              ...
              <button onClick={() => { setResult(""); setImage1(""); setImage2(""); }}
