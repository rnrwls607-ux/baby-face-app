    ...
    <div style={{ maxWidth: 480, margin: "0 auto", minHeight: "100vh", background: "#F7F8FA", fontFamily: "var(--font-noto), 'Apple SD Gothic Neo', sans-serif" }}>
      ...
      <div style={{ padding: "18px 18px 40px" }}>
        <div style={{ background: "#fff", borderRadius: 16, padding: "22px 20px 26px" }}>
          ...
          <p style={st.p}>④ &quot;쿠폰&quot;이란 서비스 이용 시 할인 또는 무상 이용 혜택을 받을 수 있도록 회사가 발행하는 것을 말합니다.</p>
          ...
          <p style={st.h2}>제4조 (약관 외 준칙)</p>
          <p style={st.p}>① 이 약관에서 정하지 않은 사항과 이 약관의 해석에 관하여는 전자상거래 등에서의 소비자보호에 관한 법률, 약관의 규제에 관한 법률, 콘텐츠산업 진흥법 등 관계 법령에 따릅니다.</p>
          <p style={st.p}>② 회사는 개별 기능·이벤트에 대하여 운영정책 또는 개별 안내를 둘 수 있으며, 그 내용이 이 약관과 다른 경우 해당 운영정책·안내가 우선하여 적용됩니다.</p>

          <p style={st.h2}>제5조 (이용자에 대한 통지)</p>
          <p style={st.p}>① 회사는 이용자에게 알릴 사항이 있는 경우 서비스 내 공지, 화면 표시, 앱 알림 등의 방법으로 통지할 수 있습니다.</p>
          <p style={st.p}>② 이용자 전체에 대한 통지는 서비스 내에 7일 이상 게시함으로써 갈음할 수 있습니다. 다만 이용자의 거래에 중대한 영향을 미치는 사항은 개별적으로 알립니다.</p>

          <p style={st.h2}>제6조 (이용계약의 체결)</p>
          <p style={st.p}>① 서비스의 일부는 로그인 없이 이용할 수 있으며, 코인 구매·클라우드 보관 등 일부 기능은 회원만 이용할 수 있습니다.</p>
          <p style={st.p}>② 만 14세 미만은 서비스를 이용할 수 없습니다.</p>
          <p style={st.p}>③ 만 14세 이상의 미성년자가 유료 결제를 하는 경우 법정대리인의 동의가 필요하며, 동의 없이 이루어진 미성년자의 결제는 관련 법령에 따라 취소될 수 있습니다.</p>
          <p style={st.p}>④ 타인의 계정·정보 도용, 부정한 용도의 이용, 기타 법령·약관 위반이 확인되는 경우 회사는 이용을 승낙하지 않거나 이용계약을 해지할 수 있습니다.</p>

          <p style={st.h2}>제7조 (계정 관리)</p>
          ...
          <p style={st.h2}>제9조 (서비스의 변경·중단)</p>
          ...
          <p style={st.h2}>제11조 (쿠폰)</p>
          <p style={st.p}>① 회사는 쿠폰을 발행할 수 있으며, 쿠폰의 사용 조건(대상·혜택·유효기간·수량 등)은 발행 시 고지한 바에 따릅니다.</p>
          <p style={st.p}>② 유효기간이 지난 쿠폰은 자동으로 소멸되며 복구·연장되지 않습니다.</p>
          <p style={st.p}>③ 쿠폰은 타인에게 양도·판매할 수 없으며, 현금으로 교환되지 않습니다.</p>
          <p style={st.p}>④ 무상으로 지급된 쿠폰은 환불 대상이 아니며, 할인 쿠폰을 사용한 결제를 환불하는 경우 실제 결제한 금액을 기준으로 하고 사용된 쿠폰은 재발급되지 않습니다.</p>
          <p style={st.p}>⑤ 부정한 방법으로 쿠폰을 취득·사용한 경우 회사는 쿠폰 회수, 이용 제한 등의 조치를 할 수 있습니다.</p>

          <p style={st.h2}>제12조 (청약철회 및 환불)</p>
          ...
          <p style={st.h2}>제13조 (업로드 콘텐츠에 대한 권리와 책임)</p>
          ...
          <p style={st.h2}>제14조 (생성물의 권리와 이용)</p>
          ...
          <p style={st.h2}>제15조 (회사 콘텐츠와 지식재산권)</p>
          <p style={st.p}>① 서비스에 포함된 예시 이미지, 디자인, 문구, 소프트웨어, 상표(MOSPIC) 등 회사가 제작·제공하는 콘텐츠에 대한 지식재산권은 회사 또는 정당한 권리자에게 귀속됩니다.</p>
          <p style={st.p}>② 이용자는 회사의 사전 동의 없이 회사 콘텐츠를 복제·배포·전송·수정하거나 2차적 저작물을 작성하는 등의 방법으로 이용할 수 없습니다. 이용자 본인의 생성물 이용(제14조)은 이에 해당하지 않습니다.</p>
          <p style={st.p}>③ 회사는 이용자에게 서비스를 이 약관에 따라 이용할 수 있는 비독점적·양도 불가능한 권한을 부여하며, 이는 서비스 이용 목적 외의 어떠한 권리도 부여하지 않습니다.</p>

          <p style={st.h2}>제16조 (공적 증명 용도 제한)</p>
          ...
          <p style={st.p}>6. 서비스 또는 서버에 과도한 부하를 일으키거나 정상적인 운영을 방해하는 행위, 서비스의 보안 체계를 침해·우회하는 행위</p>
          <p style={st.p}>7. 스팸 등 원치 않는 광고성 정보를 전송하는 행위</p>
          <p style={st.p}>8. 회사와 제휴·보증 관계가 있는 것처럼 제3자를 오인하게 하는 행위</p>
          <p style={st.p}>9. 회사 또는 제3자의 지식재산권·초상권 등 권리를 침해하는 행위</p>
          <p style={st.p}>10. 기타 관련 법령, 이 약관 및 운영정책을 위반하는 행위</p>
          <p style={st.p}>② 회사는 위반 행위에 대해 사전 통지 후(긴급한 경우 사후 통지) 이용 제한, 계약 해지 등의 조치를 할 수 있으며, 필요한 경우 수사기관에 협조할 수 있습니다.</p>

          <p style={st.h2}>제18조 (피드백)</p>
          <p style={st.p}>이용자가 서비스에 관하여 제공하는 의견·제안은 자발적인 것으로, 회사는 이를 별도의 보상 없이 서비스 개선에 활용할 수 있습니다.</p>

          <p style={st.h2}>제19조 (회원 탈퇴 및 계약 해지)</p>
          <p style={st.p}>① 회원은 언제든지 서비스 내 기능 또는 고객센터를 통해 탈퇴할 수 있습니다.</p>
          <p style={st.p}>② 탈퇴 시 클라우드 히스토리 등 회원 데이터는 개인정보처리방침에 따라 삭제·파기되며, 잔여 코인·쿠폰은 소멸됩니다. 유상 코인의 환불을 원하는 경우 탈퇴 전에 제12조의 절차를 이용해야 합니다.</p>
          <p style={st.p}>③ 회사가 제17조에 따라 계약을 해지하는 경우 부정하게 취득한 혜택은 회수될 수 있습니다.</p>

          <p style={st.h2}>제20조 (회사의 의무)</p>
          <p style={st.p}>회사는 관련 법령과 이 약관을 준수하고 안정적인 서비스 제공을 위해 노력하며, 이용자의 정당한 의견과 불만을 적절한 절차로 처리합니다.</p>

          <p style={st.h2}>제21조 (면책)</p>
          <p style={st.p}>① 생성물은 AI가 만든 이미지로서 실제 인물·사물과의 유사도, 품질, 특정 목적에 대한 적합성이 보장되지 않으며, 결과가 이용자의 기대와 다르다는 사정만으로는 회사가 책임을 지지 않습니다. 다만 제10조 제2항 및 제12조 제4항은 그대로 적용됩니다.</p>
          ...
          <p style={st.h2}>제22조 (손해배상)</p>
          <p style={st.p}>회사 또는 이용자가 이 약관을 위반하여 상대방에게 손해를 입힌 경우 그 손해를 배상할 책임이 있습니다. 회사의 책임은 회사의 고의 또는 과실이 있는 경우로 한정됩니다.</p>

          <p style={st.h2}>제23조 (일반 조항)</p>
          <p style={st.p}>① 이 약관의 일부 조항이 법원에 의해 무효로 판단되더라도 나머지 조항의 효력에는 영향이 없습니다.</p>
          <p style={st.p}>② 회사가 이 약관상의 권리를 즉시 행사하지 않더라도 이를 포기한 것으로 보지 않습니다.</p>
          <p style={st.p}>③ 이용자는 회사의 동의 없이 이 약관상의 지위 또는 권리·의무를 제3자에게 양도할 수 없습니다.</p>

          <p style={st.h2}>제24조 (개인정보 보호)</p>
          <p style={st.p}>회사는 이용자의 개인정보를 관련 법령 및 개인정보처리방침에 따라 보호합니다.</p>

          <p style={st.h2}>제25조 (준거법 및 관할)</p>
