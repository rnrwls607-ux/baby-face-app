...
export default function IdBlacktieGrayPage() {
  ...
  const COIN_GATED = LIVE_COIN_CONCEPTS.includes("idblacktie");
  const COIN_COST = CONCEPTS.idblacktie?.coinCost ?? 0;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      void addToHistory(data.output, "증명사진 (블랙정장+넥타이)", Array.isArray(data.originalUrls) ? data.originalUrls : undefined);
    } catch (e: unknown) {
      clearTimeout(tid);
  ...
  const handleDownload = (url: string, idx: number) => { void saveImage(url, `id-blacktie-gray-${idx + 1}.png`); };
  const handleShare = (url: string, idx: number) => { void shareImage(url, `id-blacktie-gray-${idx + 1}.png`, "MOSPIC에서 만든 사진이에요 · mospic.com"); };
      ...
      <div style={{ padding: "18px 18px 100px" }}>
        {results.length === 0 && (
          <>
            ...
            {BA_LIVE.includes("idblacktie") ? (
              <BeforeAfterHero pairs={[1, 2, 3].flatMap(n => [
                { before: `/examples/ba/idblacktie-before-${n}.webp`, after: `/examples/ba/idblacktie-after-${n}.webp` },
                { before: `/examples/ba/idblacktie-before.webp`, after: `/examples/ba/idblacktie-after-${n}.webp` },
              ])} />
            ) : (
              <PreviewCard image="/details/idblacktie.webp" caption="블랙정장+넥타이 증명사진, 미리 만나보세요" accent={ACCENT} />
            )}
