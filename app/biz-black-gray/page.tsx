...
export default function BizBlackGrayPage() {
  ...
  const COIN_GATED = LIVE_COIN_CONCEPTS.includes("bizblack");
  const COIN_COST = CONCEPTS.bizblack?.coinCost ?? 0;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      void addToHistory(data.output, "비즈프로필 (블랙)", Array.isArray(data.originalUrls) ? data.originalUrls : undefined);
    } catch (e: unknown) {
      clearTimeout(tid);
  ...
  const handleDownload = (url: string, idx: number) => { void saveImage(url, `biz-black-gray-${idx + 1}.png`); };
  const handleShare = (url: string, idx: number) => { void shareImage(url, `biz-black-gray-${idx + 1}.png`, "MOSPIC에서 만든 사진이에요 · mospic.com"); };
      ...
      <div style={{ padding: "18px 18px 100px" }}>
        {results.length === 0 && (
          <>
            ...
            {BA_LIVE.includes("bizblack") ? (
              <BeforeAfterHero pairs={[1, 2, 3].flatMap(n => [
                { before: `/examples/ba/bizblack-before-${n}.webp`, after: `/examples/ba/bizblack-after-${n}.webp` },
                { before: `/examples/ba/bizblack-before.webp`, after: `/examples/ba/bizblack-after-${n}.webp` },
              ])} />
            ) : (
              <PreviewCard image="/details/bizblack.webp" caption="블랙 정장 프로필, 미리 만나보세요" accent={ACCENT} />
            )}
