...
export default function BizBlackGrayPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/biz-black-gray", {
        method: "POST",
      ...
      void addToHistory(data.output, "비즈프로필 (블랙)");
    } catch (e: unknown) {
      clearTimeout(tid);
  ...
  const handleDownload = (url: string, idx: number) => {
    ...
    a.download = `biz-black-gray-${idx + 1}.png`;
  ...
  return (
    <div style={{ maxWidth: 480, margin: "0 auto", minHeight: "100vh", background: "#F7F8FA", fontFamily: "var(--font-noto), 'Apple SD Gothic Neo', sans-serif" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "0 12px", height: 56, position: "sticky", top: 0, background: "#fff", zIndex: 10 }}>
        ...
        <span style={{ fontSize: 16, fontWeight: 800, color: "#191919" }}>블랙 정장 프로필</span>
      </div>

      <div style={{ padding: "18px 18px 100px" }}>
        <div style={{ background: "#F0F1F4", borderRadius: 16, padding: "16px 18px", marginBottom: 22 }}>
          <p style={{ fontSize: 14, fontWeight: 800, color: "#2B2F38", margin: "0 0 5px" }}>🖤 격식을 갖춘 블랙 정장</p>
          <p style={{ fontSize: 12.5, color: "#6A6E78", margin: 0, lineHeight: 1.55 }}>클래식한 블랙 정장과 깔끔한 밝은 회색 배경의 전문가 프로필. 정면 사진 3~6장을 올리면, 팔짱·손모음·자연스러운 각도까지 서로 다른 포즈의 프로필 3장을 만들어드려요.</p>
        </div>
          ...
          <>
            <div style={{ background: "#fff", borderRadius: 20, padding: "20px 18px", boxShadow: "0 2px 16px rgba(0,0,0,0.04)" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                ...
                <span style={{ fontSize: 12, fontWeight: 700, color: images.length >= MIN_PHOTOS ? "#2B2F38" : "#9B9B9B" }}>{images.length}/{MAX_PHOTOS}장</span>
              </div>

              <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 8 }}>
                {images.map((img, idx) => (
                  <div key={idx} style={{ position: "relative", aspectRatio: "1", borderRadius: 12, overflow: "hidden", border: "1.5px solid #2B2F38" }}>
              ...
              <div style={{ marginTop: 14, background: "#F4F4F6", borderRadius: 12, padding: "13px 14px" }}>
                <p style={{ fontSize: 12, fontWeight: 800, color: "#2B2F38", margin: "0 0 7px" }}>📸 이렇게 찍으면 더 잘 나와요</p>
            ...
            <button onClick={handleSubmit} disabled={loading || images.length < MIN_PHOTOS}
              style={{ width: "100%", marginTop: 18, background: loading || images.length < MIN_PHOTOS ? "#E8E9ED" : "#2B2F38", color: loading || images.length < MIN_PHOTOS ? "#AEB2BA" : "#fff", border: "none", borderRadius: 16, padding: "16px 0", fontSize: 16, fontWeight: 800, cursor: loading || images.length < MIN_PHOTOS ? "not-allowed" : "pointer", boxShadow: loading || images.length < MIN_PHOTOS ? "none" : "0 6px 18px rgba(43,47,56,0.32)" }}>
              {loading ? `만드는 중... (${elapsed}초)` : images.length < MIN_PHOTOS ? `사진을 ${MIN_PHOTOS}장 이상 올려주세요` : "프로필 3장 만들기 ✨"}
            </button>
          </>
        )}
              ...
              {results.map((url, idx) => (
                <div key={idx}>
                  <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 8 }}>
                    <span style={{ fontSize: 13, fontWeight: 800, color: "#2B2F38", background: "#F0F1F4", borderRadius: 8, padding: "3px 10px" }}>{idx + 1}번</span>
                  </div>
                  <div style={{ borderRadius: 16, overflow: "hidden", marginBottom: 10, boxShadow: "0 4px 20px rgba(0,0,0,0.08)" }}>
                    <img src={url} alt={`비즈프로필 ${idx + 1}`} style={{ width: "100%", display: "block" }} />
                  </div>
                  <button onClick={() => handleDownload(url, idx)}
                    style={{ width: "100%", background: "#2B2F38", color: "#fff", border: "none", borderRadius: 14, padding: "14px 0", fontSize: 14, fontWeight: 800, cursor: "pointer", boxShadow: "0 6px 18px rgba(43,47,56,0.3)" }}>이 사진 저장하기</button>
                </div>
              ))}
            </div>
